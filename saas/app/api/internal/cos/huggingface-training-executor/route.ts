...
function signedResponse(input: { body: Record<string, unknown>; idempotencyKey: string; secret: string; status?: number }) {
  const rawBody = JSON.stringify(input.body)
  const timestamp = new Date().toISOString()
  const signature = signTrainingExecutorPayload({ secret: input.secret, timestamp, idempotencyKey: input.idempotencyKey, rawBody })
  return new NextResponse(rawBody, { status: input.status ?? 200, headers: { 'content-type': 'application/json', 'cache-control': 'no-store, max-age=0', 'x-itmounts-training-profile': COS_UNIVERSITY_TRAINING_EXECUTOR_PROFILE, 'x-itmounts-training-timestamp': timestamp, 'x-itmounts-training-idempotency-key': input.idempotencyKey, 'x-itmounts-training-signature': signature } })
}
...
export async function POST(req: NextRequest) {
  ...
  if (!executor || !hf) return NextResponse.json({ ok: false, error: 'huggingface_training_not_configured' }, { status: 503 })
  if (process.env.COS_UNIVERSITY_TRAINING_EXECUTOR_DISPATCH_ENABLED !== 'true') return NextResponse.json({ ok: false, error: 'training_executor_dispatch_disabled' }, { status: 503 })
  ...
  if (profile !== COS_UNIVERSITY_TRAINING_EXECUTOR_PROFILE || !idempotencyKey || !verifyTrainingExecutorPayload({ secret: executor.secret, timestamp, idempotencyKey, rawBody, signature })) return NextResponse.json({ ok: false, error: 'huggingface_training_signature_invalid' }, { status: 401 })
  ...
  try {
    ...
    const spec = buildHuggingFaceJobSpec({ envelope, callbackUrl, idempotencyKey, callbackSecret: executor.secret, config: hf })
    ...
    return signedResponse({ body: { accepted: true, jobId: submitted.jobId, provider: 'huggingface-jobs', jobUrl: submitted.jobUrl, flavor: hardware.flavor, hourlyCostUsd: hardware.hourlyCostUsd, maxEstimatedCostUsd }, idempotencyKey, secret: executor.secret })
  } catch (error) {
    const reason = error instanceof Error ? error.message.split(':')[0] : 'huggingface_training_submission_failed'
    return signedResponse({ body: { accepted: false, error: reason }, idempotencyKey, secret: executor.secret, status: 400 })
  }
}
