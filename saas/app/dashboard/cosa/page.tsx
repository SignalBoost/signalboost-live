...
const text = {
  ...
  created: 'COSA created the campaign and prepared an internal review draft.',
  ...
  errorRender: 'Could not start the video render.',
  ...
  rendering: 'Rendering video… this appears here when ready.',
  ready: 'Video ready for review.',
  branding: 'Voice and brand banner are being prepared. Video appears here only when the banner is burned in.',
}

const card = { background: 'rgba(15,23,42,.72)', border: '1px solid rgba(255,255,255,.09)', borderRadius: 18, padding: 20, boxShadow: '0 20px 60px rgba(0,0,0,.28)' }
const primary = { border: 'none', background: GOLD, color: '#000', borderRadius: 12, padding: '10px 14px', fontWeight: 900, cursor: 'pointer' }
const secondary = { border: '1px solid rgba(255,255,255,.16)', background: 'rgba(255,255,255,.06)', color: '#fff', borderRadius: 12, padding: '10px 14px', fontWeight: 800, cursor: 'pointer' }
const muted = { color: 'rgba(255,255,255,.62)', lineHeight: 1.6 }
...
function CampaignCard({ campaign, busy, onPatch, onGenerateDraft, onGenerateAll, onPublish, onRenderVideo, onCheckStatus }: { campaign: CampaignRow; busy: boolean; onPatch: (id: string, status: 'approved' | 'rejected' | 'queued') => void; onGenerateDraft: (id: string) => void; onGenerateAll: (id: string) => void; onPublish: (id: string, language?: string) => void; onRenderVideo: (id: string) => void; onCheckStatus: (id: string) => void }) {
  ...
  return <section style={{ ...card, position: 'relative', overflow: 'hidden' }}>
...
export default function MarketingSalesCosaPage() {
  ...
  async function load(keepMessage = false) { setLoading(true); if (!keepMessage) setMessage(''); try { const [admRes, campaignRes] = await Promise.all([fetch('/api/admin/adm', { cache: 'no-store' }), fetch('/api/cos/campaign-queue', { cache: 'no-store' })]); const admJson = await admRes.json().catch(() => null); const campaignJson = await campaignRes.json().catch(() => null); if (!admRes.ok || !campaignRes.ok) throw new Error(text.errorLoad); setOutreach(Array.isArray(admJson?.recentOutreach) ? admJson.recentOutreach : []); setCampaigns(Array.isArray(campaignJson?.campaigns) ? campaignJson.campaigns : []) } catch { setMessage(text.errorLoad) } finally { setLoading(false) } }
  useEffect(() => { load() }, [])
  ...
  async function generateDraft(id: string) { const res = await fetch('/api/cos/script-worker', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ campaign_id: id }) }); if (!res.ok) throw new Error(text.errorDraft); return res.json().catch(() => null) }
  async function createAutonomousCampaign(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!autonomousDirective.trim()) { setMessage(text.requiredCommand); return } setBusy(true); try { const res = await fetch('/api/cos/campaign-queue', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ directive: autonomousDirective }) }); const json = await res.json().catch(() => null); if (!res.ok) throw new Error(text.errorAuto); if (json?.campaign?.id) await generateDraft(json.campaign.id); setMessage(text.created); await load(true) } catch { setMessage(text.errorAuto) } finally { setBusy(false) } }
  async function patchCampaign(id: string, status: 'approved' | 'rejected' | 'queued') { setBusy(true); try { const res = await fetch('/api/cos/campaign-queue', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, status }) }); if (!res.ok) throw new Error(text.errorMark); setMessage(`${text.marked} ${status}.`); await load(true) } catch { setMessage(text.errorMark) } finally { setBusy(false) } }
  async function generateDraftFromButton(id: string) { setBusy(true); try { await generateDraft(id); setMessage(text.draftDone); await load(true) } catch { setMessage(text.errorDraft) } finally { setBusy(false) } }
  async function generateAllFromButton(id: string) { setBusy(true); try { const res = await fetch('/api/cos/script-worker/batch', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ campaign_id: id }) }); if (!res.ok) throw new Error(text.errorBatch); setMessage(text.batchDone); await load(true) } catch { setMessage(text.errorBatch) } finally { setBusy(false) } }
  async function publishLanguage(id: string, language?: string) { setBusy(true); try { const res = await fetch('/api/cos/campaign-queue/publish', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, language }) }); const j = await res.json().catch(() => null); if (!res.ok || !j?.ok) throw new Error(j?.error || text.errorPublish); setMessage(text.published); await load(true) } catch { setMessage(text.errorPublish) } finally { setBusy(false) } }
  async function renderVideo(id: string) { setBusy(true); try { const res = await fetch('/api/cos/campaign-queue/render-video', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) }); const j = await res.json().catch(() => null); if (!res.ok || !j?.ok) throw new Error(j?.error || text.errorRender); setMessage(text.rendering); await load(true) } catch { setMessage(text.errorRender) } finally { setBusy(false) } }
  async function checkRenderStatus(id: string) { setBusy(true); try { const res = await fetch('/api/cos/campaign-queue/render-status', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) }); const j = await res.json().catch(() => null); if (!res.ok || !j?.ok) throw new Error(j?.error || text.errorRender); setMessage(j.status === 'ready' ? text.ready : j.status === 'failed' ? (j.error || 'Video failed') : text.rendering); await load(true) } catch (e: any) { setMessage(e?.message || text.errorRender) } finally { setBusy(false) } }
  return <main style={{ maxWidth: 1280, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 20 }}><section style={{ background: 'linear-gradient(145deg, rgba(15,23,42,0.96), rgba(2,6,23,0.98))', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 24, padding: 28, boxShadow: '0 28px 80px rgba(0,0,0,0.38)' }}><div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 18, flexWrap: 'wrap' }}><div><p style={{ margin: 0, color: GOLD, fontSize: 12, fontWeight: 950, letterSpacing: '.12em', textTransform: 'uppercase' }}>Marketing & Sales Department</p><h1 style={{ color: '#fff', fontSize: 34, lineHeight: 1.05, letterSpacing: '-0.04em', margin: '10px 0 0', fontWeight: 950 }}>{text.title}</h1><p style={{ color: 'rgba(255,255,255,0.68)', maxWidth: 820, lineHeight: 1.7, marginTop: 14 }}>{text.intro}</p></div><button onClick={() => load()} disabled={loading || busy} style={secondary}>{loading ? text.loading : text.refresh}</button></div></section>{message && <div style={{ border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(15,23,42,0.75)', color: '#fff', padding: '12px 16px', borderRadius: 14 }}>{message}</div>}<section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 14 }}>{[['Campaigns', campaigns.length], ['Awaiting approval', stats.waiting], ['Approved', stats.approved], ['Running', stats.running], ['Drafted', stats.drafted], ['Outreach approvals', outreach.filter(row => row.status === 'pending').length]].map(([label, value]) => <section key={String(label)} style={{ ...card, padding: 16 }}><p style={{ margin: 0, color: 'rgba(255,255,255,.55)', fontSize: 12, fontWeight: 800 }}>{label}</p><strong style={{ display: 'block', color: '#fff', marginTop: 6, fontSize: 28 }}>{String(value)}</strong></section>)}</section><section style={{ ...card, background: 'linear-gradient(145deg, rgba(255,195,0,0.11), rgba(15,23,42,0.72))', border: '1px solid rgba(255,195,0,0.24)' }}><p style={{ margin: 0, color: GOLD, fontSize: 12, fontWeight: 950, letterSpacing: '.12em', textTransform: 'uppercase' }}>{text.command}</p><h2 style={{ color: '#fff', margin: '8px 0 0', fontSize: 20 }}>Tell COSA what to create</h2><p style={muted}>COSA creates a governed campaign draft. Public publishing stays locked until approval.</p><form onSubmit={createAutonomousCampaign} style={{ display: 'grid', gap: 12, marginTop: 16 }}><label style={{ display: 'grid', gap: 8, color: 'rgba(255,255,255,.72)', fontSize: 12, fontWeight: 800 }}>{text.command}<textarea value={autonomousDirective} onChange={e => setAutonomousDirective(e.target.value)} placeholder="Create a YouTube promotional video campaign for SignalBoostAi. Feature www.saas.signalboostapp.com." rows={4} style={{ width: '100%', resize: 'vertical', borderRadius: 14, border: '1px solid rgba(255,255,255,.12)', background: 'rgba(2,6,23,.65)', color: '#fff', padding: 12, outline: 'none' }} /></label><button type="submit" disabled={busy} style={primary}>{busy ? text.working : text.run}</button></form></section><section style={card}><h2 style={{ color: '#fff', margin: 0, fontSize: 20 }}>Campaign repository</h2><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 14, marginTop: 14 }}>{loading && <p style={muted}>{text.loading}</p>}{!loading && campaigns.length === 0 && <p style={muted}>{text.noCampaigns}</p>}{campaigns.map(campaign => <CampaignCard key={campaign.id} campaign={campaign} busy={busy} onPatch={patchCampaign} onGenerateDraft={generateDraftFromButton} onGenerateAll={generateAllFromButton} onPublish={publishLanguage} onRenderVideo={renderVideo} onCheckStatus={checkRenderStatus} />)}</div></section></main>
}
