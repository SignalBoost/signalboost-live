...
import { recordCosUniversityDistillationTeacherDatasetEvidence } from '@/lib/ai/cos/cosUniversityDistillationTeacherDataset'
...
export async function POST(req: NextRequest) {
  ...
  if (profile !== COS_UNIVERSITY_TRAINING_EXECUTOR_PROFILE || !idempotencyKey || !verifyTrainingExecutorPayload({
    ...
    return NextResponse.json({ ok: false, error: 'teacher_dataset_signature_invalid' }, { status: 401 })
  }

  let body: any = null
  try { body = JSON.parse(rawBody) } catch { body = null }
  if (!body || typeof body !== 'object' || Array.isArray(body)) {
    return NextResponse.json({ ok: false, error: 'teacher_dataset_payload_invalid' }, { status: 400 })
  }

  try {
    const result = await recordCosUniversityDistillationTeacherDatasetEvidence(body, { idempotencyKey })
    return NextResponse.json(result, { headers: { 'Cache-Control': 'no-store, max-age=0' } })
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error)
    const status = message.startsWith('teacher_dataset_') || message.startsWith('distillation_dataset_') ? 400 : 500
