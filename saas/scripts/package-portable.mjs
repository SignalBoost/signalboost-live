...
function stripComments(source) {
  // Line comments are only stripped when the whole line is one, so a '//' inside
  // a URL string is untouched.
  return source.replace(/\/\*[\s\S]*?\*\//g, '').replace(/^\s*\/\/.*$/gm, '');
}

function specifiersIn(rawSource) {
...
// payloadRoot may be one directory or several. With one, that directory is
// stripped from the buyer-facing paths. With several, full repo-relative paths
// are kept, because two roots can each contain an index.ts and a buyer must not
// receive two files that claim the same path.
...
for (const { rel, source } of [...visited.values()].sort((a, b) => a.rel.localeCompare(b.rel))) {
  ...
  if (!root) {
    fail(
      `graph reached "${rel}", which is outside every declared payloadRoot ` +
        `(${payloadRoots.join(', ')}).\n` +
...
if (!NO_ARCHIVE) {
  ...
  // Build the archive OUTSIDE the directory being archived, then move it in.
  // Writing it in place makes tar walk over its own output file and record an
  // entry for it, so the archive ends up containing a truncated copy of itself.
  const staged = path.join(outDir, '..', tgz);
  fs.rmSync(staged, { force: true });
  execFileSync(
    'tar',
    ['--sort=name', '--mtime=@0', '--owner=0', '--group=0', '--numeric-owner', '-czf', staged, '-C', outDir, '.'],
    { cwd: ROOT },
  );
