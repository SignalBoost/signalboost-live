...
test('GitHub route and migration wire authenticated, serialized, service-role-only persistence', () => {
  ...
  const productionHardening = readFileSync(new URL('../supabase/migrations/20260911012329_security_repository_patrol_evidence_rpc_only_hardening.sql', import.meta.url), 'utf8')
  ...
  assert.match(route, /event === 'ping'/)
  assert.match(route, /payload\?\.repository\?\.owner\?\.id/)
  assert.match(route, /payload_hash: payloadHash/)
  assert.match(route, /safe_metadata: \{ repositoryFullName, payloadDigest: payloadHash \}/)
  assert.doesNotMatch(route, /repository_full_name:/)
  assert.doesNotMatch(route, /payload_digest:/)
  ...
  assert.match(productionHardening, /revoke insert, update, delete, truncate, references, trigger/)
  assert.match(productionHardening, /revoke usage, select, update[\s\S]*security_repository_patrol_evidence_id_seq/)
  assert.match(productionHardening, /set search_path = ''/)
  assert.match(productionHardening, /before update or delete/)
})
