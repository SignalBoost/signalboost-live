...
test('approved source remediation uses deterministic i18n plus governed AI for security findings', () => {
  const engine = read('../lib/audit/approvedRunRemediation.ts')
  ...
  assert.match(engine, /applyModelFindings/)
  assert.match(engine, /Return the COMPLETE replacement file and nothing else/)
  assert.match(engine, /findBadImports/)
  assert.match(engine, /preservedFraction/)
  assert.match(engine, /MIN_PRESERVED_FRACTION/)
  assert.match(engine, /commitFileToBranch/)
  assert.match(engine, /never write directly to main/i)
})
...
test('transient GitHub failures retry without repeating owner approval', () => {
  ...
  for (const route of [approval, runs, cron]) assert.match(route, /runApprovedAuditRemediationWithRetry/)
})
