...
function allPassingEvidence(): MarketingSalesRcEvidenceMap {
  return Object.fromEntries(MARKETING_SALES_RC_REQUIREMENTS.map(requirement => [
    requirement.checkId,
    { status: 'pass' as const, summary: `${requirement.summary} Verified in the real target environment.`, evidence: [{ ref: `evidence://${requirement.checkId}`, kind: 'report' as const, observedAt: generatedAt }] },
...
test('Marketing & Sales evidence coverage exposes exactly what remains unproven', () => {
  ...
  evidence['marketing_sales.performance.load_soak'] = { status: 'warn', evidence: [{ ref: 'evidence://partial-load', kind: 'metric', observedAt: generatedAt }] }
...
test('a warning on any required Marketing & Sales gate prevents release-candidate status', () => {
  const evidence = allPassingEvidence()
  evidence['marketing_sales.performance.load_soak'] = { status: 'warn', summary: 'Load/soak evidence is incomplete.', evidence: [{ ref: 'evidence://load-soak-partial', kind: 'metric', observedAt: generatedAt }] }
