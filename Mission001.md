...
## Mission 001 Sprint — Durable Federated Supervisor Coordination Store

The active-active Supervisor control plane now has a Supabase/Postgres durable coordination authority for production. The previous in-memory authority is retained only for deterministic unit tests and explicit local test injection; production must fail closed when the durable store is unavailable and must never silently fall back to in-memory ownership.

Durable coordination uses `supervisor_instances`, `supervisor_work_items`, `supervisor_leases`, and `supervisor_coordination_events`. Atomic lease acquisition is performed by the `supervisor_acquire_lease` Postgres RPC, which locks the work item and owner runtime, rejects unexpired active leases, increments the work-item `fencing_generation` exactly once, inserts the active lease, and transitions the work item to `leased` in one transaction. Protected writes use `supervisor_assert_fence`, `supervisor_renew_lease`, `supervisor_release_lease`, and `supervisor_transition_work_item` so stale instance/runtime identities, expired leases, changed fencing tokens, terminal work, or inconsistent state fail closed before executor invocation or completion.

Work enqueueing is idempotent by provider plus tenant/organization plus idempotency key. Equivalent requests return the existing work item; conflicting unsafe metadata is rejected. Provider, tenant, project, and resource scope remain explicit fields on durable work items so provider workers cannot claim another provider's work and tenant reads can be bounded by server-side authorization.

Expired-lease reconciliation is idempotent. API/idempotent work can be requeued only when attempts remain and policy/capability metadata still permits retry. Browser work is non-migratable: owner/runtime loss marks it abandoned, pending continuation approval must be invalidated by the live-session layer, and durable records cannot reconstruct a lost Browser/Page/Context or reuse approval tokens. Browser retries require a new execution ID, new policy decision, and new Browser Runtime approvals.

RLS is enabled on the coordination tables. Anonymous reads are denied, public-client writes are not granted, mutation RPCs are revoked from anonymous/authenticated client roles, and server-side service-role paths are the approved mutation boundary. Read-only internal APIs expose sanitized instance/work/lease/event visibility for authenticated operators only; they do not acquire, release, execute, approve, resume Browser Runtime, or mutate providers.
