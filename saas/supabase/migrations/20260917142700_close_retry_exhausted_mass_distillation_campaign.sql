-- Terminalize a mass-distillation campaign when every remaining failure is no longer retryable.
-- This releases the rolling campaign-in-progress lock without expanding provider, retry, spend,
-- promotion, Production-traffic, or RunPod authority.
...
begin
  ...
  if v_rearmed > 0 then
    ...
    where c.id=p_campaign_id;
  elsif v_retry_exhausted > 0
    and v_budget_blocked=0
    ...
    if v_nonterminal_runs=0 then
      update public.cos_university_mass_distillation_campaigns c
      set status='failed', completed_at=coalesce(c.completed_at,v_now), updated_at=v_now
      where c.id=p_campaign_id and c.status in ('authorized','active','running','failed');
      ...
      if v_campaign_terminalized then
        insert into public.cos_university_learning_assurance_events (
          ...
          encode(extensions.digest(convert_to(p_campaign_id::text||':retry_exhausted:'||v_retry_exhausted::text,'UTF8'),'sha256'),'hex'),
          jsonb_build_object(
            'profile','cos-university-mass-distillation-campaign-v1',
            'claim','mass_distillation_campaign_terminalized',
            'campaignId',p_campaign_id,
            'terminalStatus','failed',
            'reason','provider_stage_retry_exhausted',
            'retryExhaustedRuns',v_retry_exhausted,
            'maxAcceptedAttemptsPerStage',2,
            'unsettledProviderJobs',0,
            'nonterminalRuns',0,
  ...
  insert into public.cos_university_learning_assurance_events (
    ...
    jsonb_build_object(
      ...
      'retryExhaustedRuns',v_retry_exhausted,
      'campaignTerminalized',v_campaign_terminalized,
      'maxAcceptedAttemptsPerStage',2,
