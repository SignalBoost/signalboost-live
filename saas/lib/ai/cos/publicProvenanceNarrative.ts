...
/**
 * Acceptance gate for a model-written narrative. Rejects (returns null) only when the text is
 * empty/absurdly long, names forbidden specifics, or cites a URL that is not in the verified fact
 * set (an invented source). Everything else — including "from my training", "from my memory",
 * "my general knowledge" — is welcome; that is the owner's requested voice.
 */
...
export function buildPublicProvenanceInstruction(
  ...
  const system = [
    ...
    'Answer in your own voice, conversational and natural, 1 to 3 sentences, specific to THIS exchange — never a stock formula.',
    ...
    'Do not quote the visitor\'s previous request back at them verbatim; refer to it naturally ("your script request", "what you asked for") if at all.',
  ].join(' ')
  const prompt = [
    `VISITOR'S QUESTION: ${cleanText(visitorQuestion, 240) || 'Where did the previous answer come from?'}`,
    facts.originalRequest ? `CONTEXT — WHAT THEY HAD ASKED FOR (do not quote verbatim): ${facts.originalRequest}` : null,
