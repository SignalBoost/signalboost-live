...
**Concierge internal Builder:** implementation is active and not Production-accepted. Concierge is the only user-facing surface: explicit coding, repair, and design objectives invoke the authenticated Builder tool loop in the backend and return its answer, evidence, and file-download links in the same conversation. Ordinary questions remain on normal COS. The legacy redirect to `/dashboard/developer` is removed from the authenticated Assistant; supported text/code attachments are staged into the isolated network-denied workspace. The actual public homepage Concierge, secondary dock Concierge, and owner COS surface render accepted/running/completed request events and elapsed time while waiting. When COS returns a durable Builder `202`, the client follows the returned job id through the authoritative read-only `/api/builder` status endpoint until the terminal result; it does not treat the queued acknowledgement as the final answer or simulate Builder stages. This is operational progress visibility, not disclosure of private chain-of-thought. The Developer workspace may remain an internal diagnostics surface, not a required customer journey. A fresh authenticated Concierge code-repair observation is required before runtime acceptance.
