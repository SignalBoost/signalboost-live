...
const STEPS = [
  { label: 'Build your creator site', desc: 'Publish a hub for your content, links, and brand.', href: '/dashboard/builder' },
  { label: 'Generate native voice & audio', desc: 'Create intros, narration, and audio clips in your language.', href: '/dashboard/audio' },
  { label: 'Create short-form videos', desc: 'Script and produce clips for every platform.', href: '/dashboard/video' },
  { label: 'Grow with promotion', desc: 'Plan outreach, captions, and campaigns to reach more people.', href: '/dashboard/promote' },
  { label: 'Track your audience', desc: 'See your growth signals and next best moves.', href: '/dashboard/metrics' },
]
    ...
    <main style={{ minHeight: '100vh', padding: '40px 24px 80px', background: 'radial-gradient(circle at top left, rgba(255,195,0,.15), transparent 30%),linear-gradient(180deg,#050505,#0f1117)', color: '#fff' }}>
      <div style={{ maxWidth: 880, margin: '0 auto' }}>
        ...
        <div style={{ display: 'inline-flex', marginTop: 16, padding: '5px 12px', borderRadius: 999, background: 'rgba(255,195,0,.1)', border: '1px solid rgba(255,195,0,.2)', color: GOLD, fontWeight: 800, fontSize: 12 }}>
          🎬 CREATOR · {experience.toUpperCase()}
        </div>

        <h1 style={{ fontSize: 'clamp(32px,6vw,56px)', lineHeight: 1.05, margin: '18px 0 10px', letterSpacing: '-.04em' }}>
          Build your creator brand, <span style={{ color: GOLD }}>step by step</span>
        </h1>
        <p style={{ color: 'rgba(255,255,255,.55)', lineHeight: 1.7, maxWidth: 640 }}>
          Turn your idea into a content ecosystem. Describe your brand, then move through each step.
        </p>

        <section style={{ marginTop: 28 }}>
          <label htmlFor="idea" style={{ display: 'block', fontWeight: 800, marginBottom: 8 }}>What's your creator brand about?</label>
          <textarea
            id="idea"
            value={idea}
            onChange={e => saveIdea(e.target.value)}
            rows={3}
            placeholder="e.g. A bilingual channel teaching home cooking to busy parents."
