// saas/lib/portable/state-snapshot-port.ts
//
// TRANSACTIONAL EXECUTION BOUNDARIES — SHARED BY EVERY PORTABLE THAT TOUCHES DATA.
//
// This lives in lib/portable/, not inside the Supervisor, because the rule it encodes is
// not an infrastructure rule. Marketing and Sales bulk-updating five thousand CRM records
// needs the same checkpoint discipline as a deployment rollback; so does anything that
// edits campaign budgets, outreach queues, feeds or permissions. One contract, enforced
// identically everywhere, is what lets a buyer's risk team review the guarantee ONCE
...
import type { SerializableValue } from '../supervisor/incident-schema.ts'
