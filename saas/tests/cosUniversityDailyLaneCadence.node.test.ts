...
test('graduation runner is agent-scoped: own enrollment, residence, evidence, capstone and credential', () => {
  ...
  assert.equal((runner.match(/\.eq\('agent_id', agentId\)/g) || []).length, 6)
  assert.equal((runner.match(/\.eq\('run_key', runKey\)\.eq\('agent_id', agentId\)/g) || []).length, 2, 'both capstone lookup and duplicate retry bind the requested agent')
