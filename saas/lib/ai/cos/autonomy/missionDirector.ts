...
export interface CosMission { missionId: string; purpose: string; priorities: readonly string[]; constraints: readonly string[]; successCriteria: readonly string[] }
export interface CosMissionDecision { decisionId: string; objective: string; priority: 'low' | 'medium' | 'high' | 'critical'; rationale: string; evidenceIds: readonly string[]; shouldAct: boolean; confidence: number }
export interface CosMissionDirector { decide(input: { mission: CosMission; manifest: PortableManifest; observation: PortableObservation; recentDecisions: readonly CosMissionDecision[] }): Promise<CosMissionDecision> }
...
function validateDecision(raw: Record<string, unknown>, observation: PortableObservation): CosMissionDecision {
  ...
  for (const evidenceId of evidenceIds) if (!availableEvidence.has(evidenceId)) throw new Error(`cos_mission_director_invented_evidence:${evidenceId}`)
...
export function createModelMissionDirector(input?: { modelPreference?: 'claude' | 'openai' | 'local'; maxTokens?: number }): CosMissionDirector {
  return {
    async decide({ mission, manifest, observation, recentDecisions }) {
      ...
      const prompt = JSON.stringify({ mission, portable: { portableId: manifest.portableId, portableVersion: manifest.portableVersion, capabilities: manifest.capabilities }, observation, recentDecisions: recentDecisions.slice(-10) })
      const text = await ai.generate({ modelPreference: input?.modelPreference, systemPrompt, prompt, maxTokens: input?.maxTokens ?? 1800 })
