...
**Builder source history — 2026-09-06:** The prior historical explanation wrongly equated current files with an older job. The correction captures bounded source excerpts and SHA-256 fingerprints of full file content before each command-runner dispatch, including automatic repair proof runs, and preserves them through both public trace serializers and checkpoints. Explanation generation/review receive separate historical excerpts and host-computed same/different/unavailable comparisons against the last recorded runner input. Legacy jobs without snapshots remain explicitly unavailable; current files cannot backfill their past. These snapshots identify runner inputs, not post-command or final-job filesystem state, and do not independently prove command execution. No schema or authority change. All 1009 mandatory regressions and TypeScript pass. Deployment and live historical comparison remain pending. See `docs/BUILDER-SOURCE-HISTORY-2026-09-06.md`.

**Builder explanation recovery — live result, 2026-09-06:** #1905 merged at `2901eb6abcde981ebc309b4dc2045436a5600899`, Production `dpl_G4h1WxfWxtGUPUaHowc1QJTFUeUn` READY. All 1005 mandatory regressions, TypeScript, ten CI workflows and exact Preview passed. A rejected draft can receive one evidence-based correction followed by independent review, within the existing initial deadline; rejected proposals remain fail-closed. The live read-only numeric-repair explanation correctly described the TypeError, both source edits, three npm test runs and final fifteen passing tests, with explicit coverage limits and no new job. That response was accepted on first review; production recovery itself was not exercised by this observation. The repaired fixture preserved all existing assertions and passed the unchanged 10,102-case probe plus six targeted checks. **Remaining historical explanation defect:** the older precision-job follow-up correctly distinguished exposed from introduced defects but falsely said current files matched that job's final state, and inaccurately described one rounding condition. Model review accepted it. Do not certify complete explanation fidelity. The next gap is source snapshots/version identity tied to each job and stronger historical comparison evidence. Full chronology and exact limits: `docs/BUILDER-EXPLANATION-FIDELITY-2026-09-06.md`.
...
**Next Builder task:** preserve source snapshots/version identity for each job and use them in historical explanations. Current files must not be presented as an older job's final source without verified comparison. The numeric repair and its follow-up are observed; model review is still probabilistic and accepted an incorrect historical-source claim. After this evidence gap, assess authorized validated-procedure use and inference transfer on unfamiliar tasks. Reuse completed capabilities; do not substitute repeated demonstrations of an accepted flow for missing functionality. Engineering owns implementation and verification. The owner evaluates the completed experience; owner certification is not a prerequisite for engineering work.
