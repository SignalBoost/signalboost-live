...
export default async function PortableProductReadinessPage() {
  ...
  if (!access.isAdmin) return <main style={page}><h1>Portable Product Readiness Dashboard</h1><p style={muted}>Admin access required.</p></main>
  return <main style={page}>
    <section style={panel}><p style={{ color: '#1af0ff', fontWeight: 800 }}>INTERNAL · READ-ONLY</p><h1>Portable Product Readiness Dashboard</h1><p style={muted}>Deterministic metadata analysis from the portable product registry, manifests, dependency graph, documented language coverage, and repository test coverage. “Ready” describes declared metadata readiness only; it does not run tests or create, package, deploy, sell, license, or execute a product.</p><p><a href="/dashboard/portable-products" style={{ color: '#1af0ff' }}>View Portable Product Catalog</a></p></section>
