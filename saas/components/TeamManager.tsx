...
export default function TeamManager({ userId }: { userId: string }) {
  ...
  async function handleInvite() {
    ...
    if (result.error) {
      ...
      setMessage(t(dict, 'team.inviteSent'))
  ...
  const publicPlan = t(dict, `team.plan.${normalized(plan)}`, plan)
  ...
  return (
    <div style={{ color: '#fff', fontFamily: 'system-ui' }}>
      ...
      >
        ...
        >
          {publicPlan} {t(dict, 'team.planSuffix')}
        </div>
      </div>
      ...
      {seatsUsed < seatsAllowed && (
        <div style={{ display: 'flex', gap: 10, marginBottom: 24 }}>
          <input
            ...
            placeholder={t(dict, 'team.emailPlaceholder')}
          ...
          >
            {loading ? t(dict, 'team.sending') : t(dict, 'team.invite')}
          </button>
        </div>
      )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {members.length === 0 ? (
          ...
          members.map((member) => (
            ...
            >
              <div>
                <div style={{ fontSize: 14, fontWeight: 600 }}>{member.member_email}</div>
                <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)', marginTop: 2 }}>
                  {t(dict, `team.role.${normalized(member.role)}`, member.role)} ·{' '}
                  {t(dict, `team.status.${normalized(member.status)}`, member.status)}
                </div>
              </div>
