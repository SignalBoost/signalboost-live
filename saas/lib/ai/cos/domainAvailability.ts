...
export function extractDomainCandidates(input: string, context = ''): string[] {
  ...
  // Concrete domains in the current owner turn are the lookup set even when
  // phrased simply as "check these names". A bare name may additionally resolve
  // to its exact prior candidate; unrelated prior suggestions remain excluded.
  if (found.length) {
    for (const match of context.matchAll(DOMAIN)) {
      const value = String(match[1] || '').toLowerCase().replace(/\.$/, '')
      const label = value.split('.')[0]
      const named = new RegExp(`(?:^|[^a-z0-9-])${label.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}(?:$|[^a-z0-9-])`, 'i').test(input)
      if (named && value && !found.includes(value)) found.push(value)
