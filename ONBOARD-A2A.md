...
- The owner-only qualification operator accepts only exact tenant/environment/portable + candidate/skill identifiers. Probe content and scoring are never caller inputs: a separately installed host assessment port owns the hidden probe, transport, and independent verifier. If that port or the service-role evidence store is unavailable, the operator fails closed.
