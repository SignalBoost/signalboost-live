  ...
  return <main style={page}>
    ...
    <Card title={t.systemDiagnostics}>
      ...
      {diagnosticSummary.attention.length ? <details open style={subcard}><summary>{`${t.diagnosticsNeedingAttention} · ${diagnosticSummary.attention.length}`}</summary>{diagnosticSummary.attention.map(d => { const policy = recommendationPolicy(d.status); return <article key={d.subsystemId} style={mini}>
        ...
        <details style={subcard}><summary>{t.explainThisFinding}</summary>
          <dl style={fields}><Field k={t.reasonLabel} v={d.explanation}/><Field k={t.evidence} v={d.evidence.join(' · ')}/><Field k={t.reasoningLabel} v={d.reasoning}/><Field k={t.reasoningConfidence} v={d.reasoningConfidence.label}/><Field k={t.changesWhen} v={d.changesWhen}/></dl>
          <ul style={plainList}>{d.reasoningConfidence.factors.map(factor => <li key={factor.statement} style={factor.supports ? supportItem : conflictItem}>{factor.statement}</li>)}</ul>
