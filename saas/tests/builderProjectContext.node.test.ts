...
test('bare npm test becomes one staged proof file', () => {
  assert.equal(
    normalizeBuilderSandboxCommand('npm test', [{ path: 'tests/builderAsyncJobs.node.test.ts' }]),
    'node --experimental-strip-types --test tests/builderAsyncJobs.node.test.ts',
  )
})
