...
export async function POST(req:NextRequest){
  ...
  try {
    ...
    return NextResponse.json({ok:true,curriculumQuestions:allGaps.length,sourceAdapters:adapters.map(a=>a.kind),batch:{offset,size:batch.length,nextOffset,done:nextOffset>=allGaps.length},result})
  } catch (error) {
