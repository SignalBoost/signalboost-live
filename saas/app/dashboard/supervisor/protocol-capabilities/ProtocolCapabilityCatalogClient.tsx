...
export default function ProtocolCapabilityCatalogClient({ labels }: { labels: ProtocolCatalogLabels }) {
  ...
  return <main style={page} aria-labelledby="protocol-catalog-title" aria-busy={!snapshot && !error}>
    <header style={{ display:'grid', gap:8 }}><div style={eyebrow}><LocalizedText fallback={uiCopy('u_5ee3357c80ee7d52')} /></div><h1 id="protocol-catalog-title" style={{ margin:0 }}>{labels.title}</h1><p style={muted}>{labels.subtitle}</p></header>
