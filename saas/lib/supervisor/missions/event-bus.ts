...
export interface MissionEventEnvelope<T> { eventId:string; occurredAt:string; correlationId:string; causationId:string|null; idempotencyKey:string; revision:number; schemaVersion:string; payload:T }
...
export class InMemoryMissionEventBus implements MissionEventBus {private readonly handlers=new Map<MissionTopic,Set<MissionEventHandler<unknown>>>();async publish<T>(topic:MissionTopic,event:MissionEventEnvelope<T>){for(const handler of [...(this.handlers.get(topic)??[])])await handler(event)}async subscribe<T>(topic:MissionTopic,handler:MissionEventHandler<T>){const handlers=this.handlers.get(topic)??new Set();this.handlers.set(topic,handlers);handlers.add(handler as MissionEventHandler<unknown>);return{unsubscribe:()=>handlers.delete(handler as MissionEventHandler<unknown>)}}}
