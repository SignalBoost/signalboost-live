...
export default function Home() {
  ...
  async function copyText(value: string, target: string) {
    ...
    try {
      ...
      // Browser clipboard permission can be blocked. The transcript remains selectable text.
    }
  }
  ...
  const latestRequest = turns.length ? turns[turns.length - 1].request : ''
  ...
  >
    ...
    <section className="concierge-panel" aria-label={c('eyebrow')}>
      ...
      {(turns.length > 0 || pendingRequest || failed) ? <section className="transcript" aria-live="polite" aria-busy={loading}>
        <div className="transcript-header">
          <div>
            <div className="transcript-title">{transcriptCopy.title}</div>
            <p>{transcriptCopy.retained}</p>
          </div>
          {turns.length ? <button type="button" className="copy-button copy-full" onClick={() => void copyText(fullTranscript, 'full')}>
            <span aria-hidden="true">⧉</span> {copiedTarget === 'full' ? transcriptCopy.copied : transcriptCopy.copyFull}
          </button> : null}
        </div>

        <div className="transcript-turns">
          {turns.map((turn, index) => <article className="transcript-turn" key={`${index}-${turn.request.slice(0, 32)}`}>
            <div className="turn-heading">{transcriptCopy.turn} {index + 1}</div>
            <div className="turn-block turn-request">
              <div className="turn-block-head">
                <div className="answer-request-label">{c('yourRequest')}</div>
                <button type="button" className="copy-button" onClick={() => void copyText(turn.request, `q-${index}`)}>
                  <span aria-hidden="true">⧉</span> {copiedTarget === `q-${index}` ? transcriptCopy.copied : transcriptCopy.copyQuestion}
                </button>
              </div>
              <p>{turn.request}</p>
            </div>
            <div className="turn-block turn-response">
              <div className="turn-block-head">
                <div className="answer-label">{c('cosLabel')}</div>
                <button type="button" className="copy-button" onClick={() => void copyText(turn.response, `a-${index}`)}>
                  <span aria-hidden="true">⧉</span> {copiedTarget === `a-${index}` ? transcriptCopy.copied : transcriptCopy.copyResponse}
                </button>
              </div>
              <div className="answer-content"><AssistantMessage content={turn.response} /></div>
            </div>
          </article>)}

          {pendingRequest ? <article className={'transcript-turn pending-turn' + (failed ? ' is-failed' : '')}>
            <div className="turn-heading">{transcriptCopy.turn} {turns.length + 1}</div>
            <div className="turn-block turn-request">
              <div className="turn-block-head"><div className="answer-request-label">{c('yourRequest')}</div></div>
              <p>{pendingRequest}</p>
            </div>
            <div className="turn-block turn-response">
              <div className="turn-block-head"><div className="answer-label">{c('cosLabel')}</div></div>
              <div className="answer-content">
                {loading ? <div className="answer-loading"><span>{c('thinking')}</span><div className="answer-placeholder" aria-hidden="true"><i /><i /><i /></div></div> : failed ? <p className="answer-error" role="status">{c('error')}</p> : null}
              </div>
            </div>
          </article> : null}
        </div>

        {latestRequest ? <div className="transcript-footer"><Link href={'/dashboard/assistant?prompt=' + encodeURIComponent(latestRequest)}>{c('continue')} →</Link></div> : null}
      </section> : null}
    ...
    <style jsx>{`.concierge-home{position:relative;isolation:isolate;display:grid;place-items:center;min-height:calc(100svh - 150px);padding:42px 20px 56px;overflow:hidden;background:#030611;color:#f7f8ff;font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}.aurora{position:absolute;inset:-25%;z-index:-1;background:radial-gradient(circle at 18% 18%,rgba(43,211,255,.19),transparent 24%),radial-gradient(circle at 77% 28%,rgba(245,196,81,.18),transparent 28%),radial-gradient(circle at 50% 96%,rgba(91,76,255,.2),transparent 35%);filter:blur(18px);animation:breathe 12s ease-in-out infinite alternate}@keyframes breathe{to{transform:scale(1.08) translate3d(1%,-1%,0)}}.concierge-panel{position:relative;width:min(1180px,100%);padding:clamp(28px,5vw,68px);border:1px solid rgba(255,255,255,.14);border-radius:32px;background:linear-gradient(145deg,rgba(16,24,45,.9),rgba(4,8,21,.78));box-shadow:0 34px 100px rgba(0,0,0,.42);backdrop-filter:blur(22px)}.identity{display:flex;align-items:center;gap:10px;color:#f6c453;font-size:11px;font-weight:850;letter-spacing:.17em}.orb{display:grid;place-items:center;width:29px;height:29px;border-radius:50%;background:linear-gradient(135deg,#f6c453,#5ce1e6);color:#07111b;font-size:15px;box-shadow:0 0 24px rgba(92,225,230,.42)}h1{max-width:940px;margin:22px 0 14px;font-size:clamp(34px,6vw,66px);line-height:.99;letter-spacing:-.065em}h1 em{font-style:normal;background:linear-gradient(110deg,#fff,#f6c453 62%,#7ee8ef);-webkit-background-clip:text;background-clip:text;color:transparent}.lead{max-width:840px;margin:0;color:#b9c3d8;font-size:clamp(15px,2vw,18px);line-height:1.65}.platform-cta{display:inline-flex;align-items:center;gap:8px;margin-top:18px;color:#f6c453;font-size:13px;font-weight:850;text-decoration:none}.platform-cta:hover{color:#fff}.ask-box{position:relative;margin-top:30px;padding:8px;border:1px solid rgba(140,235,244,.34);border-radius:20px;background:rgba(2,8,23,.65);box-shadow:0 0 0 5px rgba(26,240,255,.035);display:flex;gap:8px;align-items:stretch}.file-input{display:none}.ask-box textarea{min-width:0;flex:1;resize:none;border:0;background:transparent;color:#fff;padding:12px 14px;font:inherit;line-height:1.45;outline:0}.ask-box textarea::placeholder{color:#74829c}.ask-box button{border:0;border-radius:14px;padding:0 20px;background:linear-gradient(135deg,#f7ca5c,#d9952c);color:#151008;font-weight:850;cursor:pointer;white-space:nowrap}.ask-box .attach-button{width:46px;padding:0;background:rgba(255,255,255,.08);color:#fff;font-size:17px}.ask-box button:disabled{opacity:.55;cursor:not-allowed}.attachments{display:flex;flex-wrap:wrap;gap:7px;margin-top:12px}.attachment{display:flex;max-width:220px;align-items:center;gap:7px;padding:5px 7px 5px 5px;border:1px solid rgba(255,255,255,.13);border-radius:10px;background:rgba(255,255,255,.06);font-size:11px;color:#dbe6f7}.attachment img,.attachment>span:first-child{width:26px;height:26px;border-radius:6px;object-fit:cover}.attachment>span:nth-child(2){overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.attachment button{border:0;background:transparent;color:#b9c3d8;cursor:pointer;font-size:16px}.drop-hint{margin:10px 2px 0;color:#8996ae;font-size:11px}.drop-overlay{position:absolute;inset:16px;z-index:2;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px;border:2px dashed #7ee8ef;border-radius:24px;background:rgba(4,11,29,.93);color:#fff;text-align:center}.drop-overlay strong{color:#7ee8ef}.drop-overlay span{color:#b9c3d8;font-size:12px}.chips{display:flex;gap:8px;flex-wrap:wrap;margin-top:14px}.chips button{border:1px solid rgba(255,255,255,.12);border-radius:999px;background:rgba(255,255,255,.045);color:#d5ddec;padding:8px 11px;font-size:12px;cursor:pointer}.chips button:hover{border-color:#7ee8ef;color:#fff}.transcript{margin-top:26px;padding:24px;border:1px solid rgba(126,232,239,.25);border-radius:22px;background:rgba(8,24,40,.62);box-shadow:0 18px 50px rgba(0,0,0,.2)}.transcript-header{display:flex;align-items:flex-start;justify-content:space-between;gap:24px;padding-bottom:18px;border-bottom:1px solid rgba(255,255,255,.1)}.transcript-title{color:#fff;font-size:18px;font-weight:850}.transcript-header p{max-width:700px;margin:6px 0 0;color:#8996ae;font-size:12px;line-height:1.55}.copy-button{display:inline-flex;align-items:center;gap:6px;border:1px solid rgba(126,232,239,.24);border-radius:999px;padding:7px 10px;background:rgba(255,255,255,.045);color:#cfe9ef;font:inherit;font-size:11px;font-weight:800;cursor:pointer;white-space:nowrap}.copy-button:hover{border-color:#7ee8ef;color:#fff}.copy-full{padding:9px 14px;color:#f6c453;border-color:rgba(246,196,83,.34)}.transcript-turns{display:grid;gap:24px;margin-top:22px}.transcript-turn{display:grid;gap:12px}.transcript-turn+.transcript-turn{padding-top:22px;border-top:1px solid rgba(255,255,255,.1)}.turn-heading{color:#8fa0b8;font-size:10px;font-weight:900;letter-spacing:.14em;text-transform:uppercase}.turn-block{padding:16px 18px;border-radius:14px}.turn-request{border-left:3px solid rgba(246,196,83,.72);background:rgba(246,196,83,.055)}.turn-response{border-left:3px solid rgba(126,232,239,.7);background:rgba(126,232,239,.045)}.turn-block-head{display:flex;align-items:center;justify-content:space-between;gap:16px}.answer-request-label{color:#f6c453;font-size:10px;font-weight:900;letter-spacing:.14em}.answer-label{color:#7ee8ef;font-size:11px;font-weight:900;letter-spacing:.14em}.turn-request p{margin:10px 0 0;color:#d8e0ef;font-size:14px;line-height:1.65;white-space:pre-wrap;overflow-wrap:anywhere}.answer-content{min-height:28px;margin:12px 0 0;line-height:1.65;color:#e8edf7;white-space:normal;overflow-wrap:anywhere}.answer-loading{display:grid;gap:12px;color:#9fb0c9;font-size:12px}.answer-placeholder{display:grid;gap:9px;max-width:94%;padding-top:5px}.answer-placeholder i{display:block;height:9px;border-radius:999px;background:linear-gradient(90deg,rgba(126,232,239,.08),rgba(255,255,255,.09),rgba(126,232,239,.08));animation:answerPulse 1.8s ease-in-out infinite}.answer-placeholder i:nth-child(2){width:82%}.answer-placeholder i:nth-child(3){width:61%}@keyframes answerPulse{50%{opacity:.45}}.pending-turn.is-failed .turn-response{border-left-color:rgba(252,165,165,.6)}.answer-error{margin:0;color:#fecaca}.transcript-footer{display:flex;justify-content:flex-end;margin-top:22px;padding-top:18px;border-top:1px solid rgba(255,255,255,.1)}.transcript-footer a,.bottom-row a{color:#f6c453;font-weight:800;text-decoration:none}.bottom-row{display:flex;align-items:flex-end;justify-content:space-between;gap:20px;margin-top:32px;padding-top:20px;border-top:1px solid rgba(255,255,255,.1)}.bottom-row p{max-width:720px;margin:0;color:#8996ae;font-size:11px;line-height:1.55}.bottom-row a{white-space:nowrap;font-size:13px}@media(max-width:720px){.concierge-home{padding:22px 12px 34px}.concierge-panel{padding:28px 20px;border-radius:24px}.ask-box{flex-wrap:wrap}.ask-box textarea{flex-basis:calc(100% - 54px)}.ask-box button[type="submit"]{height:46px;flex:1}.transcript{padding:18px}.transcript-header,.turn-block-head,.bottom-row{align-items:flex-start;flex-direction:column}.copy-full{width:100%;justify-content:center}.turn-block{padding:14px}.transcript-footer{justify-content:flex-start}}`}</style>
  </section>
}
