...
**Concierge artifacts:** implementation is active and not Production-accepted. An authenticated Concierge prompt that explicitly asks for a `.txt` or text-based `.pdf` routes to a backend artifact tool; it drafts only from user-supplied details, persists the file in the private workspace, and returns a same-chat download link. It never sends the document or treats a request for research as permission to invent factual content. A fresh authenticated PDF and TXT download observation is required before runtime acceptance.
