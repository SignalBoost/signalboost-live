...
**Concierge visual designer:** implemented and deployed to Production on 2026-08-30; the provider-boundary repair is `316d8e2ecd8dc7ff14ea22e6602c75812141e9a2`, deployment `dpl_G15A7tkVdc6DKG6VZSHxhqToZEwy` — READY. Explicit sketch, drawing, illustration, diagram, scene, and image-creation requests run through an authenticated backend visual tool, store the result only in the requesting user's private workspace, render it inline in the homepage Concierge, and provide a download. Visual generation is pinned to the approved DeepInfra managed COS runtime; OpenAI keys and all other provider fallbacks are prohibited. If that approved runtime is unavailable, the request fails closed. A fresh signed-in Production visual-generation observation remains required before runtime acceptance.
