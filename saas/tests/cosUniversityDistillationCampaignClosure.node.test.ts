...
test('closure runs in the workflow, records why, and authorizes no retry or traffic', () => {
  ...
  assert.ok(workflow.indexOf('authorizeAvailableUniversityMassDistillationCampaigns') < workflow.indexOf("isolatedStep('semantic_reconciliation'"), 'authorization must precede semantic maintenance')
  assert.ok(workflow.indexOf('runMassDistillationCampaignConsumer({ now, maxDispatches: 5 })') < workflow.indexOf("isolatedStep('semantic_reconciliation'"), 'provider dispatch must precede semantic maintenance')
  assert.ok(workflow.indexOf('runMassDistillationCampaignConsumer({ now, maxDispatches: 5 })') < workflow.indexOf('prepareUniversityMassDistillationCurriculum(now'), 'provider dispatch must precede curriculum maintenance')
