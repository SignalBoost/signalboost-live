...
export default function SearchableDropdown({ label, options, value, onChange, placeholder = 'Select an option', multiple = false }: { label: string; options: Option[]; value: string | string[]; onChange: (value: string | string[]) => void; placeholder?: string; multiple?: boolean }) {
  ...
  return <div style={{ display: 'grid', gap: 8, position: 'relative' }}>
    ...
    <button id={id} type="button" onClick={() => setOpen(!open)} aria-expanded={open} aria-haspopup="listbox" style={{ minHeight: 46, width: '100%', textAlign: 'left', border: '1px solid rgba(255,255,255,.14)', background: 'rgba(2,6,23,.78)', color: selected.length ? '#fff' : 'rgba(255,255,255,.55)', borderRadius: 14, padding: '10px 12px', cursor: 'pointer' }}>
      {selected.length ? <span style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>{selected.map((item) => <span key={item.id} style={{ border: '1px solid rgba(26,240,255,.24)', background: 'rgba(26,240,255,.1)', borderRadius: 999, padding: '4px 8px', fontSize: 12 }}>{item.icon} {item.label}</span>)}</span> : placeholder}
    </button>
    {open && <div style={{ position: 'absolute', zIndex: 40, top: '100%', left: 0, right: 0, marginTop: 6, padding: 8, border: '1px solid rgba(255,255,255,.16)', borderRadius: 16, background: '#07111f', boxShadow: '0 24px 70px rgba(0,0,0,.5)' }}>
      <input type="search" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search metadata" autoFocus style={{ width: '100%', boxSizing: 'border-box', border: '1px solid rgba(255,255,255,.13)', borderRadius: 12, background: '#020617', color: '#fff', padding: '10px 11px', marginBottom: 8 }} />
      ...
      <button type="button" onClick={() => setOpen(false)} style={{ width: '100%', marginTop: 8, border: '1px solid rgba(255,255,255,.14)', borderRadius: 10, background: 'rgba(255,255,255,.06)', color: '#fff', padding: 8 }}>Done</button>
    </div>}
  </div>
}
