...
export default async function SupervisorOperationsCenter({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const execution = {
    ...
    nextObservation: due ? (due.due ? (t.observationDue) : due.reason) : (t.noPolicy),
  ...
  return <main style={page}>
    ...
    <details style={panel}>
      ...
      <div style={grid2}>
        <Card title={t.subsystemHealth}><div style={cards}>{health.subsystems.map(s => { const d = diagnostics.find(x => x.subsystemId === s.id); return <article key={s.id} style={card}><h3>{(t as any)[s.id] || s.id}</h3><dl style={fields}><Field k={t.status} v={d ? d.label : s.status}/><Field k={t.metric} v={s.metric ?? '—'}/><Field k={t.operationalImpactLabel} v={d ? d.impactStatement : '—'}/><Field k={t.evidence} v={s.evidence.join(' · ') || s.summary}/></dl></article> })}</div></Card>
    ...
    <details style={panel}>
      ...
      <Card title={t.healthAssessment}>
        <dl style={fields}><Field k={t.healthScore} v={ledger.score === null ? (t.notMeasured) : `${ledger.score}%`}/><Field k={t.coverage} v={ledger.coverage}/><Field k={t.reconciles} v={ledger.reconciles ? (t.yes) : (t.no)}/></dl>
