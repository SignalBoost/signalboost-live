...
import { compareBackupCosDecision } from '@/lib/cos/backupCos'
...
export async function POST(req: Request) {
  ...
  const decision = answerSignalBoostConcierge(query, locale, options)
  const backupComparison = compareBackupCosDecision({ query, locale, options }, decision)
  if (backupComparison.diverged) {
    // Sanitized fingerprints only: never log request content, account data, or secrets.
    console.error('Supervisor: Backup COS divergence detected', backupComparison)
  }
