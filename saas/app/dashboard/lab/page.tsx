    ...
    >
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 16 }}>
          <div style={{ fontSize: 11, fontWeight: 700, padding: '5px 14px', borderRadius: 6, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)', color: licenseColor, fontFamily: 'monospace' }}>{licenseLabel}</div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <button onClick={() => { onCaption(asset); handleClose() }} style={{ width: '100%', padding: '13px', borderRadius: 10, background: GOLD, color: '#000', fontWeight: 800, fontSize: 14, border: 'none', cursor: 'pointer' }}>
            💬 Generate subtitles in 5 languages
          </button>
          {asset.watchUrl && (
            <a href={asset.watchUrl} target="_blank" rel="noopener noreferrer" style={{ display: 'block', width: '100%', padding: '13px', borderRadius: 10, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: '#fff', fontWeight: 700, fontSize: 14, textAlign: 'center', textDecoration: 'none', boxSizing: 'border-box' }}>
  ...
  return (
    <div style={{ background: 'rgba(16, 22, 42, 0.65)', border: '1px solid rgba(59, 130, 246, 0.25)', borderRadius: 12, padding: 20, marginBottom: 20, backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)', boxShadow: '0 20px 40px rgba(0,0,0,0.5)' }}>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <textarea value={script} onChange={e => setScript(e.target.value)} rows={4} style={{ width: '100%', background: 'rgba(4, 5, 11, 0.85)', border: '1px solid rgba(255, 255, 255, 0.15)', borderRadius: 8, padding: '10px 14px', color: '#fff', fontSize: 13, fontFamily: 'monospace', outline: 'none', resize: 'vertical', boxSizing: 'border-box' }} />
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
        <div>
          ...
          <select value={selectedAvatar} onChange={e => setSelectedAvatar(e.target.value)} style={{ width: '100%', background: 'rgba(4, 5, 11, 0.85)', border: '1px solid rgba(255, 255, 255, 0.15)', borderRadius: 6, padding: '9px 12px', color: '#fff', fontSize: 12, fontFamily: 'monospace' }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {(['9:16', '16:9', '1:1'] as const).map(f => (
              <button key={f} onClick={() => setSelectedFormat(f)} style={{ flex: 1, padding: '9px 0', borderRadius: 6, border: '1px solid rgba(255,255,255,0.15)', background: selectedFormat === f ? 'rgba(59,130,246,0.25)' : 'rgba(4, 5, 11, 0.5)', color: selectedFormat === f ? '#fff' : 'rgba(255,255,255,0.4)', fontSize: 11, fontFamily: 'monospace', fontWeight: 700, cursor: 'pointer' }}>{f}</button>
            ))}
          </div>
        </div>
      </div>
...
export default function LabPage() {
  ...
  const licenseBg = (l: string) => l === 'public' ? 'rgba(74,222,128,0.06)' : l === 'embeddable' ? 'rgba(255,195,0,0.06)' : 'rgba(239,68,68,0.04)'
  ...
  return (
    <div style={{ color: '#fff', fontFamily: 'system-ui, -apple-system, sans-serif', maxWidth: 1300, margin: '0 auto', padding: '24px 20px' }}>
      ...
      {/* ── ONLY CHANGE: body background updated to match dark room + orb atmosphere ── */}
      <style>{`
        body {
          background-color: #02030a !important;
          background-image:
            radial-gradient(ellipse 60% 50% at 50% 42%, rgba(0, 220, 255, 0.18) 0%, transparent 60%),
            radial-gradient(ellipse 80% 40% at 50% 100%, rgba(20, 10, 60, 0.9) 0%, transparent 70%),
            radial-gradient(ellipse 100% 60% at 50% 0%, rgba(5, 5, 30, 0.95) 0%, transparent 80%),
            radial-gradient(ellipse 40% 30% at 15% 60%, rgba(0, 100, 200, 0.12) 0%, transparent 60%),
            radial-gradient(ellipse 40% 30% at 85% 60%, rgba(100, 0, 200, 0.10) 0%, transparent 60%),
            linear-gradient(180deg, #01020a 0%, #03060f 40%, #020408 100%) !important;
        ...
        <div>
          <div className="fathom-glass" style={{ borderRadius: 16, padding: '28px', marginBottom: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
              <div>
                <h1 className="terminal-text" style={{ fontSize: 20, fontWeight: 800, margin: 0, letterSpacing: '-0.02em', color: '#fff' }}>CORE_ENGINE // The Lab</h1>
                <p style={{ fontSize: 12, color: '#6178a3', marginTop: 4 }}>Synthesize parameters: footage parsing, generative avatar streaming, audio dub orchestration.</p>
              </div>
            </div>
            <div style={{ background: 'rgba(4, 5, 11, 0.85)', border: '1px solid rgba(59, 130, 246, 0.35)', borderRadius: 10, padding: '6px', display: 'flex', gap: 4, alignItems: 'center', marginBottom: 20, boxShadow: 'inset 0 4px 12px rgba(0,0,0,0.5)' }}>
              ...
              <input ref={inputRef} value={prompt} onChange={e => setPrompt(e.target.value)} onKeyDown={e => e.key === 'Enter' && runSearch()} placeholder='Injection query... e.g., "create a 30s ad in Spanish" or "show me last goal of Pelé"' className="terminal-text" style={{ flex: 1, background: 'transparent', border: 'none', padding: '10px 8px', color: '#fff', fontSize: 13, outline: 'none' }} />
              {hasSearched && (
                <button onClick={reset} className="terminal-text" style={{ padding: '8px 14px', borderRadius: 6, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.5)', fontSize: 11, cursor: 'pointer' }}>ABORT</button>
              )}
            ...
            <div style={{ display: 'flex', gap: 6, background: 'rgba(4, 5, 11, 0.6)', padding: 6, borderRadius: 8, border: '1px solid rgba(255,255,255,0.08)' }}>
              {MODES.map(m => (
                <button key={m.id} onClick={() => setMode(m.id)} className="terminal-text" style={{ flex: 1, padding: '10px 8px', borderRadius: 6, fontSize: 11, border: 'none', cursor: 'pointer', background: mode === m.id ? 'rgba(59,130,246,0.2)' : 'transparent', color: mode === m.id ? '#fff' : '#6178a3', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, transition: 'all 0.15s', fontWeight: mode === m.id ? 700 : 500 }}>
