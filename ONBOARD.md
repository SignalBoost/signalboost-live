...
**Builder request intent — 2026-09-06:** The live README extension was marked failed despite fifteen passing tests because descriptive error vocabulary selected the repair gate. A bounded check by the configured Builder model now identifies documentation-only work among ambiguous repair-like requests that explicitly name plain document paths. The host independently restricts mutations to those paths, retains the restriction through checkpoints, preserves genuine failed-check handling, and keeps the original repair path on unavailable/negative classification. No implementation/test/configuration mutation is authorized by documentation mode. Explicit Builder job evidence questions, including Does/Is/Are variants, bypass prior-answer provenance and retain same-user/conversation/repository scope checks. Incomplete-task fallback reports recorded passing commands separately from an unsatisfied task gate. No schema or model change. All 1016 mandatory regressions and TypeScript pass after integrating #1908. Live acceptance pending; see `docs/BUILDER-REQUEST-INTENT-2026-09-06.md`.
