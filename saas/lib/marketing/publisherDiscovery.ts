...
function wantsPaidPlacement(brief: string) {
  ...
  return /\b(paid|pay|paid\s+ad|paid\s+ads|paid\s+advertising|paid\s+placement|sponsored|sponsorship|media\s*kit|rate\s*card|buy\s+ad|purchase\s+ad)\b/i.test(t)
}

function extractEmails(html: string) {
  const found = new Set<string>()
  const decoded = html.replace(/\s*\[at\]\s*/gi, '@').replace(/\s*\(at\)\s*/gi, '@').replace(/\s+at\s+/gi, '@').replace(/\s*\[dot\]\s*/gi, '.').replace(/\s*\(dot\)\s*/gi, '.').replace(/\s+dot\s+/gi, '.')
...
function hasPublisherSignals(html: string, url: string, result: SearchResult) { const host = domainOf(url); const text = `${result.title} ${result.description || ''} ${html.slice(0, 8000)}`.toLowerCase(); const publicationSignals = /(newspaper|magazine|journal|newsroom|editorial|local news|community news|daily|weekly|gazette|times|tribune|herald|observer|post|press|independent|courier|ledger|record|bulletin|news tip|letters to the editor|submit news|submit a story)/i.test(text); const publisherDomainSignals = /\b(news|times|tribune|herald|gazette|observer|post|press|journal|magazine|weekly|daily|courier|ledger|record|bulletin)\b/i.test(host.replace(/[-.]/g, ' ')); return publicationSignals || publisherDomainSignals }
function isFreeEditorialContact(email: string, allowPaid: boolean) { if (/^(advertising|ads|sponsor|sales|partnerships|partners)@/i.test(email)) return allowPaid; return /^(editor|editors|newsroom|tips|news|press|media|submit|submissions|letters|opinion|events|info|hello|contact)@/i.test(email) }
...
async function fetchWithTimeout(url: string) {
  ...
  try { const res = await fetch(url, { headers: { 'user-agent': 'SignalBoostBot/1.0 publisher-contact-discovery' }, signal: controller.signal }); if (!res.ok) return ''; const type = res.headers.get('content-type') || ''; if (!type.includes('text/html') && !type.includes('text/plain')) return ''; return (await res.text()).slice(0, 250000) } catch { return '' } finally { clearTimeout(timer) }
}
...
async function serperSearch(query: string): Promise<SearchResult[]> {
  ...
  if (!key) return []
  const res = await fetch('https://google.serper.dev/search', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-API-KEY': key }, body: JSON.stringify({ q: query, num: SEARCH_LIMIT }) })
  if (!res.ok) return []
  const json: any = await res.json().catch(() => ({}))
...
function queriesFor(args: PublisherDiscoveryArgs) { const channel = String(args.channel || '').toLowerCase(); const paid = wantsPaidPlacement(args.brief); const base = channel.includes('trade') ? 'technology business magazine publication' : channel.includes('print') ? 'local print newspaper' : 'local newspaper publication'; const freeQueries = [`${base} submit news editor email`, `${base} news tips editor email`, `${base} submit story contact editor`, `${base} letters to the editor email`]; const paidQueries = [`${base} advertise contact email`, `${base} media kit advertising contact`]; return paid ? [...freeQueries, ...paidQueries] : freeQueries }
...
async function inspectCandidate(result: SearchResult, allowPaid: boolean): Promise<PublisherDiscoveryResult> {
  ...
  const root = (() => { try { const u = new URL(result.url); return `${u.protocol}//${u.host}` } catch { return '' } })()
  if (root) { urls.push(`${root}/contact`, `${root}/news-tips`, `${root}/submit-news`, `${root}/submit`, `${root}/letters`, `${root}/opinion`, `${root}/events`, `${root}/write-for-us`); if (allowPaid) urls.push(`${root}/advertise`, `${root}/media-kit`) }
  for (const url of [...new Set(urls)].slice(0, FETCH_LIMIT)) {
    ...
    if (emails[0]) return { ok: true, publicationName: titleToPublication(result.title, result.url), editorContact: emails[0], method: 'email', sourceUrl: url }
    const form = extractSubmissionFormUrl(html, url, allowPaid)
    if (form) return { ok: true, publicationName: titleToPublication(result.title, result.url), editorContact: form, method: 'online_form', sourceUrl: url }
  }
...
export async function discoverPublisherTarget(args: PublisherDiscoveryArgs): Promise<PublisherDiscoveryResult> {
  if (!process.env.BRAVE_SEARCH_API_KEY && !process.env.SERPER_API_KEY) return { ok: false, skipped: true, error: 'publisher_search_api_not_configured' }
  const allowPaid = wantsPaidPlacement(args.brief)
  const seen = new Set<string>()
  for (const query of queriesFor(args)) {
    const results = await searchWeb(query)
    for (const result of results) { if (!result.url || seen.has(result.url)) continue; seen.add(result.url); const inspected = await inspectCandidate(result, allowPaid); if (inspected.ok) return inspected }
  }
