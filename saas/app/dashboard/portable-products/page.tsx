...
export default async function PortableProductsPage({ searchParams }: { searchParams?: Promise<SearchParams> }) {
  ...
  return <main style={page}>
    ...
    {items.map(item => <article key={item.productId} style={panel}><h2>{item.displayName}</h2><p style={muted}><code>{item.productId}</code> · {item.status} · {item.maturity} · {item.category} · public: {String(item.publicVisible)} · licensing: {String(item.licensingAvailable)}</p><p>{item.shortDescription}</p><dl style={{ display: 'grid', gap: 10 }}><div><dt><LocalizedText fallback={"Supported languages"} /></dt><dd>{values(item.supportedLanguages)}</dd></div><div><dt><LocalizedText fallback={"Architecture references"} /></dt><dd>{values(item.architectureReferences)}</dd></div><div><dt>Documentation references</dt><dd>{values(item.documentationReferences)}</dd></div><div><dt>Dependencies</dt><dd>{values(item.dependencies)}</dd></div><div><dt>Exclusions</dt><dd>{values(item.exclusions)}</dd></div></dl></article>)}
