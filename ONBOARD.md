...
**Concierge public prompt-exfiltration boundary:** deployed to Production on 2026-08-30 as `ea4ee219e663a5fb9a6adf3658ecc26557d8aa7c`, deployment `dpl_Gc2pVNDs1BQpFsgabMANuWWKPyje` — READY with `saas.signalboostapp.com` attached. A deterministic pre-inference guard blocks system/developer/private-context extraction and encoding requests, including fictional and completion framing, with one opaque response. A bounded output guard blocks raw or Base64-wrapped internal reference-context markers. Original attacks must still be re-observed safely in Production before acceptance.    
