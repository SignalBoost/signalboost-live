-- Keep mass distillation progressing after transient or terminal stage failures without
-- widening the owner-authorized campaign expiration, total cost, promotion, traffic, or RunPod
-- authority. Ambiguous provider submissions cool down before a named-job discovery retry.
...
begin
  ...
  for v_run in
    ...
    for update
  loop
    -- The active reservation identifies the failed stage even when provider submission failed
    -- before its Job id could be persisted. Settled runs fall back to their furthest durable Job.
    ...
    -- A transport failure after POST may hide an accepted paid Job. Wait for provider list
    -- consistency, then let the consumer discover the deterministic name before any new POST.
    ...
    -- Release only the current claim's reserve when no provider-ledger row was accepted during
    -- that claim. Older settled attempts do not pin a later rejected/pre-provider reservation.
  ...
  insert into public.cos_university_learning_assurance_events (
    ...
    encode(extensions.digest(convert_to(p_campaign_id::text||':'||v_rearmed::text||':'||v_released::text||':'||v_budget_blocked::text||':'||btrim(p_source),'UTF8'),'sha256'),'hex'),
    jsonb_build_object(
