  ...
  return (
    <div className="sb-reviews-page" style={{ color: 'var(--text-primary)' }}>
      ...
      <section className="sb-reviews-hero" aria-label="Reviews module">
        ...
        {/* Wireflow — compact vertical */}
        <div aria-hidden="true" style={{ background: 'rgba(2,6,23,.52)', border: '1px solid rgba(255,255,255,.14)', borderRadius: 20, padding: 14, display: 'flex', flexDirection: 'column', gap: 4, alignSelf: 'flex-start', minWidth: 140 }}>
          {['Submission', 'AI Sentiment', 'Moderation', 'Outreach'].map((label, i, arr) => (
            <div key={label} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
              <div style={{ border: '1px solid rgba(26,240,255,.22)', borderRadius: 10, color: '#dffcff', fontSize: 11, fontWeight: 900, padding: '7px 10px', textAlign: 'center', textTransform: 'uppercase', width: '100%', boxSizing: 'border-box', background: 'rgba(26,240,255,.06)' }}>
                {label}
              </div>
              {i < arr.length - 1 && (
                <div style={{ color: '#ffc300', fontSize: 12, fontWeight: 900, lineHeight: 1 }}>↓</div>
              )}
            </div>
          ))}
        </div>
