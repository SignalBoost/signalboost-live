...
const PROJECTS: readonly PreviewProject[] = Object.freeze([
  ...
  },
  {
    ...
    description: { en: 'Three setup paths for every provider: governed AI infrastructure PR, Browser Agent assistance, and direct configuration when appropriate.', es: 'Tres rutas de configuración para cada proveedor: PR de infraestructura gobernada por IA, asistencia del Browser Agent y configuración directa cuando corresponda.', pt: 'Três caminhos de configuração para cada provedor: PR de infraestrutura governada por IA, assistência do Browser Agent e configuração direta quando apropriado.', pl: 'Trzy ścieżki konfiguracji dla każdego dostawcy: zarządzany przez AI PR infrastruktury, pomoc Browser Agenta i konfiguracja bezpośrednia, gdy jest odpowiednia.', ru: 'Три пути настройки для каждого провайдера: управляемый ИИ PR инфраструктуры, помощь Browser Agent и прямая настройка, когда это уместно.' },
    foundation: { en: 'AI-first onboarding with Browser Agent assistance and direct configuration when appropriate.', es: 'Incorporación centrada en IA con asistencia del Browser Agent y configuración directa cuando corresponda.', pt: 'Integração orientada por IA com assistência do Browser Agent e configuração direta quando apropriado.', pl: 'Wdrażanie oparte przede wszystkim na AI z pomocą Browser Agenta i konfiguracją bezpośrednią, gdy jest odpowiednia.', ru: 'Подключение в первую очередь с помощью ИИ, с поддержкой Browser Agent и прямой настройкой, когда это уместно.' },
  },
  {
