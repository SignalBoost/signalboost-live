...
export default function PressPrintMediaPage() {
  const [campaigns, setCampaigns] = useState<Campaign[]>([]), [loading, setLoading] = useState(true)
  ...
  return <main style={shell}>
    ...
    {campaigns.map((campaign) => {
      const currentReview = review(campaign), currentStage = stage(campaign), approved = currentReview === 'APPROVED', submitted = currentStage === 'submitted_to_publisher' || campaign.metadata?.publisher_submission_status === 'submitted', completed = currentStage === 'published', exec = campaign.metadata?.press_print_execution || {}
      const publisher = String(campaign.metadata?.publisher_name || campaign.metadata?.publication_name || ''), publisherEmail = String(campaign.metadata?.publisher_email || ''), publisherForm = String(campaign.metadata?.publisher_submission_form_url || ''), sourceUrl = String(campaign.metadata?.publisher_discovery_source_url || '')
      const hasPublisherTarget = Boolean(publisher && (publisherEmail || publisherForm)), paidRequested = campaign.metadata?.automation_mode === 'automated_paid_user_requested'
      return <section key={campaign.id} style={card}>
        ...
        <div style={targetBox}><h3 style={h3}>{paidRequested ? 'Paid publisher/advertising target' : 'Free publisher target'}</h3>{hasPublisherTarget ? <p style={body}>Publisher: <strong>{publisher}</strong><br />Submission method: <strong>{publisherEmail ? 'Publisher/editor email' : paidRequested ? 'Online advertising/submission form' : 'Free online submission/contact form'}</strong><br />Target: {publisherEmail || publisherForm}{sourceUrl ? <><br />Discovery source: {sourceUrl}</> : null}{paidRequested ? <><br /><strong>Exception:</strong> user explicitly requested paid advertising/placement.</> : null}</p> : <p style={body}>No verified free newspaper, magazine, or publication contact is recorded. Automated campaigns should stop here unless the user explicitly asked for paid advertising.</p>}</div>
