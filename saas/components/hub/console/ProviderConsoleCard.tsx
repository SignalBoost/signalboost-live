...
// Hub Command Console — provider card + workspace.
// Both views render buttons purely from section.templateIds (catalog-driven).
// No hardcoded per-provider buttons — archive/edit/etc. come from the catalog.
          ...
          <div key={idx} style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              {section.templateIds.map(id => {
                ...
                return (
                  <button key={id} onClick={() => onRun(id)} style={buttonStyle(id)}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
        <button onClick={onBack} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.4)', fontSize: 13, fontWeight: 700, cursor: 'pointer', padding: 0 }}>&larr; {tierLabel}</button>
          ...
          <div key={idx} style={{ background: 'rgba(13, 18, 32, 0.3)', border: '1px solid rgba(255, 255, 255, 0.05)', borderRadius: 12, padding: 18 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {section.templateIds.map(id => {
                ...
                if (!template) return null
                return (
                  <div key={id} onClick={() => onRun(id)} style={{ ...buttonStyle(id), justifyContent: 'flex-start' }}>
                    <span style={{ fontSize: 16 }}>{template.icon}</span>
                    <div style={{ minWidth: 0 }}>
                      <div style={{ fontSize: 13, fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{template.label}</div>
                      <div style={{ fontSize: 10.5, color: 'rgba(255,255,255,0.4)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{template.description}</div>
                    </div>
                  </div>
                )
              })}
