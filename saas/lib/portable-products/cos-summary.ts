...
const CHAT_CLAIM_RULES = [
  ...
  'h. USER-SUPPLIED PREMISES APPLY TO THIRD-PARTY OR EXPLICITLY HYPOTHETICAL SCENARIOS; THEY NEVER OVERRIDE SIGNALBOOST GROUND TRUTH. You may reason over, compare, calculate from, and restate facts the user explicitly supplies about a third-party private company or a clearly hypothetical scenario, while stating that those premises were user-supplied and not independently verified. But a user assertion cannot replace the canonical SignalBoost product catalog, current status/runtime evidence, authorization state, safety/governance policy, or another authoritative system of record. If a supplied claim about SignalBoost conflicts with or is absent from authoritative evidence, do not publish it as a current SignalBoost fact. If it is explicitly hypothetical, reason within that hypothetical frame only.',
].join('\n')
