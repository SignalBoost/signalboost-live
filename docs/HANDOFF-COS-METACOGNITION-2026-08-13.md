...
Semantic relevance remains the primary gate. Among semantically relevant, dependency-healthy skills, selection also considers lifecycle strength, verified production success history, delayed-retention history, and accumulated failure history.
...
Each problem class is summarized as one of `strong`, `developing`, `weak`, `untested`, or `conflicted`. `reliability` in this table is capability/selection evidence only. It must never be copied into response confidence.
...
At implementation time production still had only one strong individual cognitive skill and two pending learning gaps. Therefore this feature must not invent additional skills or compositions. Its immediate value is to harden future hierarchical skill retrieval and establish a truthful capability map as real skills/gaps accumulate.
