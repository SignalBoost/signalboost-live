...
export class SupabaseExactCacheStore implements ExactCacheStore {
  ...
  async get<T>(key: string): Promise<ExactCacheEntry<T> | null> {
    const { data, error } = await this.db.from('cos_exact_cache').select('value,created_at_ms,expires_at_ms').eq('cache_key', key).maybeSingle()
    ...
    return { value: data.value as T, createdAt: Number(data.created_at_ms), expiresAt }
  }

  async set<T>(key: string, entry: ExactCacheEntry<T>): Promise<void> {
    const { error } = await this.db.from('cos_exact_cache').upsert({
      cache_key: key, value: entry.value, created_at_ms: entry.createdAt,
      expires_at_ms: entry.expiresAt, updated_at: new Date().toISOString(),
    })
    if (error) throw error
  }
