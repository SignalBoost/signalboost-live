...
**Builder storage contract:** corrective release in progress. The first merge imported `storage-contract.ts` without including its source file, so Vercel correctly blocked the build at commit `b633b84`. This release adds the missing file and removes the remaining path sanitizer that could alter literal escaped source text. Direct U+0000 content is rejected before any Supabase call with a distinct `builder_file_contains_null_byte` error; literal `\\0` and `\\u0000` code text is preserved. Do not call it Production-accepted until this corrected deployment is READY and a fresh Production rejection observation is recorded.
