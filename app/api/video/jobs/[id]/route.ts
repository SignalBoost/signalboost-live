...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!SAFE_JOB_ID_PATTERN.test(id)) {
    return NextResponse.json({ ok: false, data: null, error: 'Invalid video job id', meta: responseMeta() }, { status: 400 })
  }
  ...
  if (!data) {
    ...
    if (existsSync(resultPath)) {
      try {
        const resultStat = statSync(resultPath)
        if (!resultStat.isFile() || resultStat.size > MAX_RESULT_FILE_BYTES) {
