...
import { configuredBuilderCodingModel } from '@/lib/builder/model-routing'
...
/**
 * Coding-specialist compute for Builder and COS Platform Engineer.
 *
 * This deliberately bypasses the general COS text cache because Builder prompts are control-plane
 * turns over mutable workspace/repository state. Reusing an old control answer across jobs is less
 * important than guaranteeing that every Builder round is generated by the configured coding model.
 * The existing local-inference transport still owns DeepInfra host allowlisting, credentials,
 * timeout handling and truthful model telemetry. Model specialization grants no additional tools or
 * execution authority.
 */
export function createBuilderAiPort(): CosAiPort {
  return {
    async generate(input) {
      const baseConfig = localInferenceConfigFromEnv()
      const model = configuredBuilderCodingModel()
      return requireText(
        await callLocalModel(input, { ...baseConfig, model }),
        `builder coding specialist ${model}`,
      )
    },
  }
}
