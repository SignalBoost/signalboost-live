// saas/lib/portable-browser/browser-profile-portability.ts
//
// Moved in from lib/browser-runtime/ alongside browser-task-contracts.ts, which imports it.
// Same reasoning: the contracts file is on the buyer surface, so everything it depends on has
// to be inside the folder the packager copies. The old path re-exports this one.
