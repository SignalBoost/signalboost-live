...
export default function PressPrintMediaPage() {
  ...
  return <main style={shell}>
    ...
    {campaigns.map((campaign) => { const currentReview = review(campaign), currentStage = stage(campaign), approved = currentReview === 'APPROVED', submitted = currentStage === 'submitted_to_publisher' || campaign.metadata?.publisher_submission_status === 'submitted', completed = currentStage === 'published', exec = campaign.metadata?.press_print_execution || {}; const publisher = String(campaign.metadata?.publisher_name || campaign.metadata?.publication_name || ''), publisherEmail = String(campaign.metadata?.publisher_email || ''), publisherForm = String(campaign.metadata?.publisher_submission_form_url || ''), sourceUrl = String(campaign.metadata?.publisher_discovery_source_url || ''); const hasPublisherTarget = Boolean(publisher && (publisherEmail || publisherForm));
      ...
      const draftText = String(campaign.objective || '');
      ...
      {draftText ? <details open={!approved && !completed} style={draftBox}><summary style={draftSummary}>{t('marketingSales.pressPrint.fullDraft', "Full draft — read before approving")}</summary><p style={draftBody}>{draftText}</p></details> : <p style={body}>{t('marketingSales.pressPrint.noDraft', "No draft text is stored for this campaign. Nothing should be approved until there is copy to read.")}</p>}
      ...
      {!approved && !completed ? <div style={actions}>{paidUnverified ? null : <Action id={campaign.id} decision="ok" primary>{uiText('generatedUi.u_6007acbe30b2cd98')}</Action>}<Action id={campaign.id} decision="no">{uiText('generatedUi.u_ab604a360777735f')}</Action><Action id={campaign.id} decision="staff"><LocalizedText fallback={uiText('generatedUi.u_9939e76315d42e7a')} /></Action></div> : null}
