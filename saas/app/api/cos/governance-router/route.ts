...
export async function GET() {
  ...
  return NextResponse.json({ ok: true, generatedAt: NOW(), mode: 'live-telemetry-with-demo-derived-fallbacks', pipelines, alerts, fixes, escalations, timeline, graph, automation: { autoApply: 'available_for_non_lifecritical_remediations_when_live_telemetry_identifies_a_target', escalation: 'life_critical_only', fallbackRouting: 'derived_from_live_queue_when_available', watchdog: 'scheduled_when_deployed_and_configured' }, sourceErrors: { campaigns: campaignRes.error?.message || null, decisions: decisionRes.error?.message || null } })
}
