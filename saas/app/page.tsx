        ...
        <section className="zone">
          ...
          <div className="grid grid-portables">
            {listPublicPortableProducts().map((p) => {
              ...
              const inner = <><div className="qcard-top"><span className="qcard-icon">{p.glyph}</span><span className={`runtime-tag runtime-${status}`}><i />{runtimeLabel(status)}</span></div><h3>{name}</h3><p>{desc}</p><div className="runtime-metrics"><strong>{runtime ? runtime.totalRows.toLocaleString() : '—'}</strong><span>verified rows</span><small>{runtime ? relativeTime(runtime.lastActivityAt) : 'Connecting to live data'}</small></div></>
