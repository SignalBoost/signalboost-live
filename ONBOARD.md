...
**Builder certification visibility:** implemented locally and not yet Production-accepted. The authenticated Developer workspace shows the owner’s current earned certification level (0–3) and number of verified attempts. The API returns only those aggregate values, scoped to the authenticated user; it does not expose attempt artifacts.
