...
**Normative/public-policy answer consistency:** Production behavior was confirmed across owner and unauthenticated Public Concierge sessions after `066811cc`: answers no longer lead with yes/no and preserve the same neutral meaning across wording variants. Every normative answer must bypass semantic/exact answer cache, retrieve live evidence for the strongest supporting and opposing arguments, and render those source groups separately. PR #1764 deployed that balanced-provenance contract. PR #1769 then aligned scope-plan acceptance/diagnosis after Production rejected valid long internal findings. A fresh public Production observation on PR #1769 progressed through the scope plan but still failed at answer acceptance: `unknown_contract_rejection`, followed by a truncated 420-token repair and `invalid_json`. `fix/normative-answer-contract-recovery-20260901` now diagnoses the missing distinct supporting/opposing source groups explicitly, gives repair an exact corrective instruction, and uses the same 700-token output budget as initial answer synthesis. The exact regression, TypeScript, and the complete 692-test COS gate pass locally. Preview, merge, Production READY, and a fresh successful public observation remain required.
...
**Concierge internal Builder:** implementation is active and not Production-accepted. Concierge is the only user-facing surface: explicit coding, repair, and design objectives invoke the authenticated Builder tool loop in the backend and return its answer, evidence, and file-download links in the same conversation. Ordinary questions remain on normal COS. The legacy redirect to `/dashboard/developer` is removed from the authenticated Assistant; supported text/code attachments are staged into the isolated network-denied workspace. The Developer workspace may remain an internal diagnostics surface, not a required customer journey. A fresh authenticated Concierge code-repair observation is required before runtime acceptance.  
**Concierge artifacts:** implementation is active and not Production-accepted. Every authenticated public Concierge request that explicitly asks for a `.txt` or text-based `.pdf` is routed at `/api/cos-browser`, before normal COS/public-delivery isolation, to a backend artifact tool; it drafts only from user-supplied details, persists the file in the private workspace, and returns a same-chat download link. It never sends the document or treats a request for research as permission to invent factual content. A fresh authenticated PDF and TXT download observation is required before runtime acceptance.
