...
import { SUPERVISOR_BROWSER_RUNTIME_DRY_RUN_ADAPTER_ID } from '../../../browser-runtime/adapter-identifiers.ts'
...
export function mapSupervisorStepsToBrowserRuntimeTask(input: { incident: SupervisorIncident; plan: RepairPlan; approvedSteps: RepairStep[]; targetOrigin: string; dispatchId: string; createdAt: string }): { browserTask: BrowserTask; stepMapping: BrowserRuntimeStepMapping[]; approvalRequirements: BrowserRuntimeApprovalRequirements; verificationRequirements: BrowserRuntimeVerificationRequirements } {
  ...
  const browserTask: BrowserTask = { taskId: `${input.dispatchId}:${input.plan.planId}:dry-run`, incidentId: input.incident.incidentId, provider: input.plan.targetProvider, adapterId: SUPERVISOR_BROWSER_RUNTIME_DRY_RUN_ADAPTER_ID, mode: 'prepare_change', issuedAt: input.createdAt, expiresAt: new Date(Date.parse(input.createdAt) + 15 * 60_000).toISOString(), allowedOrigins: [input.targetOrigin], steps, approvalToken: 'DRY_RUN_NO_RUNTIME_APPROVAL_TOKEN_CREATED', metadata: { dryRunOnly: true, supervisorPolicyApprovalDoesNotGrantRuntimeApproval: true } }
