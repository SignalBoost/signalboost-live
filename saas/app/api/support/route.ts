...
export async function POST(req: NextRequest) {
  try {
    ...
    const local = getConciergeAnswer(
      latestUserMessage,
      languageCode,
      currentPage,
    );
    ...
    if (!process.env.OPENAI_API_KEY) {
      return NextResponse.json({
        reply: local.reply,
        telemetry: local,
        source: "deterministic-concierge",
      });
    }
