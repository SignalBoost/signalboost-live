...
- Fixed findings remain durable audit evidence but must not be presented as active findings after finalization. The Audit Console must use live audit_findings.fixed state instead of the immutable scan log for current status, show zero remaining when all findings are fixed, and keep the original approved/fixed totals as audit evidence.
