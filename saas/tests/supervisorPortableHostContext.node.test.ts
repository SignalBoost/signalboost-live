...
test('the portable boundary modules themselves are unconditionally clean', () => {
  for (const name of ['host-context.ts', 'enterprise-notifier.ts', 'enterprise-dispatch-store.ts', 'siem-audit-sink.ts', 'incident-source.ts', 'webhook-intake.ts', 'incident-runtime.ts', 'reference-verifier.ts', 'monitoring-authenticators.ts', 'triage-thinker.ts', 'index.ts']) {
