...
export default async function PortableProductsPage({ searchParams }: { searchParams?: Promise<SearchParams> }) {
  ...
  if (!access.isAdmin) return <main style={page}><h1>Portable Product Catalog</h1><p style={muted}>Admin access required.</p></main>
  let filters
  try { filters = parsePortableProductCatalogFilters(toSearchParams(await searchParams ?? {})) } catch { return <main style={page}><h1>Portable Product Catalog</h1><p style={muted}>Invalid catalog filters. Use only the listed read-only filters.</p></main> }
  const items = listPortableProductCatalogItems(filters)
  return <main style={page}>
    <section style={panel}><p style={{ color: '#1af0ff', fontWeight: 800 }}>INTERNAL · READ-ONLY</p><h1>Portable Product Catalog</h1><p style={muted}>Registry-backed metadata for developer and operator inspection. No commercial, fulfillment, mutation, or execution actions are available here.</p>
      <nav aria-label="Catalog filters"><strong>Bounded filters: </strong><a href="/dashboard/portable-products" style={link}>All</a>{portableProductCatalogFilterOptions.statuses.map(value => <a key={value} href={`/dashboard/portable-products?status=${value}`} style={link}>status: {value}</a>)}{portableProductCatalogFilterOptions.categories.map(value => <a key={value} href={`/dashboard/portable-products?category=${value}`} style={link}>category: {value}</a>)}{portableProductCatalogFilterOptions.booleans.map(value => <a key={`visible-${value}`} href={`/dashboard/portable-products?publicVisible=${value}`} style={link}>public: {value}</a>)}{portableProductCatalogFilterOptions.booleans.map(value => <a key={`licensed-${value}`} href={`/dashboard/portable-products?licensingAvailable=${value}`} style={link}>licensed: {value}</a>)}</nav>
    </section>
    {items.map(item => <article key={item.productId} style={panel}><h2>{item.displayName}</h2><p style={muted}><code>{item.productId}</code> · {item.status} · {item.maturity} · {item.category} · public: {String(item.publicVisible)} · licensing: {String(item.licensingAvailable)}</p><p>{item.shortDescription}</p><dl style={{ display: 'grid', gap: 10 }}><div><dt>Supported languages</dt><dd>{values(item.supportedLanguages)}</dd></div><div><dt>Architecture references</dt><dd>{values(item.architectureReferences)}</dd></div><div><dt>Documentation references</dt><dd>{values(item.documentationReferences)}</dd></div><div><dt>Dependencies</dt><dd>{values(item.dependencies)}</dd></div><div><dt>Exclusions</dt><dd>{values(item.exclusions)}</dd></div></dl></article>)}
    {!items.length && <section style={panel}><p style={muted}>No catalog items match these bounded filters.</p></section>}
  </main>
}
