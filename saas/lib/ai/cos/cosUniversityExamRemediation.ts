...
async function persistPlan(agentId: string, failure: FailedExamRow): Promise<{ row: PlanRow; strategy: CosUniversityStudyStrategy } | null> {
  ...
  // Recover a plan stalled by the pre-fix same-day run-key collision only when both host-accepted
  // study and completed practice prove that it had legitimately reached independent re-examination.
