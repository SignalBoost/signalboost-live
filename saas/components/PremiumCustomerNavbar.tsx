...
const CUSTOMER_GROUPS: Group[] = [
  {
    ...
    items: [
      ...
      { icon: '🤖', label: 'Assistant / Concierge', href: '/dashboard/assistant', desc: 'The assistant users can ask for planning, building, and support.' },
...
const OWNER_GROUP: Group = {
  id: 'owner', label: 'Owner/Admin', eyebrow: 'Internal control', accent: RED, align: 'right', width: 430, ownerOnly: true,
  items: [
...
export default function PremiumCustomerNavbar() {
  ...
  const groups = [...(user ? CUSTOMER_GROUPS : []), ...PRODUCT_GROUPS, ...(ownerAccess ? [OWNER_GROUP] : []), HELP_GROUP]
    ...
    return (
      <div style={{ position: 'relative' }} onMouseEnter={() => openNow(group.id)} onMouseLeave={closeSoon}>
        ...
        <div style={{ position: 'absolute', top: '100%', ...dropdownPosition(group.align), paddingTop: 12, opacity: open ? 1 : 0, visibility: open ? 'visible' : 'hidden', transform: open ? 'translateY(0)' : 'translateY(8px)', pointerEvents: open ? 'auto' : 'none', transition: 'opacity .18s ease, transform .18s ease, visibility .18s', zIndex: 220 }} onMouseEnter={() => openNow(group.id)} onMouseLeave={closeSoon}>
          <div style={{ width: group.width || 360, maxWidth: '92vw', position: 'relative', overflow: 'hidden', borderRadius: 22, border: `1px solid ${group.accent}33`, background: 'linear-gradient(145deg, rgba(3,7,18,.98), rgba(15,23,42,.96))', boxShadow: `0 28px 90px rgba(0,0,0,.58), 0 0 45px ${group.accent}12`, backdropFilter: 'blur(18px)' }}>
  ...
  return (
    <>
      ...
      <nav ref={navRef} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '11px 22px', background: 'linear-gradient(135deg, rgba(4,8,18,.92), rgba(15,23,42,.76))', borderBottom: '1px solid rgba(26,240,255,.18)', boxShadow: '0 18px 60px rgba(0,0,0,.30), inset 0 1px 0 rgba(255,255,255,.08)', position: 'sticky', top: 0, zIndex: 100, backdropFilter: 'blur(14px)' }}>
      ...
      {mobileOpen ? <div style={{ position: 'sticky', top: 65, zIndex: 99, background: 'rgba(4,8,18,.98)', borderBottom: '1px solid rgba(255,255,255,.14)', padding: 16, maxHeight: '80vh', overflowY: 'auto', backdropFilter: 'blur(14px)' }}>
