...
export function evaluateReleaseCandidateReadiness(input: RcReadinessInput): RcReadinessSnapshot {
  ...
  const score = input.checks.length ? input.checks.reduce((sum, check) => sum + scoreStatus(check.status), 0) / input.checks.length : 0
  const releaseCandidate = required.length > 0 && failedRequiredCheckIds.length === 0 && notRunRequiredCheckIds.length === 0 && requiredPassRate === 1

  return deepFreeze({ schemaVersion: RC_SCHEMA_VERSION, tenant: { ...input.tenant }, generatedAt: input.generatedAt, releaseCandidate, score, requiredPassRate, failedRequiredCheckIds, warningCheckIds, notRunRequiredCheckIds, categoryScores, checks: input.checks.map(check => ({ ...check, evidence: [...check.evidence] })) })
}
