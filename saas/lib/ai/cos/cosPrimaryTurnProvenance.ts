...
/**
 * Provenance/introspection replies are deterministic views over the previous substantive turn and
 * are intentionally not persisted as a new substantive provenance record. If the user asks the
 * same provenance question again with a paraphrase ("answer" -> "idea" -> "where did that come
 * from?"), the browser transcript's preceding assistant text is the provenance report, while the
 * database still correctly points at the substantive answer. Recognize only our own deterministic
 * report shapes so the second paraphrase stays pinned to that same record.
 */
