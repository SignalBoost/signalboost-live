...
Health snapshots, subsystem metrics, alerts, recoveries, and trend buckets are persisted as sanitized metadata only. No raw logs, credentials, provider secrets, browser objects, provider responses, repair controls, provider mutations, Browser Runtime execution, or approval-gate bypass are introduced. The Supervisor Operations Center remains admin-only and read-only for diagnostics while displaying Platform Health, Subsystem Health, Trend Graphs, Recent Alerts, Recent Recoveries, Current Warnings, and Health History. Its only exception is the Global AI Kill Switch: an authenticated administrator may set the singleton `system_status.ai_autonomous_execution_enabled` flag to `false`, which Vercel Edge Middleware uses to fail closed autonomous cron, webhook, and Supervisor ingress without interrupting standard human traffic. Re-enabling execution is intentionally not exposed by this emergency control and requires the governed owner/admin recovery workflow.
...
- 2026-07-18: Added Mission 001 Global AI Kill Switch: a Supabase singleton flag with admin-only mutation RLS, Vercel Edge Middleware fail-closed ingress blocks for autonomous cron/webhook/Supervisor routes, and an admin Supervisor Operations Center override to preserve fail-safe human parity while keeping standard human traffic available.
