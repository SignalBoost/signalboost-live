...
**Current top priority:** accept the first specialist-learning slice on `feat/specialist-learning-telemetry-20260906`: exact Preview/CI, Production deployment, one real owner-directed software study observation, and inspection of the owner-only competency report. The accepted 600-record Builder observation closes the earlier large-task blocker, but broader parity/private-repository claims remain prohibited. After specialist telemetry acceptance, remove inactive legacy Software Specialist routing fallbacks only after confirming no Production traffic depends on them.
