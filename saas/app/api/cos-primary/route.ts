...
import { isExplicitBuilderEvidenceRequest } from '@/lib/builder/execution-evidence'
// saas/app/api/cos-primary/route.ts
...
export async function postCosPrimary(req:NextRequest){
  ...
  if(!isExplicitBuilderEvidenceRequest(input) && (isProvenanceIntrospection(input) || asksWhereTheAnswerCameFrom(input))){
    if(!isPrivileged){
  ...
  if(directCategory){
    ...
    if(!grounded){
      ...
      return NextResponse.json({ok:false,reply,error:reply,source:'cos-authoritative-source-unavailable',confidence_score:0,confidence_threshold:confidenceThreshold(),escalation_reason_code:'insufficient_live_authority',fresh_failure_class:'insufficient_live_authority',external_ai_invoked:false,external_fallback_invoked:false,local_model_invoked:false,execution_provenance:executionProvenance,authoritative_source_attempted:true,authoritative_source_urls:directCategory.sources.map(source=>source.url),live_evidence_retrieved_this_turn:true,live_evidence_sources:[],live_telemetry:liveTelemetry,execution_allowed:false,external_action_taken:false},{status:200})
    }
  ...
  if(requiresFreshEvidence){
    ...
    const constructedEconomics=constructEconomicFactsReply(lookupInput,freshSources)
    if(constructedEconomics){
      const reply=constructedEconomics.reply
      const executionProvenance=attachFreshEvidenceProvenance(authoritativeProvenance(null,{invoked:false}),{sources:constructedEconomics.sources,retrievedAt:freshRetrievedAt,error:null,synthesisAccepted:null})
      await writeCosPrimaryProvenance(userId,reply,executionProvenance,'cos-fresh-economic-facts',{prompt:lookupInput,answered:true,confidence:.99,branch:'fresh_economic_facts'})
      return NextResponse.json({ok:true,reply,source:'cos-fresh-economic-facts',confidence_score:.99,external_ai_invoked:false,local_model_invoked:false,execution_provenance:executionProvenance,live_evidence_retrieved_this_turn:true,live_evidence_sources:constructedEconomics.sources.map(source=>({id:source.id,title:source.title,url:source.url})),execution_allowed:false,external_action_taken:false})
    }
  ...
  const conversationRecall=conversationRecallRequested
    ? await buildConversationRecallContext({userId,request:input,currentConversationId:String(body?.context?.conversationId||body?.conversationId||'')||null,search:searchPastConversations})
    : null
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (payload.suggested_followups.length === 2) {
      after(async () => {
        const userId = (await getAccess().catch(() => null))?.userId || null
        if (!userId) return
        await attachSuggestedFollowupsToStoredTurn(userId, String(payload.reply), payload.suggested_followups).catch(() => false)
      })
    }
    return NextResponse.json(payload, { status: response.status, headers })
  } catch {
