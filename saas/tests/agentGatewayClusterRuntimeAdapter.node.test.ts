...
import { ClusterRuntimeAdapterContract, InMemoryClusterRuntimeInstructionStore, translateClusterTransition } from '../agent-gateway/cluster-runtime-adapter.ts'
...
function commit(overrides: Partial<ClusterTransitionCommit> = {}): ClusterTransitionCommit {
  return Object.freeze({ schemaVersion: 'agent-gateway-cluster-transition-commit-v1', clusterId: 'gateway-us-east', previousTerm: 4, term: 5, leaderId: 'gateway-b', promotedReplicaId: 'gateway-b', demotedReplicaIds: Object.freeze(['gateway-a']), disposition: 'promote-candidate', committedAt: '2026-07-25T21:00:00.000Z', idempotent: false, readOnly: true, executable: false, ...overrides })
}
...
test('emits a noop when no runtime change is required', () => {
  const instructions = translateClusterTransition(commit({ promotedReplicaId: undefined, demotedReplicaIds: Object.freeze([]), disposition: 'retain-leader', idempotent: true }))
  assert.equal(instructions[0].action, 'noop')
})

test('sorts demotion instructions deterministically', () => {
  const instructions = translateClusterTransition(commit({ promotedReplicaId: undefined, demotedReplicaIds: Object.freeze(['gateway-z', 'gateway-a']), disposition: 'demote-conflicting-leaders' }))
