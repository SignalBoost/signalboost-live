...
const LABEL_GROUPS = {
  ...
  hypotheses: /(?:\b(?:candidate\s+(?:causes?|hypotheses)|hypotheses|possible\s+causes?|hip[oó]tesis|hip[oó]teses|hipotezy)\b|(?<![\p{L}\p{N}_])(?:гипотезы|возможные\s+причины)(?![\p{L}\p{N}_]))/iu,
  distinguishers: /(?:\b(?:distinguishing\s+checks?|what\s+would\s+distinguish|how\s+to\s+distinguish|discriminating\s+checks?|distinguir|rozr[oó][żz]ni[cć]|odr[oó][żz]ni[cć])\b|(?<![\p{L}\p{N}_])(?:различить|отличить)(?![\p{L}\p{N}_]))/iu,
  missing: /(?:\b(?:missing\s+(?:readings?|evidence|data|baselines?)|readings?\s*\/\s*baselines?|still\s+needed|faltan|faltantes|brakuj[aą]c)\b|(?<![\p{L}\p{N}_])(?:недостающ\p{L}*|отсутствующ\p{L}*)(?![\p{L}\p{N}_]))/iu,
} as const
