...
**Core Platform pricing:** implementation is active on `fix/platform-pricing-15-coding-20260904` and is **not Production-accepted**. The public Launch tier starts at $15/month and lists the full isolated AI coding workspace (create, edit, run, test, debug, and repair code) in all five supported locales. The server checkout POST is restored and verifies the live Stripe Price is an active USD monthly price whose amount exactly matches the catalog before creating a Checkout Session; a stale $29 Price therefore fails closed. Production requires a new $15 Stripe Price assigned to `STRIPE_PRICE_WEBSITE_LAUNCH`, followed by Preview/Production deployment and a real authenticated checkout handoff observation.
