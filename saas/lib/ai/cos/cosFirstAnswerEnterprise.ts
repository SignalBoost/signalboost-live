...
import { queryNearestLearnedCorpus } from '@/lib/ai/cos/learnedCorpusSemantic'
...
type CachedAnswerOrigin={
  knowledgeFactsUsed:number;learnedItemsUsed:number;enterpriseMemoriesUsed:number;userMemoriesUsed:number;cognitiveSkillsUsed:number
  knowledgeFactsCited:number;learnedItemsCited:number;enterpriseMemoriesCited:number;userMemoriesCited:number;cognitiveSkillsCited:number
  evidenceFunnel?:COSEvidenceFunnel;cognitiveSkillFunnel?:EvidenceFunnelStage
}
type CachedCosAnswer={reply:string;confidence:number;reasonerLabel:string|null;policyVersion?:string|null;storedAt?:string|null;origin?:CachedAnswerOrigin}
type RetrievalCounts={retrieved:number;relevant:number;selected:number}
type InternalContext={
  systems:string[];facts:string[];learned:string[];enterpriseMemories:string[];memories:string[];skills:string[];skillIds:string[]
  enterpriseMemoryStatus:string;enterpriseMemoryOrganizationId:string|null
  funnel:{knowledgeGraph:RetrievalCounts;learnedCorpus:RetrievalCounts;enterpriseMemory:RetrievalCounts;userMemory:RetrievalCounts;cognitiveSkills:RetrievalCounts}
}
...
function cacheHitProvenance(payload:CachedCosAnswer,base:{knowledgeFactsUsed:number;learnedItemsUsed:number;enterpriseMemoriesUsed:number;userMemoriesUsed:number;cognitiveSkillsUsed:number;enterpriseMemoryStatus:string;enterpriseMemoryOrganizationId:string|null;internalSystemsConsulted:string[];evidenceFunnel:COSEvidenceFunnel;cognitiveSkillFunnel:EvidenceFunnelStage},responseSource:'semantic_cache'|'semantic_similarity',similarityScore?:number):COSProvenance{
  const origin=payload.origin
  return{
    responseSource,externalAiInvoked:false,localModelInvoked:false,reasonerLabel:payload.reasonerLabel,internalSystemsConsulted:base.internalSystemsConsulted,
    knowledgeFactsUsed:origin?.knowledgeFactsUsed??0,learnedItemsUsed:origin?.learnedItemsUsed??0,enterpriseMemoriesUsed:origin?.enterpriseMemoriesUsed??0,userMemoriesUsed:origin?.userMemoriesUsed??0,cognitiveSkillsUsed:origin?.cognitiveSkillsUsed??0,
    enterpriseMemoryStatus:base.enterpriseMemoryStatus,enterpriseMemoryOrganizationId:base.enterpriseMemoryOrganizationId,evidenceFunnel:base.evidenceFunnel,cognitiveSkillFunnel:base.cognitiveSkillFunnel,
    knowledgeFactsCited:origin?.knowledgeFactsCited??0,learnedItemsCited:origin?.learnedItemsCited??0,enterpriseMemoriesCited:origin?.enterpriseMemoriesCited??0,userMemoriesCited:origin?.userMemoriesCited??0,cognitiveSkillsCited:origin?.cognitiveSkillsCited??0,
    cacheOrigin:{storedAt:payload.storedAt??null,policyVersion:payload.policyVersion??null,retrievedThisTurn:{facts:base.evidenceFunnel.knowledgeGraph.retrieved,learned:base.evidenceFunnel.learnedCorpus.retrieved,enterprise:base.evidenceFunnel.enterpriseMemory.retrieved,memories:base.evidenceFunnel.userMemory.retrieved,skills:base.cognitiveSkillFunnel.retrieved},originEvidenceFunnel:origin?.evidenceFunnel??null,originCognitiveSkillFunnel:origin?.cognitiveSkillFunnel??null},
...
async function semanticKnowledgeFacts(prompt:string,db:NonNullable<ReturnType<typeof cosServiceDb>>){const work=(async()=>{const vector=await generateLocalEmbedding(prompt);const rows=await new SupabaseKnowledgeStore(db).queryNearestFacts(vector,{matchCount:32,minSimilarity:0});if(rows.some(row=>Number(row.similarityScore||0)>=knowledgeFactSimilarityThreshold()))return rows;const pending=await db.from('cos_knowledge_facts').select('id',{count:'exact',head:true}).is('embedding',null);if(!pending.error&&Number(pending.count??0)>0){console.warn('cosFirstAnswer: relevant semantic fact coverage incomplete; lexical fallback remains active',{pending:pending.count});return null}return rows})().catch(error=>{console.warn('cosFirstAnswer: semantic knowledge retrieval unavailable; lexical fallback will be used',error);return null});const budgetMs=knowledgeFactRetrievalBudgetMs();return Promise.race([work,new Promise<null>(resolve=>setTimeout(()=>{console.warn('cosFirstAnswer: semantic knowledge retrieval exceeded budget; lexical fallback will be used',{budgetMs});resolve(null)},budgetMs))])}

// Corpus mirror of semanticKnowledgeFacts: nearest-neighbour retrieval over cos_continuous_learning
// embeddings. Returns rows when semantic coverage is adequate; returns null ONLY when relevance is
// weak AND un-embedded rows remain (so the caller's lexical ILIKE path fills the gap during
// backfill). Once every row is embedded, a weak result is the honest answer "nothing relevant" and
// we do NOT fall back to keyword noise. Same time budget as the KG path.
async function semanticLearnedCorpus(prompt:string,db:NonNullable<ReturnType<typeof cosServiceDb>>){const work=(async()=>{const vector=await generateLocalEmbedding(prompt);const rows=await queryNearestLearnedCorpus(vector,{matchCount:40,minSimilarity:0});if(rows.some(row=>Number(row.similarity||0)>=learnedContextSimilarityThreshold()))return rows;const pending=await db.from('cos_continuous_learning').select('content_hash',{count:'exact',head:true}).is('embedding',null);if(!pending.error&&Number(pending.count??0)>0){console.warn('cosFirstAnswer: relevant semantic corpus coverage incomplete; lexical fallback remains active',{pending:pending.count});return null}return rows})().catch(error=>{console.warn('cosFirstAnswer: semantic corpus retrieval unavailable; lexical fallback will be used',error);return null});const budgetMs=knowledgeFactRetrievalBudgetMs();return Promise.race([work,new Promise<null>(resolve=>setTimeout(()=>{console.warn('cosFirstAnswer: semantic corpus retrieval exceeded budget; lexical fallback will be used',{budgetMs});resolve(null)},budgetMs))])}
...
async function retrieveInternalContext(prompt:string,userId?:string|null,privileged=false):Promise<InternalContext>{
  const systems=['semantic/exact cache preflight'],facts:string[]=[],learned:string[]=[],enterpriseMemories:string[]=[],memories:string[]=[],skills:string[]=[],skillIds:string[]=[]
  const terms=queryTerms(prompt),db=cosServiceDb(),funnel={knowledgeGraph:emptyRetrieval(),learnedCorpus:emptyRetrieval(),enterpriseMemory:emptyRetrieval(),userMemory:emptyRetrieval(),cognitiveSkills:emptyRetrieval()}
  let enterpriseMemoryStatus=privileged?'organization_not_found':'no_authorized_scope',enterpriseMemoryOrganizationId:string|null=null

  if(db){
    systems.push('Knowledge Graph','Continuous Learning Corpus')
    const [semanticResult,semanticLearnedResult]=await Promise.allSettled([semanticKnowledgeFacts(prompt,db),semanticLearnedCorpus(prompt,db)])
    ...
    if(semanticLearned!==null){
      // Meaning-matched: rows already carry cosine similarity from the RPC, so relevance is judged
      // on the same axis as the KG path and no keyword gate stands in front of it. No re-embed.
      funnel.learnedCorpus.retrieved=semanticLearned.length
      const relevant=semanticLearned.filter(r=>Number(r.similarity||0)>=learnedContextSimilarityThreshold())
  ...
  if(scopeResolution.scope){
    enterpriseMemoryOrganizationId=scopeResolution.scope.organizationId;systems.push('Organization Enterprise Memory')
    try{
...
function executionFunnel(context:InternalContext,injected:boolean,cited={kg:0,cl:0,em:0},enterpriseCited=0):COSEvidenceFunnel{return{knowledgeGraph:stage(context.funnel.knowledgeGraph,injected,cited.kg),learnedCorpus:stage(context.funnel.learnedCorpus,injected,cited.cl),enterpriseMemory:stage(context.funnel.enterpriseMemory,injected,enterpriseCited),userMemory:stage(context.funnel.userMemory,injected,cited.em)}}
function executionSkillFunnel(context:InternalContext,injected:boolean,cited=0):EvidenceFunnelStage{return stage(context.funnel.cognitiveSkills,injected,cited)}
function contextFingerprint(context:{facts:string[];learned:string[];enterpriseMemories:string[];memories:string[];skills:string[]}):string{return createHash('sha256').update(JSON.stringify({facts:context.facts,learned:context.learned,enterpriseMemories:context.enterpriseMemories,memories:context.memories,skills:context.skills})).digest('hex')}
...
export async function tryCOSFirstAnswer(input:{prompt:string;userId?:string|null;language?:string;privileged?:boolean}):Promise<COSFirstAnswerResult>{
  const startedAt=Date.now(),context=await retrieveInternalContext(input.prompt,input.userId,Boolean(input.privileged))
  const base={externalAiInvoked:false as const,localModelInvoked:false,reasonerLabel:null as string|null,internalSystemsConsulted:context.systems,knowledgeFactsUsed:context.facts.length,learnedItemsUsed:context.learned.length,enterpriseMemoriesUsed:context.enterpriseMemories.length,userMemoriesUsed:context.memories.length,cognitiveSkillsUsed:context.skills.length,enterpriseMemoryStatus:context.enterpriseMemoryStatus,enterpriseMemoryOrganizationId:context.enterpriseMemoryOrganizationId,evidenceFunnel:executionFunnel(context,false),cognitiveSkillFunnel:executionSkillFunnel(context,false)}
  ...
  if(cached&&cached.reply&&cachedCurrent.ok&&cached.confidence>=threshold()){recordAvoidedCost('exact_cache',input.prompt.length,cached.reply.length,Date.now()-startedAt);return{handled:true,reply:cached.reply,confidence:cached.confidence,provenance:cacheHitProvenance(cached,base,'semantic_cache')}}
  ...
  const cited=citedEvidence(parsed.answer),enterpriseCited=organizationMemoryCitationCount(parsed.answer),citedProvenance={...reasoningProvenance,knowledgeFactsCited:cited.kg,learnedItemsCited:cited.cl,enterpriseMemoriesCited:enterpriseCited,userMemoriesCited:cited.em,cognitiveSkillsCited:cited.sk,evidenceFunnel:executionFunnel(context,true,cited,enterpriseCited),cognitiveSkillFunnel:executionSkillFunnel(context,true,cited.sk)}
  const groundedCount=citedKnowledgeEvidenceCount({kg:cited.kg,cl:cited.cl,oem:enterpriseCited}),ceiling=groundedEvidenceCeiling(groundedCount),specificity=assessAnswerSpecificity(parsed.answer),confidence=Math.min(parsed.confidence,ceiling,specificity.cap)
  ...
  if(confidence<threshold()){const cappedBySpecificity=specificity.applies&&specificity.cap<Math.min(parsed.confidence,ceiling),reason=cappedBySpecificity?`COS confidence ${confidence.toFixed(2)} is below escalation threshold ${threshold().toFixed(2)}. ${specificityReason(specificity)}`:`COS confidence ${confidence.toFixed(2)} is below escalation threshold ${threshold().toFixed(2)}.`;void recordKnowledgeGap(input.prompt,confidence,reason);return{handled:false,confidence,reason,bestEffortReply:parsed.answer,provenance:{responseSource:'external_fallback_required',...citedProvenance}}}

  const citedSkillIds=citedIndexedValues(parsed.answer,'SK',context.skillIds);if(citedSkillIds.length)void recordCitedCognitiveSkillReuse(citedSkillIds)
  const storedAnswer:CachedCosAnswer={reply:parsed.answer,confidence,reasonerLabel:citedProvenance.reasonerLabel,policyVersion,storedAt:new Date().toISOString(),origin:{knowledgeFactsUsed:context.facts.length,learnedItemsUsed:context.learned.length,enterpriseMemoriesUsed:context.enterpriseMemories.length,userMemoriesUsed:context.memories.length,cognitiveSkillsUsed:context.skills.length,knowledgeFactsCited:cited.kg,learnedItemsCited:cited.cl,enterpriseMemoriesCited:enterpriseCited,userMemoriesCited:cited.em,cognitiveSkillsCited:cited.sk,evidenceFunnel:citedProvenance.evidenceFunnel,cognitiveSkillFunnel:citedProvenance.cognitiveSkillFunnel}}
  const cacheWriteBudgetMs=Number(process.env.COS_CACHE_WRITE_BUDGET_MS??'8000');await Promise.race([Promise.allSettled([writeCachedAnswer(cacheKey,storedAnswer),knowledge?knowledge.commitToMemory(cacheTaskId,input.prompt,contextWindow,storedAnswer):Promise.resolve()]),new Promise<void>(resolve=>setTimeout(()=>{console.warn('cosFirstAnswer: cache write exceeded its budget and was abandoned',{budgetMs:cacheWriteBudgetMs});resolve()},cacheWriteBudgetMs))])
  recordAvoidedCost('local_reasoner',input.prompt.length,parsed.answer.length,Date.now()-startedAt);void resolveKnowledgeGap(input.prompt);return{handled:true,reply:parsed.answer,confidence,provenance:{responseSource:'local_cos_reasoning',...citedProvenance}}
}

export function formatCosWorkflowStatement(result:COSFirstAnswerResult,language='en'):string{const p=result.provenance,evidence=`${p.knowledgeFactsUsed} knowledge facts, ${p.learnedItemsUsed} learned items, ${p.enterpriseMemoriesUsed} enterprise memories, ${p.cognitiveSkillsUsed} validated skills, ${p.userMemoriesUsed} saved memories`,source=p.responseSource==='semantic_cache'?'exact-match cache':p.responseSource==='semantic_similarity'?`semantic match, similarity ${(p.similarityScore??0).toFixed(2)}`:p.reasonerLabel;if(language==='pt')return result.handled?`Fluxo: COS consultou primeiro seu conhecimento, corpus, memória empresarial, habilidades validadas e memória do usuário (${evidence}) → respondeu via ${source} com confiança ${result.confidence.toFixed(2)}. Nenhuma IA externa foi chamada.`:`Fluxo: COS consultou primeiro sua memória interna (${evidence}) → não atingiu confiança suficiente → IA externa é apenas o último recurso.`;if(language==='es')return result.handled?`Flujo: COS consultó primero su conocimiento, corpus, memoria empresarial, habilidades validadas y memoria del usuario (${evidence}) → respondió vía ${source} con confianza ${result.confidence.toFixed(2)}. No se llamó IA externa.`:`Flujo: COS consultó primero su memoria interna (${evidence}) → no alcanzó confianza suficiente → la IA externa es solo el último recurso.`;return result.handled?`Workflow: COS searched its knowledge, learning corpus, organization Enterprise Memory, validated skills and saved user memory first (${evidence}) → answered via ${source} at confidence ${result.confidence.toFixed(2)}. No external AI was called.`:`Workflow: COS searched its internal memory first (${evidence}) → did not reach sufficient confidence → external AI is the last resort.`}
