...
begin
  ...
  select pg_catalog.coalesce(pg_catalog.jsonb_agg(observation), '[]'::jsonb) into incident_indicators
  from pg_catalog.jsonb_array_elements(pg_catalog.coalesce(p_evidence_entry#>'{event,observations}', '[]'::jsonb)) observation
  ...
  if pg_catalog.jsonb_array_length(incident_indicators) > 0 then
    ...
    on conflict (engagement_id, repository) do update set
      ...
      last_observed_at = pg_catalog.greatest(public.security_repository_incident_cases.last_observed_at, excluded.last_observed_at),
