...
test('readiness surfaces remain internal, guarded, and read-only', () => {
  const route = hydrateLocalizedSource(readFileSync(new URL('../app/api/internal/portable-product-readiness/route.ts', import.meta.url), 'utf8'))
  const page = hydrateLocalizedSource(readFileSync(new URL('../app/dashboard/portable-products/readiness/page.tsx', import.meta.url), 'utf8'))
