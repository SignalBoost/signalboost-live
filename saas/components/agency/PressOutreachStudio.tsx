  ...
  return (
    <section className="fathom-glass" style={panel} aria-labelledby="press-pending-title">
      <div style={sectionHead}>
        ...
        <h2 id="press-pending-title" style={h2}><LocalizedText fallback={uiText('generatedUi.u_07e8fb5d12b1d621')} /></h2>
      </div>
        ...
        <div style={cards}>
          {campaigns.map((campaign) => (
            <article key={campaign.id} style={approvalCard}>
              <div style={miniGrid}>
                <p style={field}><strong>{uiText('generatedUi.u_0e570ca6fabe24f9')}</strong><span>{campaign.source || uiText('generatedUi.u_2be7c65058553436')}</span></p>
                <p style={field}><strong>{uiText('generatedUi.u_ce4683e7013a18cd')}</strong><span>{campaign.channel || targetLabel(copy, campaign)}</span></p>
                <p style={field}><strong>{uiText('generatedUi.u_15fc65b69327ad3a')}</strong><span>{campaign.publication_name || targetLabel(copy, campaign)}</span></p>
                <p style={field}><strong>{uiText('generatedUi.u_2b5c3d26721ae9c3')}</strong><span>{campaign.editor_contact || campaign.publication_contact}</span></p>
              </div>
              <h3 style={h3}>{campaign.headline || uiText('generatedUi.u_128e51c1acf569a1')}</h3>
              <p style={muted}><strong style={{ color: '#fff' }}>{uiText('generatedUi.u_60735360aecaf19e')}</strong> {campaign.cta_url || uiText('generatedUi.u_b7b9fc81a82f6021')}</p>
              <pre style={preview}>{campaign.content_body}</pre>
              <label style={label}><LocalizedText fallback={uiText('generatedUi.u_5a74dbb65bc9fd23')} /><input
                  value={publishUrls[campaign.id] ?? campaign.published_url ?? ''}
                  onChange={(event) => onUrlChange(campaign.id, event.target.value)}
                  placeholder={uiText('generatedUi.u_314f1d5543fd3c8b')}
                  style={input}
                />
              </label>
              <div style={actions}>
                <button disabled={saving === campaign.id} style={primary} onClick={() => onDecision(campaign, 'approve')}>
                  {saving === campaign.id ? uiText('generatedUi.u_23e39291d6135814') : uiText('generatedUi.u_04bab00eb0f7c86b')}
                </button>
                <button disabled={saving === campaign.id} style={danger} onClick={() => onDecision(campaign, 'reject')}>{uiText('generatedUi.u_ab604a360777735f')}</button>
              </div>
            </article>
          ))}
        </div>
        ...
        {campaigns.map((campaign) => (
          <article key={campaign.id} style={timelineItem}>
            ...
            <div>
              ...
              {campaign.published_url ? <a href={campaign.published_url} target="_blank" rel="noreferrer" style={link}>{uiText('generatedUi.u_49ca9fb9c16cae68')}</a> : null}
            </div>
          </article>
        ))}
      </div>
  ...
  return (
    <main style={shell} aria-label={copy.ariaLabel}>
      <section className="fathom-glass" style={hero}>
        ...
        <p style={lead}><LocalizedText fallback={uiText('generatedUi.u_7debf3211dadefd4')} /></p>
        <div style={gridStats}>
