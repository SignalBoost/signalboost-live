...
function envPrefix(platform: SocialPlatform) {
...
function envStatus(platform: SocialPlatform): EnvStatus {
  ...
  return {
    clientId: Boolean(process.env[`${prefix}_CLIENT_ID`]),
    clientSecret: Boolean(process.env[`${prefix}_CLIENT_SECRET`]),
  }
}
...
export async function GET() {
  ...
  const platforms = (Object.keys(SOCIAL_CONNECTORS) as SocialPlatform[]).map(platform => {
    ...
    if (needsAccountRef && !token?.accountRef) missing.push('account_ref_destination')
