...
function EndpointDropdown({ provider, value, onChange }) {
  ...
  const endpoints = provider?.endpoints || [] // Placeholder for fetch(`/providers/${provider.id}/endpoints`).
  const selected = endpoints.find(endpoint => endpoint.path === value)
  const filtered = endpoints.filter(endpoint => `${endpoint.path} ${endpoint.example}`.toLowerCase().includes(query.toLowerCase()))
  return <div className="relative"><label className="grid gap-2 text-sm font-bold text-slate-200">Endpoint<input disabled={!provider} className="rounded-xl border border-white/10 bg-slate-950/70 px-3 py-3 text-white outline-none disabled:cursor-not-allowed disabled:opacity-50" value={open ? query : selected?.path || ''} onFocus={() => { setOpen(true); setQuery('') }} onChange={event => { setQuery(event.target.value); setOpen(true) }} placeholder={provider ? 'Search endpoints from provider metadata' : 'Select provider first'} /></label>{open && provider && <div className="absolute z-30 mt-2 max-h-72 w-full overflow-auto rounded-2xl border border-white/10 bg-slate-950 p-2 shadow-2xl">{filtered.map(endpoint => <button key={endpoint.id} type="button" onClick={() => { onChange(endpoint.path); setOpen(false) }} className="w-full rounded-xl px-3 py-3 text-left text-slate-100 hover:bg-cyan-400/10"><span className="block font-mono text-sm text-cyan-100">{endpoint.path}</span><span className="text-xs text-slate-400">Example: {endpoint.example}</span></button>)}{!filtered.length && <p className="px-3 py-4 text-sm text-slate-400">No endpoints found.</p>}</div>}</div>
}
...
export default function ToolBuilder() {
  ...
  const [governance, setGovernance] = useState({ requiresApproval: true, secretsBackendOnly: true, supervisorMonitoring: true })
  const [logsOpen, setLogsOpen] = useState(false)
  const [testStatus, setTestStatus] = useState('Ready')

  useEffect(() => { if (provider) { setAuth({}); setMethod(provider.methods[0]); setEndpoint(provider.endpoints[0]?.path || '') } }, [provider])
  ...
  const blueprint = useMemo(() => ({
    ...
    auth: Object.fromEntries(Object.entries(auth).map(([key, val]) => [key, val ? `backend_ref:${key}` : null])),
    endpoint,
    method,
    request: parseJson(requestBody),
    responseMapping: { [responseMapping.source]: responseMapping.target },
    governance,
