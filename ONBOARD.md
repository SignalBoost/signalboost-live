...
**Normative/public-policy answer consistency:** Production behavior was confirmed across owner and unauthenticated Public Concierge sessions after `066811cc`: answers no longer lead with yes/no and preserve the same neutral meaning across wording variants. Every normative answer must bypass semantic/exact answer cache, retrieve live evidence for the strongest supporting and opposing arguments, and render those source groups separately. PR #1764 deployed that balanced-provenance contract, but the owner's sports-eligibility observation still failed closed. Production logs identified the exact cause: both scope-plan model calls succeeded, yet the repaired plan was rejected as `unknown_contract_rejection` because the acceptance validator enforced undocumented label/finding length limits that the diagnostic validator omitted. `fix/normative-scope-plan-contract-20260901` now uses shared limits in acceptance, diagnosis, and repair instructions; the exact production-length regression passes. TypeScript and the complete 692-test COS gate pass locally. Preview, merge, Production READY, and a fresh owner observation remain required.
