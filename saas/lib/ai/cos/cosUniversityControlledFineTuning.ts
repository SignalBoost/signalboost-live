 ...
 * Once training succeeds, however, the artifact is immediately tracked as an iTMounts local-runtime
 * candidate; promotion controls traffic, not ownership. Once promoted, the graduate is registered
 * for platform adoption and its local artifact lifecycle advances toward runtime binding.
 */
export async function runCosUniversityControlledFineTuning(now = new Date()) {
  ...
  // into a candidate after the existing failure threshold is proven, but it does not attach a dataset,
  // approve training, enable dispatch, call a provider, or expand authority.
  const distillationPreparation = await reconcileCosUniversityDistillationCandidates(now)

  // Backfill/reconcile every completed distilled artifact into the local model library before
  // considering new work. This is deliberately non-spending and never authorizes Production traffic.
