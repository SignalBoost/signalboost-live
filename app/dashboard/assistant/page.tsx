...
export default function PersonalAssistantPage() {
  ...
  const [query, setQuery] = useState(t('assistant.defaultQuery'))
  ...
  return (
    <main className="min-h-screen bg-[#05070b] text-white">
      <section className="rounded-[2rem] border border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(255,215,0,.2),transparent_35%),linear-gradient(135deg,#101827,#05070b)] p-8">
        <p className="text-xs uppercase tracking-[0.35em] text-[#FFD700]">{t('assistant.kicker')}</p>
        <h1 className="mt-4 text-4xl font-black">{t('assistant.title')}</h1>
        <p className="mt-3 max-w-3xl text-white/70">{t('assistant.subtitle')}</p>
      </section>

      <section className="mt-8 grid gap-6 lg:grid-cols-[.8fr_1.2fr]">
        <div className="rounded-3xl border border-white/10 bg-white/[.04] p-6">
          <label className="text-sm font-bold text-white/70" htmlFor="concierge-query">{t('assistant.questionLabel')}</label>
          <textarea
            id="concierge-query"
            value={query}
        ...
        <div className="rounded-3xl border border-[#FFD700]/20 bg-black/50 p-6 shadow-[0_0_45px_rgba(255,215,0,.08)]">
          ...
          <p className="mt-5 rounded-2xl border border-white/10 bg-white/[.04] p-4 text-sm text-white/60">{t('assistant.telemetry', 'Telemetry: {event}', { event: answer.telemetryEvent })}</p>
