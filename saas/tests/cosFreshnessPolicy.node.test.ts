...
test('no-source cascade explanation stays on the prior-answer reasoning path', () => {
  for (const query of [
    "Explain the considerations already identified about should men play in women' sport?",
    "Explain what the answer does not establish about should men play in women' sport?",
  ]) {
    assert.equal(requiresFreshExternalEvidence(query), false, query)
  }
})
...
test('company identity questions never reach public web search, in any phrasing or platform language', () => {
  for (const question of [
    'what or who is signalboost and who owns it?',
    'what is SignalBoost?',
