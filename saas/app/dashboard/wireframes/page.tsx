...
const panelFallbacks: Record<ModuleKey, string[]> = {
  promote: ['Campaign builder panel', 'Reach + conversions chart', 'Concierge suggestion box'],
  ...
  return (
    <div className="sb-hmi-chart" aria-hidden="true">
      {[36, 58, 44, 76, 62, 88].map((height, index) => (
        <span key={index} style={{ height: `${height}%`, background: `linear-gradient(180deg, ${accent}, rgba(26,240,255,.18))` }} />
      ))}
    </div>
  )
}
  ...
  return (
    <article id={module.key} className="sb-hmi-module" tabIndex={0} aria-labelledby={`${module.key}-title`}>
      <div className="sb-hmi-module__top">
        <span className="sb-hmi-icon" style={{ color: module.accent, borderColor: module.accent }}>{module.icon}</span>
        <div>
      ...
      <div className="sb-hmi-module__grid">
        ...
        <div className="sb-hmi-panel">
          ...
          <div className="sb-hmi-feed" aria-hidden="true"><i /><i /><i /></div>
        </div>
      </div>
      <Link className="sb-hmi-link" href={module.href}>{openLabel}</Link>
    </article>
  )
}
...
export default function SaaSWireframesPage() {
  ...
  const modules = useMemo<ModuleWireframe[]>(() => moduleKeys.map((module) => ({
    ...module,
    title: t(dict, `wireframes.modules.${module.key}.title`, {
    ...
    metricLabel: t(dict, `wireframes.modules.${module.key}.metricLabel`, {
    ...
    description: t(dict, `wireframes.modules.${module.key}.description`, {
      ...
      calendar: 'Plan monthly operations with event creation overlays and mission-style reminder timelines.',
    ...
    panels: panelFallbacks[module.key].map((fallback, index) => t(dict, `wireframes.modules.${module.key}.panel${index + 1}`, fallback)),
  })), [dict])

  return (
    <main className="sb-hmi-shell" role="main" aria-labelledby="wireframe-title">
      <section className="sb-hmi-dashboard" aria-label={t(dict, 'wireframes.unifiedAria', 'Unified SaaS dashboard wireframe preview')}>
        <aside className="sb-hmi-sidebar" aria-label={t(dict, 'wireframes.sidebarAria', 'SaaS module navigation')}>
          <Link href="/" className="sb-hmi-brand">SignalBoost</Link>
          <nav>
            {modules.map((module) => (
              <a key={module.key} href={`#${module.key}`} className="sb-hmi-nav-item">
                <span className="sb-hmi-nav-icon" style={{ color: module.accent }}>{module.icon}</span>
                {module.title}
              </a>
            ))}
          </nav>
        ...
        <section className="sb-hmi-main">
          <header className="sb-hmi-hero">
            <p className="sb-hmi-kicker">{t(dict, 'wireframes.kicker', 'NASA-style SaaS HMI wireframes')}</p>
            <h1 id="wireframe-title">{t(dict, 'wireframes.title', 'Cockpit console for office utilities + personal assistant')}</h1>
            <p>{t(dict, 'wireframes.subtitle', 'A responsive dark-neon dashboard system with telemetry cards, keyboard-friendly navigation, glass panels, and module previews ready for approval before production coding.')}</p>
            <div className="sb-hmi-cta-row">
              <Link href="/pricing" className="sb-button-primary">{t(dict, 'wireframes.viewPricing', 'View cockpit pricing')}</Link>
              <a href="#pricing-wireframe" className="sb-button-secondary">{t(dict, 'wireframes.jumpPricing', 'Jump to pricing wireframe')}</a>
            </div>
          </header>

          <section className="sb-hmi-telemetry" aria-label={t(dict, 'wireframes.telemetryAria', 'Unified telemetry cards')}>
            {modules.map((module) => (
              <Link key={module.key} href={`#${module.key}`} className="sb-hmi-telemetry-card" style={{ '--module-accent': module.accent } as CSSProperties}>
          ...
          <section className="sb-hmi-modules" aria-label={t(dict, 'wireframes.modulesAria', 'Module wireframe previews')}>
            {modules.map((module) => <ModulePreview key={module.key} module={module} openLabel={t(dict, 'wireframes.openDashboard', 'Open dashboard')} />)}
          </section>

          <section className="sb-hmi-preview-gallery" aria-labelledby="preview-gallery-title">
            <div>
              <p className="sb-hmi-kicker">{t(dict, 'wireframes.previewKicker', 'PR preview images')}</p>
              <h2 id="preview-gallery-title">{t(dict, 'wireframes.previewTitle', 'Wireframe image exports')}</h2>
            </div>
            <div className="sb-hmi-preview-grid">
              {['unified-saas-dashboard', 'module-promote', 'module-reviews', 'module-calendar', 'module-spreadsheets', 'module-outreach', 'module-assistant', 'pricing-cockpit'].map((image) => (
                <a key={image} href={`/wireframes/${image}.svg`} className="sb-hmi-preview-card">
                  <img src={`/wireframes/${image}.svg`} alt={`${image.replaceAll('-', ' ')} wireframe preview`} />
          ...
          <section id="pricing-wireframe" className="sb-hmi-pricing-preview" aria-labelledby="pricing-wireframe-title">
            <div>
              ...
              <h2 id="pricing-wireframe-title">{t(dict, 'wireframes.pricing.title', 'Tiered SaaS cockpit panels')}</h2>
              <p>{t(dict, 'wireframes.pricing.description', 'Pricing cards use the same console styling and route CTAs directly into each SaaS dashboard module.')}</p>
            </div>
            <div className="sb-hmi-price-grid">
              {['Starter', 'Mission Control', 'Enterprise Orbit'].map((plan, index) => (
                <Link key={plan} href={index === 0 ? '/dashboard/promote' : index === 1 ? '/dashboard/wireframes' : '/dashboard/assistant'} className="sb-hmi-price-card">
                  <span>{index === 0 ? '$19' : index === 1 ? '$49' : 'Custom'}</span>
                  <strong>{t(dict, `wireframes.pricing.plan${index + 1}`, plan)}</strong>
                  <small>{t(dict, 'wireframes.openDashboard', 'Open dashboard')}</small>
                </Link>
              ))}
            </div>
          </section>
