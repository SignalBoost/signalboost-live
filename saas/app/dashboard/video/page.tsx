...
const COPY = {
  ...
  heroSubtitle:     { en: 'Upload a video, generate timestamped captions, choose a branded template, drag the overlay, and export a ready-to-post MP4.', es: 'Sube un video, genera subtítulos, elige una plantilla y exporta un MP4 listo.', pt: 'Faça upload, gere legendas, escolha um template e exporte um MP4 pronto.', pl: 'Prześlij wideo, generuj napisy, wybierz szablon i eksportuj MP4.', ru: 'Загрузите видео, создайте субтитры, выберите шаблон и экспортируйте MP4.' },
  ...
  stepStyle:        { en: 'Style template', es: 'Estilo plantilla', pt: 'Estilo template', pl: 'Styl szablonu', ru: 'Стиль шаблона' },
  ...
  demoNote:         { en: 'Free/demo users can preview the editor. Use credits for captions and exports.', es: 'Usuarios gratuitos pueden previsualizar. Usa créditos para subtítulos y exportaciones.', pt: 'Usuários gratuitos podem pré-visualizar. Use créditos para legendas e exportações.', pl: 'Użytkownicy bezpłatni mogą przeglądać edytor. Użyj kredytów na napisy i eksporty.', ru: 'Бесплатные пользователи могут просматривать. Используйте кредиты для субтитров.' },
  ...
  noCaptionsTL:     { en: 'Generate AI captions or upload SRT/VTT captions to populate the timeline.', es: 'Genera subtítulos IA o sube SRT/VTT para poblar la línea de tiempo.', pt: 'Gere legendas IA ou faça upload de SRT/VTT.', pl: 'Wygeneruj napisy AI lub prześlij SRT/VTT.', ru: 'Создайте AI-субтитры или загрузите SRT/VTT.' },
  ...
  uploadPrompt:     { en: 'Upload a source video to start editing', es: 'Sube un video fuente para empezar', pt: 'Faça upload de um vídeo para começar', pl: 'Prześlij wideo, aby rozpocząć edycję', ru: 'Загрузите видео для начала редактирования' },
  ...
  exportNote:       { en: 'Renders are queued for the video worker, burned in with FFmpeg ASS subtitles, and stored as downloadable MP4s.', es: 'Los renders se ponen en cola, se integran con subtítulos FFmpeg y se almacenan como MP4.', pt: 'Renders são enfileirados, incorporados com legendas FFmpeg e armazenados como MP4.', pl: 'Renderowania są kolejkowane i przechowywane jako MP4.', ru: 'Рендеры ставятся в очередь и сохраняются как MP4.' },
  ...
  initCaptions:     { en: 'Generate AI captions after uploading a video, or upload an SRT/VTT file.', es: 'Genera subtítulos IA después de subir un video, o sube un archivo SRT/VTT.', pt: 'Gere legendas IA após upload ou faça upload de um SRT/VTT.', pl: 'Generuj napisy AI po przesłaniu wideo lub prześlij plik SRT/VTT.', ru: 'Создайте AI-субтитры после загрузки видео или загрузите SRT/VTT.' },
  msgUploading:     { en: 'Preparing', es: 'Preparando', pt: 'Preparando', pl: 'Przygotowywanie', ru: 'Подготовка' },
  msgVideoSelected: { en: 'Video selected. Uploading before caption generation…', es: 'Video seleccionado. Subiendo antes de generar subtítulos…', pt: 'Vídeo selecionado. Fazendo upload…', pl: 'Wideo wybrane. Przesyłanie…', ru: 'Видео выбрано. Загрузка…' },
  msgUploadReady:   { en: 'uploaded directly to storage.', es: 'subido directamente al almacenamiento.', pt: 'enviado diretamente para o armazenamento.', pl: 'przesłane do przechowywania.', ru: 'загружено в хранилище.' },
  ...
  msgWaitUpload:    { en: 'Wait for the video upload to finish before generating captions.', es: 'Espera que termine la carga antes de generar subtítulos.', pt: 'Aguarde o upload terminar antes de gerar legendas.', pl: 'Poczekaj na zakończenie przesyłania.', ru: 'Дождитесь окончания загрузки.' },
  msgGenCaptions:   { en: 'Generating AI captions from stored video…', es: 'Generando subtítulos IA del video almacenado…', pt: 'Gerando legendas IA do vídeo armazenado…', pl: 'Generowanie napisów AI…', ru: 'Создание AI-субтитров…' },
  msgCaptionsReady: { en: 'caption cues. Refresh to update credit count.', es: 'señales de subtítulos.', pt: 'indicações de legendas.', pl: 'wskazówek napisów.', ru: 'реплик субтитров.' },
  ...
  msgNeedSource:    { en: 'Upload the video to storage before exporting.', es: 'Sube el video al almacenamiento antes de exportar.', pt: 'Faça upload do vídeo antes de exportar.', pl: 'Prześlij wideo przed eksportem.', ru: 'Загрузите видео перед экспортом.' },
  msgExportFailed:  { en: 'Export failed.', es: 'Exportación fallida.', pt: 'Exportação falhou.', pl: 'Eksport nie powiódł się.', ru: 'Экспорт не удался.' },
  msgJobFailed:     { en: 'Could not refresh job.', es: 'No se pudo actualizar el trabajo.', pt: 'Não foi possível atualizar o trabalho.', pl: 'Nie można odświeżyć zadania.', ru: 'Не удалось обновить задание.' },
  msgUrlFailed:     { en: 'Could not create direct upload URL.', es: 'No se pudo crear la URL de carga directa.', pt: 'Não foi possível criar a URL de upload direto.', pl: 'Nie można utworzyć URL przesyłania.', ru: 'Не удалось создать URL для загрузки.' },
  msgFinalizeFailed:{ en: 'Could not finalize video upload.', es: 'No se pudo finalizar la carga del video.', pt: 'Não foi possível finalizar o upload do vídeo.', pl: 'Nie można sfinalizować przesyłania wideo.', ru: 'Не удалось завершить загрузку видео.' },
  footerSync:       { en: 'Sync health: canvas time', es: 'Sincronización: tiempo de lienzo', pt: 'Sincronização: tempo do canvas', pl: 'Synchronizacja: czas canvas', ru: 'Синхронизация: время холста' },
  footerDuration:   { en: 'duration', es: 'duración', pt: 'duração', pl: 'czas trwania', ru: 'длительность' },
  footerCaptions:   { en: 'captions', es: 'subtítulos', pt: 'legendas', pl: 'napisy', ru: 'субтитров' },
  footerStorage:    { en: 'storage-based captions enabled', es: 'subtítulos basados en almacenamiento habilitados', pt: 'legendas baseadas em armazenamento habilitadas', pl: 'napisy oparte na przechowywaniu włączone', ru: 'субтитры на основе хранилища включены' },
  footerWorker:     { en: 'queue-backed exports require', es: 'las exportaciones con cola requieren', pt: 'exportações com fila requerem', pl: 'eksporty z kolejką wymagają', ru: 'экспорт с очередью требует' },
}
...
const defaultCaptionStyle: CaptionStyle = { fontFamily: 'Inter, Arial, sans-serif', color: '#ffffff', backgroundColor: 'rgba(0,0,0,0.68)', fontSize: 34, animation: 'fade', x: 50, y: 78 }
const captionPresets: CaptionPreset[] = [
  { id: 'signal',  label: 'SignalBoost',  description: 'descSignal',  style: defaultCaptionStyle, aspectRatio: '9:16' },
  { id: 'tiktok',  label: 'TikTok Bold',  description: 'descTiktok',  style: { ...defaultCaptionStyle, fontFamily: 'Arial Black, Inter, sans-serif', fontSize: 48, color: '#ffffff', backgroundColor: 'rgba(0,0,0,0.72)', animation: 'pop', x: 50, y: 76 }, aspectRatio: '9:16' },
  { id: 'hormozi', label: 'Hormozi',       description: 'descHormozi', style: { ...defaultCaptionStyle, fontFamily: 'Impact, Inter, sans-serif', fontSize: 52, color: '#FFD700', backgroundColor: 'rgba(0,0,0,0.86)', animation: 'pop', x: 50, y: 70 }, aspectRatio: '9:16' },
  { id: 'minimal', label: 'Minimal',       description: 'descMinimal', style: { ...defaultCaptionStyle, fontFamily: 'Inter, Arial, sans-serif', fontSize: 32, color: '#ffffff', backgroundColor: 'rgba(15,23,42,0.52)', animation: 'fade', x: 50, y: 84 }, aspectRatio: '16:9' },
]
...
function normalizeTier(v: string): SubscriptionTier { return ['free','demo','launch','growth','command','paid'].includes(v) ? v as SubscriptionTier : 'free' }
function calculateVideoQuota(tierValue: string, usedMinutes: number, billingProvider: 'stripe'|'paypal' = 'stripe'): VideoQuota {
  const tier = normalizeTier(tierValue)
  const inc: Record<SubscriptionTier,number> = { free:0, demo:0, launch:100, growth:500, command:2000, paid:100 }
  const rate: Record<SubscriptionTier,number> = { free:0.2, demo:0.2, launch:0.18, growth:0.14, command:0.1, paid:0.18 }
  const includedMinutes = inc[tier], overageMinutes = Math.max(0, usedMinutes - includedMinutes), demoOnly = tier==='free'||tier==='demo', exportEnabled = !demoOnly
  return { tier, usedMinutes, includedMinutes, overageMinutes, overageRateUsd: rate[tier], exportEnabled, demoOnly, requiresOverageCharge: exportEnabled && overageMinutes > 0, overageProvider: billingProvider }
}
function activeCue(cues: CaptionCue[], time: number) { return cues.find(c => time >= c.start && time <= c.end) || null }
function clamp(v: number, min: number, max: number) { return Math.min(max, Math.max(min, v)) }
function formatTime(s: number) { const safe=Math.max(0,Number(s)||0), mins=Math.floor(safe/60), secs=Math.floor(safe%60), tenths=Math.floor((safe-Math.floor(safe))*10); return `${mins}:${String(secs).padStart(2,'0')}.${tenths}` }
function formatFileSize(b: number) { const mb=b/1024/1024; return `${mb.toFixed(mb>=10?0:1)} MB` }
async function readJsonResponse<T>(r: Response): Promise<T> { const text=await r.text(); try { return JSON.parse(text) as T } catch { throw new Error(text.replace(/\s+/g,' ').trim()||`Request failed with status ${r.status}`) } }
function getBrowserSupabase() {
  const url=process.env.NEXT_PUBLIC_SUPABASE_URL, key=process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY
  if (!url||!key) throw new Error('Supabase browser keys are missing.')
...
function wrapCaption(ctx: CanvasRenderingContext2D, text: string, maxWidth: number) {
  ...
  let current=''
  for (const word of words) { const next=current?`${current} ${word}`:word; if (ctx.measureText(next).width<=maxWidth||!current) { current=next } else { lines.push(current); current=word } }
  if (current) lines.push(current)
  return lines.slice(0,4)
}
function drawRoundRect(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, r: number) {
  const sr=Math.min(r,w/2,h/2)
  ctx.beginPath(); ctx.moveTo(x+sr,y); ctx.lineTo(x+w-sr,y); ctx.quadraticCurveTo(x+w,y,x+w,y+sr); ctx.lineTo(x+w,y+h-sr); ctx.quadraticCurveTo(x+w,y+h,x+w-sr,y+h); ctx.lineTo(x+sr,y+h); ctx.quadraticCurveTo(x,y+h,x,y+h-sr); ctx.lineTo(x,y+sr); ctx.quadraticCurveTo(x,y,x+sr,y); ctx.closePath()
}
function QuotaStatusBar({ quota }: { quota: VideoQuota }) {
  const { lang } = useI18n()
  const pct = quota.includedMinutes > 0 ? Math.min(100, Math.round((quota.usedMinutes / Math.max(1, quota.includedMinutes)) * 100)) : 0
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[.04] p-4">
      <div className="flex items-center justify-between text-sm"><span>{c('videoUsage', lang)}</span><span>{quota.usedMinutes}/{quota.includedMinutes} min</span></div>
      ...
      {quota.demoOnly ? <p className="mt-2 text-sm text-amber-200">{c('demoNote', lang)}</p> : null}
    </div>
  )
}
function BillingBanner({ quota }: { quota: VideoQuota }) {
  ...
  return (
    <div className="rounded-2xl border border-amber-300/30 bg-amber-300/10 p-4 text-sm text-amber-100">
      {c('overageMsg', lang)} {quota.overageMinutes} {c('overageAt', lang)} ${quota.overageRateUsd.toFixed(2)}{c('overageMin', lang)} {c('overageCreate', lang)} {quota.overageProvider} {c('overageEvent', lang)}
    </div>
  )
}
    ...
    <section className="rounded-3xl border border-white/10 bg-black/40 p-5">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        {captionPresets.map(preset => (
          <button key={preset.id} type="button" onClick={() => onPreset(preset)} className={`rounded-2xl border p-4 text-left transition ${activePreset===preset.id?'border-[#FFD700] bg-[#FFD700]/10':'border-white/10 bg-white/[.03] hover:border-white/25'}`}>
            <span className="font-bold">{preset.label}</span><span className="mt-1 block text-xs text-white/55">{c(preset.description, lang)}</span>
          </button>
        ))}
      </div>
    </section>
...
function CaptionTimeline({ cues, currentTime, selectedCueId, onSeek, onSelect, onUpdateText }: { cues: CaptionCue[]; currentTime: number; selectedCueId: string|null; onSeek: (s:number)=>void; onSelect: (id:string)=>void; onUpdateText: (id:string, text:string)=>void }) {
  const { lang } = useI18n()
  const selectedCue = cues.find(c => c.id===selectedCueId) || activeCue(cues, currentTime) || cues[0]
  return (
    <section className="rounded-3xl border border-white/10 bg-black/40 p-5">
      ...
      {selectedCue ? (
        <label className="mt-4 block text-sm">{c('editCaption', lang)}
          <textarea className="mt-2 min-h-24 w-full rounded-2xl border border-white/10 bg-white/10 p-3" value={selectedCue.text} onChange={e => onUpdateText(selectedCue.id, e.target.value)} />
      ...
      <div className="mt-4 max-h-72 space-y-2 overflow-auto">
        {cues.map(cue => (
          <button key={cue.id} type="button" onClick={() => { onSelect(cue.id); onSeek(cue.start) }} className={`w-full rounded-2xl border p-3 text-left text-sm transition ${selectedCue?.id===cue.id||(currentTime>=cue.start&&currentTime<=cue.end)?'border-[#FFD700] bg-[#FFD700]/10':'border-white/10 bg-white/[.03] hover:border-white/25'}`}>
            <span className="font-mono text-xs text-[#FFD700]">{formatTime(cue.start)} → {formatTime(cue.end)}</span><br />{cue.text}
          </button>
        ))}
      </div>
    </section>
...
function CanvasEditorPanel({ videoUrl, cues, style, aspectRatio, seekTime, durationSec, onStyleChange, onTime, onDuration }: { videoUrl: string|null; cues: CaptionCue[]; style: CaptionStyle; aspectRatio: AspectRatio; seekTime: number; durationSec: number; onStyleChange: (s:CaptionStyle)=>void; onTime: (t:number)=>void; onDuration: (s:number)=>void }) {
  const { lang } = useI18n()
  const videoRef = useRef<HTMLVideoElement>(null), canvasRef = useRef<HTMLCanvasElement>(null)
  const [time, setTime] = useState(0), [isPlaying, setIsPlaying] = useState(false), [dragging, setDragging] = useState(false)
  const size = canvasSizes[aspectRatio]
  useEffect(() => { if (videoRef.current && Math.abs(videoRef.current.currentTime - seekTime) > 0.08) { videoRef.current.currentTime = seekTime; setTime(seekTime) } }, [seekTime])
  useEffect(() => {
    let frame = 0
    const draw = () => {
      ...
      if (canvas) {
        ...
        if (ctx) {
          ...
          if (!videoUrl) {
            ctx.fillStyle='rgba(255,255,255,.08)'; drawRoundRect(ctx, canvas.width*.12, canvas.height*.42, canvas.width*.76, canvas.height*.16, 28); ctx.fill()
            ctx.fillStyle='rgba(255,255,255,.68)'; ctx.font=`700 ${Math.max(22,canvas.width*.035)}px Inter, Arial, sans-serif`; ctx.textAlign='center'; ctx.textBaseline='middle'
            ctx.fillText(c('uploadPrompt', lang), canvas.width/2, canvas.height/2)
          }
          const renderTime = video?.currentTime ?? time, cue = activeCue(cues, renderTime)
          if (cue) {
            const animOffset = style.animation==='slide' ? Math.max(0, 1-((renderTime-cue.start)/0.2))*(canvas.height*.05) : 0
            const scale = style.animation==='pop' ? 1+Math.max(0,1-((renderTime-cue.start)/0.18))*.08 : 1
            const fontScale = aspectRatio==='9:16' ? canvas.width/720 : aspectRatio==='1:1' ? canvas.width/1080 : canvas.width/1280
            const fontSize=style.fontSize*scale*fontScale, x=canvas.width*style.x/100, y=(canvas.height*style.y/100)+animOffset
            ctx.save(); ctx.globalAlpha=style.animation==='fade' ? clamp(Math.min(renderTime-cue.start,cue.end-renderTime)/0.18,0.25,1) : 1
            ctx.font=`800 ${fontSize}px ${style.fontFamily}`; ctx.textAlign='center'; ctx.textBaseline='middle'
            const lines=wrapCaption(ctx,cue.text,canvas.width*.82), lineHeight=fontSize*1.18
            const textWidth=Math.max(...lines.map(l=>ctx.measureText(l).width),fontSize*2), boxWidth=Math.min(canvas.width-48,textWidth+48), boxHeight=(lines.length*lineHeight)+34
            ctx.fillStyle=style.backgroundColor; drawRoundRect(ctx,x-boxWidth/2,y-boxHeight/2,boxWidth,boxHeight,22); ctx.fill()
            ctx.fillStyle=style.color; lines.forEach((line,i) => { ctx.fillText(line, x, y+((i-(lines.length-1)/2)*lineHeight)) }); ctx.restore()
          }
        }
      }
  ...
  const setPosition = useCallback((e: React.PointerEvent<HTMLCanvasElement>) => {
    const rect = e.currentTarget.getBoundingClientRect()
    onStyleChange({ ...style, x: Math.round(clamp(((e.clientX-rect.left)/rect.width)*100,8,92)), y: Math.round(clamp(((e.clientY-rect.top)/rect.height)*100,8,92)) })
  }, [onStyleChange, style])
  const seek = (s: number) => { const max=Math.max(0,durationSec||videoRef.current?.duration||0), next=clamp(s,0,max); if (videoRef.current) videoRef.current.currentTime=next; setTime(next); onTime(next) }
  return (
    <section className="rounded-3xl border border-white/10 bg-black/50 p-5">
      ...
      <div className={`relative mx-auto mt-4 w-full overflow-hidden rounded-2xl bg-black ring-1 ring-white/10 ${aspectClasses[aspectRatio]}`}>
        {videoUrl ? <video ref={videoRef} src={videoUrl} className="absolute inset-0 h-full w-full object-cover" playsInline preload="metadata" onLoadedMetadata={e => onDuration(Number((e.currentTarget.duration||0).toFixed(2)))} onPlay={() => setIsPlaying(true)} onPause={() => setIsPlaying(false)} onTimeUpdate={e => { const n=e.currentTarget.currentTime; setTime(n); onTime(n) }} /> : null}
        <canvas ref={canvasRef} width={size.width} height={size.height} onPointerDown={e => { setDragging(true); e.currentTarget.setPointerCapture(e.pointerId); setPosition(e) }} onPointerMove={e => dragging && setPosition(e)} onPointerUp={e => { setDragging(false); e.currentTarget.releasePointerCapture(e.pointerId) }} onPointerCancel={() => setDragging(false)} className="relative z-10 h-full w-full cursor-move touch-none" aria-label="Video canvas with draggable caption overlay" />
      </div>
...
function ExportPanel({ canExport, hasSource, job, onExport, onRefresh }: { canExport: boolean; hasSource: boolean; job: JobState|null; onExport: ()=>void; onRefresh: ()=>void }) {
  ...
  return (
    <section className="rounded-3xl border border-white/10 bg-black/40 p-5">
      ...
      <button onClick={onExport} className="mt-4 w-full rounded-full bg-[#FFD700] px-5 py-3 font-bold text-black disabled:opacity-50" disabled={!canExport||!hasSource}>{c('exportBtn',lang)}</button>
      {!canExport ? <p className="mt-3 rounded-2xl border border-amber-300/30 bg-amber-300/10 p-3 text-sm text-amber-100">{c('exportDemoNote',lang)}</p> : null}
      {job ? (
        <div className="mt-4 rounded-2xl bg-white/[.04] p-4 text-sm">
          ...
          <button onClick={onRefresh} className="mt-3 rounded-full border border-white/20 px-4 py-2 disabled:opacity-50" disabled={job.jobId==='blocked'}>{c('refresh',lang)}</button>
          {job.result_url ? <a className="ml-3 text-[#FFD700]" href={job.result_url} download>{c('downloadMp4',lang)}</a> : null}
        </div>
      ) : null}
    </section>
  )
...
export default function VideoPage() {
  ...
  const [captionState, setCaptionState]   = useState<CaptionGenerationState>({ status: 'idle', message: '' })

  useEffect(() => {
    setUploadState({ status: 'idle', message: c('initUpload', l) })
    setCaptionState({ status: 'idle', message: c('initCaptions', l) })
  }, [l])

  const quota = useMemo(() => {
    const proj = usedMinutes + Math.ceil(Math.max(1, durationSec) / 60)
    return calculateVideoQuota(tier, proj)
  ...
  async function uploadVideo(file: File) {
    ...
    const nextUrl = URL.createObjectURL(file); localPreviewRef.current = nextUrl; setPreviewUrl(nextUrl)
    try {
      const r1 = await fetch('/api/video/upload-url', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ filename: file.name }) })
      const p1 = await readJsonResponse<{ ok: boolean; data?: { bucket: string; path: string; token: string }; error?: string }>(r1)
      if (!p1.ok || !p1.data) throw new Error(p1.error || c('msgUrlFailed',l))
      const sb = getBrowserSupabase(), { bucket, path, token } = p1.data
      const { error: ue } = await sb.storage.from(bucket).uploadToSignedUrl(path, token, file)
      if (ue) throw new Error(ue.message)
      const r2 = await fetch('/api/video/upload-complete', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ bucket, path }) })
      const p2 = await readJsonResponse<{ ok: boolean; data?: { signedUrl: string; bucket: string; path: string }; error?: string }>(r2)
      if (!p2.ok || !p2.data?.signedUrl) throw new Error(p2.error || c('msgFinalizeFailed',l))
      setSourceUrl(p2.data.signedUrl); setStorageObject({ bucket: p2.data.bucket, path: p2.data.path, signedUrl: p2.data.signedUrl })
    ...
    } catch (err) { setUploadState({ status: 'failed', message: err instanceof Error ? err.message : c('msgUploadFailed',l) }) }
  }
  ...
  async function generateCaptions() {
    ...
    try {
      ...
      const p = await readJsonResponse<{ ok: boolean; data?: { cues?: CaptionCue[] }; error?: string }>(res)
      if (p.ok) {
        const nc = Array.isArray(p.data?.cues) ? p.data.cues : []
        setCues(nc); setSelectedCueId(nc[0]?.id||null); setCurrentTime(nc[0]?.start||0)
        setCaptionState({ status: 'ready', message: `${nc.length} ${c('msgCaptionsReady',l)}` })
      } else { setCaptionState({ status: 'failed', message: p.error || c('msgCaptionFailed',l) }) }
    } catch (err) { setCaptionState({ status: 'failed', message: err instanceof Error ? err.message : c('msgCaptionFailed',l) }) }
  }
  ...
  async function uploadCaptions(file: File) {
    ...
    try {
      ...
      if (p.ok) {
        const nc = Array.isArray(p.data?.cues) ? p.data.cues : []
        setCues(nc); setSelectedCueId(nc[0]?.id||null); setCurrentTime(nc[0]?.start||0)
        setCaptionState({ status: 'ready', message: `${nc.length} ${c('msgCaptionLoaded',l)} ${file.name}.` })
      } else { setCaptionState({ status: 'failed', message: p.error || c('msgCaptionUpFailed',l) }) }
    } catch (err) { setCaptionState({ status: 'failed', message: err instanceof Error ? err.message : c('msgCaptionUpFailed',l) }) }
  }
  ...
  async function exportVideo() {
    ...
    try {
      ...
      const p = await readJsonResponse<{ ok: boolean; data?: { jobId: string; status: string }; error?: string }>(res)
      if (p.ok && p.data) { setJob({ jobId: p.data.jobId, status: p.data.status }) }
      else { setJob({ jobId: 'blocked', status: 'failed', error: p.error || c('msgExportFailed',l) }) }
    } catch (err) { setJob({ jobId: 'blocked', status: 'failed', error: err instanceof Error ? err.message : c('msgExportFailed',l) }) }
  }
  ...
  async function refreshJob() {
    ...
    try {
      ...
      const p = await readJsonResponse<{ ok: boolean; data?: { status: string; result_url?: string|null; error?: string|null }; error?: string }>(res)
      if (p.ok && p.data) setJob({ jobId: job.jobId, status: p.data.status, result_url: p.data.result_url, error: p.data.error })
    } catch (err) { setJob({ jobId: job.jobId, status: 'failed', error: err instanceof Error ? err.message : c('msgJobFailed',l) }) }
  }
    ...
    <main className="min-h-screen bg-[#05070b] p-6 text-white">
      ...
      <section className="mt-6 rounded-3xl border border-white/10 bg-white/[.03] p-5">
        <div className="grid gap-4 md:grid-cols-5">
          <label className="text-sm">{c('videoLabel',l)}<input type="file" accept="video/*" onChange={e => e.target.files?.[0] && uploadVideo(e.target.files[0])} className="mt-2 w-full" /></label>
          <label className="text-sm">{c('captionsFallback',l)}<input type="file" accept=".srt,.vtt,text/vtt" onChange={e => e.target.files?.[0] && uploadCaptions(e.target.files[0])} className="mt-2 w-full" /></label>
          <label className="text-sm">{c('tierLabel',l)}<select className="mt-2 w-full rounded-xl bg-black p-2" value={tier} onChange={e => setTier(e.target.value as SubscriptionTier)}><option value="free">{c('tierFree',l)}</option><option value="launch">Launch</option><option value="growth">Growth</option><option value="command">Command</option></select></label>
          <label className="text-sm">{c('localeLabel',l)}<select className="mt-2 w-full rounded-xl bg-black p-2" value={locale} onChange={e => setLocale(e.target.value as SupportedVideoLocale)}><option value="en">en</option><option value="es">es</option><option value="pt">pt</option><option value="pl">pl</option><option value="ru">ru</option></select></label>
          <label className="text-sm">{c('usedMinutesLabel',l)}<input type="number" min="0" className="mt-2 w-full rounded-xl bg-black p-2" value={usedMinutes} onChange={e => setUsedMinutes(Number(e.target.value))} /></label>
        </div>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[1fr_.42fr]">
        <div className="space-y-6">
          <CanvasEditorPanel videoUrl={previewUrl} cues={cues} style={style} aspectRatio={aspectRatio} seekTime={currentTime} durationSec={durationSec} onStyleChange={setStyle} onTime={setCurrentTime} onDuration={setDurationSec} />
