...
if (start === -1 || end === -1) {
  throw new Error('Could not find stripe.create_product block')
}
...
const replacement = `  // Create Product + first Price
  if (template.id === 'stripe.create_product') {
    ...
    if (!priceRes.ok) {
      ...
      return {
        ok: false,
        error: 'Product created, but price creation failed: ' + (e || priceRes.statusText),
        data: {
          productId: product.id,
          productName: product.name,
        },
    ...
    return {
      ok: true,
      message: 'Product and price created: ' + product.name,
      data: {
        productId: product.id,
        productName: product.name,
        priceId: price.id,
...
console.log('Fixed stripe.create_product: now creates product + first price.')
