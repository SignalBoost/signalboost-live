...
test('unified nav and admin sidebar include Marketplace, SaaS, Concierge, and role sections', () => {
  ...
  assert.deepEqual(ADMIN_SIDEBAR.map(item => item.label), ['Overview', 'Logs', 'Outreach', 'Marketing + Sales', 'Insights', 'Role Management', 'Marketplace Monitor', 'SaaS Monitor', 'Concierge Monitor', 'Governance', 'Timeline'])
})
