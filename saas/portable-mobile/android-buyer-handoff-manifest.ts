...
const PROVIDER_HUB_PORTABLE_ID = 'provider-hub' as const
const PROVIDER_HUB_PACKAGE_NAME = 'com.signalboost.providerhub' as const
const MAX_REFERENCE_DECODE_ROUNDS = 4
...
function fullyDecodedReference(value: string): string | null {
  let current = value
  for (let round = 0; round < MAX_REFERENCE_DECODE_ROUNDS; round += 1) {
    if (current.includes('..') || current.includes('@') || UNSAFE.test(current) || CREDENTIAL_PARAMETER.test(current)) return null
    let decoded: string
    try {
...
function validReference(value: unknown): value is string {
  return typeof value === 'string' && value.length > 0 && value.length <= 512 && REFERENCE.test(value) && fullyDecodedReference(value) !== null
}
...
function validChainReport(chain: Value | null): boolean {
  ...
  if (chain.portableId !== PROVIDER_HUB_PORTABLE_ID || chain.packageName !== PROVIDER_HUB_PACKAGE_NAME) return false
...
export function validateAndroidBuyerHandoffManifest(inputValue: unknown) {
  ...
  if (portableId !== PROVIDER_HUB_PORTABLE_ID || packageName !== PROVIDER_HUB_PACKAGE_NAME || portableId !== chain?.portableId || packageName !== chain?.packageName) blockers.push('identity')
