...
**Builder project continuity — 2026-09-06:** implemented on `feat/builder-project-continuity`; Production acceptance pending. Executable follow-ups that name an existing file or refer to the current project select the latest persisted Builder workspace in the same authenticated user/conversation scope. New projects, uploads, repository imports and ordinary explanation questions keep their existing routing. Private platform-repair jobs never become public workspace context. Active selected jobs and unavailable history stop the follow-up before workspace mutation. Previous objective/status/commands are bounded context only; current files and fresh command results remain authoritative. Explicit feature extensions no longer require an invented pre-existing failure, while actual repair objectives and failures observed during extension still require fail/edit/pass evidence. Both direct Builder and shared Software Specialist entry points use the same selection helper. Live acceptance must extend an existing project without reattaching its source, preserve its prior files/assertions, and record fresh verification.
