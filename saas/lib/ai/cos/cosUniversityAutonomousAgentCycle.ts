  ...
  for (const agent of registered) {
    try {
      ...
      if (readyExam.runs.length > 0) {
        ...
        // The dedicated COS lane has always required host-written accepted-study proof before it may
        // touch the practice queue. This cycle executed practice without it, so a specialist's practice
        // carried a weaker guarantee than COS's for the same stage. Every learner now clears the same
        // gate, bound to the exact plan and round; a learner with no current proof does not practise
