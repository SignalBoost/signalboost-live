// saas/self-healing-host/incident-intake.ts
//
// THE PLATFORM'S INTAKE WIRING.
//
// Everything under lib/supervisor/portable/ is host-agnostic by design and reads no
// configuration. This file is the opposite end: it is the one place that turns this
// deployment's environment into a running set of incident sources. A buyer replaces
// exactly this file and keeps everything else.
//
// FAIL-CLOSED IS THE WHOLE POINT OF THE SHAPE HERE. A source is mounted only when a
// secret for it exists. No secret means the source does not exist, and the endpoint
// answers "unknown source" rather than accepting unauthenticated alerts — an
// unauthenticated intake endpoint on the public internet is worse than no endpoint,
// because it looks like it is working.
//
// WHAT THIS DELIBERATELY DOES NOT DO YET: it does not run diagnosis or repair. An
// accepted alert is validated, deduplicated and recorded, and the response says so.
// Wiring the orchestrator needs a Thinker bound to this platform's provider access,
// which is its own slice. Claiming more than that in the response would be the exact
// overstatement the whole portable is built to avoid.
...
export interface IntakeWiringNote { sourceId: string; mounted: boolean; reason: string }
...
export function getIncidentIntake(): { runtime: ReturnType<typeof createIncidentRuntime>; notes: IntakeWiringNote[] } {
  ...
  } else notes.push({ sourceId: GENERIC_SOURCE_ID, mounted: false, reason: 'SUPERVISOR_INTAKE_SECRET is not set' })

  for (const adapterId of monitoringAdapterIds) {
    const envName = VENDOR_SECRET_ENV[adapterId]
    const secret = secretOf(envName)
    if (!secret) { notes.push({ sourceId: adapterId, mounted: false, reason: `${envName} is not set` }); continue }
    try {
  ...
  if (sources.length === 0) sources.push(createIncidentSource({ sourceId: '__unconfigured__', vendor: 'none', status: 'disabled', map: () => null }))
