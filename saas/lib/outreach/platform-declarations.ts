// saas/lib/outreach/platform-declarations.ts
//
...
// to "where do declarations live": a table.
//
// NAMED DELIBERATELY UNLIKE ITS PARTNER. This started life as social-custom-platform-store.ts
// and the near-identical name caused a real content swap during delivery — the store's body
// landed inside the registry file, which then imported itself. Two files that must never be
// confused should not differ by one suffix. A buyer running the portable elsewhere writes
