...
export async function GET(_req: NextRequest, context: { params: Promise<{ capability: string; filename: string }> }) {
  ...
  if (!hfToken || !verifyHfWorkerDeliveryToken(capability, hfToken)) {
  ...
  try {
    ...
    return new NextResponse('worker artifact unavailable', { status: 503, headers: { 'cache-control': 'no-store' } })
  }
}
