...
export function buildDiagnosticRepairPrompt(originalPrompt: string, firstRaw: string): string {
  ...
  return [
    originalPrompt,
    '',
    'QUALITY REPAIR — solve the ORIGINAL diagnostic task again from the facts actually supplied. The prior draft was rejected for weak specificity; do not import facts, symptoms, headings, or assumptions from any other case.',
    '',
    'Scenario fidelity is mandatory:',
    '- Treat only facts in the ORIGINAL prompt as recorded facts. Do not invent or import tenant classes, deployment state, traffic conditions, resource telemetry, infrastructure details, organizations, counts, or outcomes that the prompt does not establish.',
    '- Separate recorded facts from inference and keep material unknowns explicit.',
    '- Diagnose the actual domain and objective named in the prompt. Do not reinterpret a policy, scientific, security, business, learning, or other case as a software-latency incident unless the ORIGINAL prompt actually describes one.',
    '- Reason from the asymmetries and constraints that are actually supplied, not from a memorized benchmark scenario.',
    '',
    'The rewritten answer should, where applicable:',
    '- identify concrete mechanisms or hypotheses rather than generic category labels;',
    '- explain how each mechanism follows from the supplied facts and what remains uncertain;',
    '- name the smallest safe observable, test, check, or falsification condition that distinguishes the hypotheses;',
    '- respect every read-only or non-mutating constraint in the ORIGINAL prompt; when production changes are forbidden, do not require a production mutation;',
    '- keep claims proportional to evidence and never claim verification, deployment, approval, recovery, or an outcome that the ORIGINAL prompt does not establish.',
    '',
    'Preserve the caller\'s requested output contract, including strict JSON when required.',
    'Return a fresh answer to the ORIGINAL task. Do not mention this repair instruction or the rejected draft.',
  ].join('\n')
}
