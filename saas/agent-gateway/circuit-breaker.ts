...
export function recordCircuitFailure(
  ...
  return Object.freeze({
    ...
    consecutiveFailures,
    openedAt: shouldOpen ? current.toISOString() : snapshot.openedAt,
    retryAfter: shouldOpen ? new Date(current.getTime() + checked.openDurationMs).toISOString() : snapshot.retryAfter,
    probeInFlight: false,
...
export function admitThroughCircuit(
  snapshot: CircuitBreakerSnapshot,
  now = new Date(),
): CircuitAdmission {
  const current = validNow(now)
  if (snapshot.state === 'closed') {
    return Object.freeze({ schemaVersion: 'agent-gateway-circuit-admission-v1', key: snapshot.key, disposition: 'allow', reason: 'dependency circuit closed', state: 'closed', readOnly: true, executable: false })
  }
  ...
  if (snapshot.state === 'open') {
    ...
    if (retryAt > current.getTime()) {
      return Object.freeze({ schemaVersion: 'agent-gateway-circuit-admission-v1', key: snapshot.key, disposition: 'reject_dependency_only', reason: 'dependency isolated until retry window', state: 'open', readOnly: true, executable: false })
    }
  ...
  return Object.freeze({
    schemaVersion: 'agent-gateway-circuit-admission-v1',
    key: snapshot.key,
    disposition: snapshot.probeInFlight ? 'reject_dependency_only' : 'probe',
    reason: snapshot.probeInFlight ? 'half-open probe already in flight' : 'one bounded half-open probe permitted',
    state: 'half_open',
    readOnly: true,
    executable: false,
  })
