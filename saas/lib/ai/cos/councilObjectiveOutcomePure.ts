...
export type CouncilObjectiveCorrelation = { kind: CouncilCorrelationKind; value: string }
...
export function extractCouncilCorrelationRefs(prompt: string): Partial<Record<CouncilCorrelationKind, string>> {
  ...
  const incidentId = firstMatch(text, [/["']incident_id["']\s*:\s*["']([^"']{3,500})["']/i, /["']incidentId["']\s*:\s*["']([^"']{3,500})["']/i, /\bincident(?:\s+id)?\s*[:=]\s*([A-Za-z0-9._:/-]{3,500})/i])
  if (incidentId) refs.incident_id = incidentId
  const traceId = firstMatch(text, [/["']trace_id["']\s*:\s*["']([^"']{3,500})["']/i, /["']traceId["']\s*:\s*["']([^"']{3,500})["']/i])
  if (traceId) refs.trace_id = traceId
  const executionId = firstMatch(text, [/["']execution_id["']\s*:\s*["']([^"']{3,500})["']/i, /["']executionId["']\s*:\s*["']([^"']{3,500})["']/i])
  if (executionId) refs.execution_id = executionId
  const recoveryKey = firstMatch(text, [/["']recovery_key["']\s*:\s*["']([^"']{3,500})["']/i, /["']recoveryKey["']\s*:\s*["']([^"']{3,500})["']/i])
  if (recoveryKey) refs.recovery_key = recoveryKey
  const deploymentId = firstMatch(text, [/["']deployment_id["']\s*:\s*["']([^"']{3,500})["']/i, /["']deploymentId["']\s*:\s*["']([^"']{3,500})["']/i])
...
export function normalizeCouncilObjectiveOutcome(input: CouncilObjectiveOutcomeInput): CouncilObjectiveOutcomeInput {
  ...
  if (!['deterministic_tool', 'production_outcome', 'authoritative_record'].includes(sourceClass)) throw new Error('Unsupported automatic Council outcome source class.')
  ...
  if (/^(?:model|council|llm|consensus|frontier_teacher):/i.test(sourceRef)) throw new Error('Model/Council output cannot be an objective outcome source.')
  const kind = safeText(input?.correlation?.kind, 80) as CouncilCorrelationKind
  if (!(COUNCIL_CORRELATION_KINDS as readonly string[]).includes(kind)) throw new Error('Unsupported Council objective-outcome correlation kind.')
  ...
  if (!['success', 'failure', 'observed'].includes(outcomeStatus)) throw new Error('Unsupported Council objective outcome status.')
  const summary = safeText(input?.summary, 4000)
  if (!summary) throw new Error('Council objective outcome summary is required.')
  return { sourceClass, sourceRef, correlation: { kind, value }, outcomeStatus, summary, facts: input?.facts && typeof input.facts === 'object' && !Array.isArray(input.facts) ? input.facts : {} }
}
...
export function boundedObjectiveFacts(result: unknown): Record<string, unknown> {
  ...
  if (!row) return {}
  const allowedKeys = ['verified','changed','status','state','verificationStatus','verification_status','policyInstanceId','previousIntervalSeconds','currentIntervalSeconds','schedulerSchedule','nativeMonitoringIntervalSeconds','deploymentId','deploymentUrl','healthy','ok']
  const facts: Record<string, unknown> = {}
  for (const key of allowedKeys) { const safe = safePrimitive(row[key]); if (safe !== undefined) facts[key] = safe }
  const verification = objectRecord(row.verification)
  if (verification) {
    const nested: Record<string, unknown> = {}
    for (const key of ['verified','status','state','deploymentId','deploymentUrl','healthy','ok']) { const safe = safePrimitive(verification[key]); if (safe !== undefined) nested[key] = safe }
...
export function classifyDeterministicToolOutcome(input: { ok: boolean; result?: unknown; error?: string | null }): { status: CouncilObjectiveOutcomeStatus; summary: string; facts: Record<string, unknown> } {
  const facts = boundedObjectiveFacts(input.result)
  const verified = explicitVerificationState(input.result)
  if (!input.ok) return { status: 'failure', summary: safeText(input.error || 'The governed deterministic tool execution failed.', 1200), facts }
  if (verified === true) return { status: 'success', summary: 'The governed deterministic tool reported a successful verification/read-back.', facts }
  if (verified === false) return { status: 'failure', summary: 'The governed deterministic tool explicitly reported verification failure.', facts }
  return { status: 'observed', summary: 'The governed tool execution completed, but its result did not contain an explicit verification predicate.', facts }
}
