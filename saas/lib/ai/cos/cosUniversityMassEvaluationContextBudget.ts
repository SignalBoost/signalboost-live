...
// HTTP 400). The output budget now fits the window using a deliberately conservative prompt estimate,
// without adding endpoint calls (the approval allows exactly one call per suite and model).
...
export function massEvaluationOutputTokens(caseCount: number, userPrompt: string): number {
  const estimatedPromptTokens=Math.ceil((MASS_EVALUATION_SYSTEM_PROMPT.length+userPrompt.length)/3)+128
