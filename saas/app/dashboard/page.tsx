...
export default function DashboardOverviewPage() {
  ...
  const conciergeMessage = useMemo(() => {
    if (!projectsLoaded) {
      return {
        icon: '🛰️',
        title: t(dict, 'dash.concierge.title'),
        message: t(dict, 'dash.concierge.loading')
      }
    }
    ...
    if (projects.length === 0) {
      return {
        icon: '👋',
        title: t(dict, 'dash.concierge.title'),
        message: t(dict, 'dash.concierge.welcome')
      }
    }
    ...
    if (draftProjects.length > 0) {
      return {
        ...
        message: draftProjects.length > 1
          ? t(dict, 'dash.concierge.drafts.many').replace('{count}', String(draftProjects.length))
          : t(dict, 'dash.concierge.drafts.one')
      }
    }
    ...
    if (liveProjects.length > 0) {
      return {
        ...
        message: liveProjects.length > 1
          ? t(dict, 'dash.concierge.live.many').replace('{count}', String(liveProjects.length))
          : t(dict, 'dash.concierge.live.one')
      }
    }
    ...
    return {
      icon: '💡',
      title: t(dict, 'dash.concierge.title'),
      message: t(dict, 'dash.concierge.help')
    }
  ...
  const QUICK_ACTIONS = [
    ...
    { type: 'review' as const, icon: '⭐', label: t(dict, 'dash.actions.review.label'), subline: t(dict, 'dash.actions.review.subline'), href: '/dashboard/reviews' },
    { type: 'podcast' as const, icon: '🎙️', label: t(dict, 'dash.actions.podcast.label'), subline: t(dict, 'dash.actions.podcast.subline'), href: '/dashboard/audio' },
    { type: 'video' as const, icon: '🎬', label: t(dict, 'dash.actions.video.label'), subline: t(dict, 'dash.actions.video.subline'), href: '/dashboard/video' },
  ]

  const NEW_USER_PROMPTS = [
    t(dict, "dash.prompts.new.p1"),
    t(dict, "dash.prompts.new.p2"),
    t(dict, "dash.prompts.new.p3"),
    t(dict, "dash.prompts.new.p4"),
  ]

  const RETURNING_PROMPTS = [
    t(dict, "dash.prompts.returning.p1"),
    t(dict, "dash.prompts.returning.p2"),
    t(dict, "dash.prompts.returning.p3"),
    t(dict, "dash.prompts.returning.p4"),
  ]
  ...
  async function sendPrompt(text?: string) {
    ...
    try {
      ...
      setPromptMessages(prev => [
        ...
        {
          role: 'assistant',
          content: data.reply || t(dict, 'dash.connectionError'),
        },
      ])
      ...
      setPromptMessages(prev => [
        ...
        {
          role: 'assistant',
          content: t(dict, 'dash.connectionError'),
        },
      ])
    }
  ...
  function timeAgo(date: string) {
    ...
    if (days > 0) return `${days}${t(dict, 'dash.time.daysShort', 'd')} ${t(dict, 'dash.time.ago')}`
    if (hours > 0) return `${hours}${t(dict, 'dash.time.hoursShort', 'h')} ${t(dict, 'dash.time.ago')}`
    if (mins > 0) return `${mins}${t(dict, 'dash.time.minsShort', 'm')} ${t(dict, 'dash.time.ago')}`

    return t(dict, 'dash.time.justNow')
  }
  ...
  const projectsTitle = firstName
    ? t(dict, 'dash.projectsTitleNamed').replace('{name}', firstName)
    : t(dict, 'dash.projectsTitle')
  ...
  const L = {
    quickActions: t(dict, 'dash.preview.quickActions'),
    yourProjects: t(dict, 'dash.preview.yourProjects'),
    createProject: t(dict, 'dash.preview.createProject'),
    openProject: t(dict, 'dash.preview.openProject'),
    updated: t(dict, 'dash.preview.updated'),
    execute: t(dict, 'dash.preview.execute'),
    thinking: t(dict, 'dash.preview.thinking'),
    thinkingMsg: t(dict, 'dash.preview.thinkingMsg'),
    livePreview: t(dict, 'dash.preview.label'),
    close: t(dict, 'dash.preview.close'),
    openInBuilder: t(dict, 'dash.preview.openInBuilder'),
    upgrade: t(dict, 'dash.upgrade'),
    team: t(dict, 'dash.team'),
    loadingTeam: t(dict, 'dash.loadingTeam'),
    noProjects: t(dict, 'dash.noProjects'),
    noProjectsSub: t(dict, 'dash.noProjectsSub'),
    feedback: t(dict, 'dash.feedback'),
    headlineFallback: t(dict, 'dash.sketch.headlineFallback'),
  }
return (
      ...
      <div style={{ opacity: showLoginGate ? 0.2 : 1, pointerEvents: showLoginGate ? 'none' : 'auto', filter: showLoginGate ? 'blur(2px)' : 'none', transition: 'all 0.3s' }}>
        ...
        <div style={{ display: 'flex', gap: 16, marginBottom: 28, alignItems: 'flex-start', flexWrap: 'wrap' }}>
          ...
          >
            ...
            <button
              onClick={() => { setPromptOpen(true); setPromptInput(t(dict, 'dash.prompt.nextAction')) }}
              className="terminal-text"
            ...
            >
              {termCase(t(dict, 'dash.concierge.askNext'), lang)}
            </button>
          </div>
          ...
          <div className="fathom-glass" style={{ flex: sketch ? '1 1 380px' : '1 1 100%', minWidth: 320, borderRadius: 16, padding: 24 }}>
            ...
            <div style={{ display: 'flex', gap: 10, marginBottom: promptOpen ? 16 : 0 }}>
              ...
              <input
                ...
                placeholder={t(dict, 'dash.askPlaceholder')}
                className="terminal-text"
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12, marginBottom: 28 }}>
          {[
            { label: t(dict, 'dash.stats.activeSites'), value: projects.filter(p => p.status === 'live').length },
            { label: t(dict, 'dash.stats.projects'), value: `${projects.length}/${projectLimit === 999 ? '∞' : projectLimit}` },
            { label: t(dict, 'dash.stats.audioGenerated'), value: `0 ${t(dict, 'dash.stats.min')}` },
            { label: t(dict, 'dash.stats.videosCreated'), value: '0' },
          ].map(stat => (
