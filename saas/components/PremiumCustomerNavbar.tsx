...
type NavGroup = { id: string; label: string; eyebrow?: string; accent: string; items: NavItem[]; width?: number; align?: 'left' | 'right' }
...
const PLAN_LABEL: Record<string, string> = {
  free: 'Free Demo', demo: 'Free Demo', starter: 'Launch', launch: 'Launch',
  pro: 'Growth', growth: 'Growth', business: 'Command', command: 'Command',
}
...
const CUSTOMER_GROUPS: NavGroup[] = [
  {
    id: 'console', label: 'Console Hub', eyebrow: 'Client command', accent: GOLD, width: 390,
    items: [
  ...
  {
    id: 'audit', label: 'Audit Cockpit', eyebrow: 'Readiness reports', accent: '#a78bfa', width: 410,
    items: [
  ...
  {
    id: 'cyber', label: 'Cybersecurity', eyebrow: 'Monitoring & fixes', accent: CYAN, width: 410,
    items: [
...
const OWNER_GROUP: NavGroup = {
  id: 'owner', label: 'Owner/Admin', eyebrow: 'Internal control', accent: RED, align: 'right', width: 420,
  items: [
...
export default function PremiumCustomerNavbar() {
  ...
  useEffect(() => {
    ...
    function draw(timestamp: number) {
      ...
      if (!last || timestamp - last > 2000) { rings.push({ r: 0, alpha: 1 }); last = timestamp }
  ...
  const groupActive = (items: NavItem[]) => items.some((item) => item.href !== '/' && (pathname === item.href || pathname?.startsWith(`${item.href}/`)))
  ...
  function Group({ group }: { group: NavGroup }) {
    ...
    const dropdownPosition: CSSProperties = group.align === 'right' ? { right: 0 } : { left: 0 }
    return (
      <div style={{ position: 'relative' }} onMouseEnter={() => openNow(group.id)} onMouseLeave={closeSoon}>
        ...
        <div
          style={{ position: 'absolute', top: '100%', ...dropdownPosition, paddingTop: 12, opacity: open ? 1 : 0, visibility: open ? 'visible' : 'hidden', transform: open ? 'translateY(0)' : 'translateY(8px)', pointerEvents: open ? 'auto' : 'none', transition: 'opacity .18s ease, transform .18s ease, visibility .18s', zIndex: 220 }}
        ...
        >
          <div style={{ width: group.width || 360, maxWidth: '92vw', position: 'relative', overflow: 'hidden', borderRadius: 22, border: `1px solid ${group.accent}33`, background: 'linear-gradient(145deg, rgba(3,7,18,.98), rgba(15,23,42,.96))', boxShadow: `0 28px 90px rgba(0,0,0,.58), 0 0 45px ${group.accent}12`, backdropFilter: 'blur(18px)' }}>
            ...
            <div style={{ padding: '15px 15px 8px' }}>
              <div style={{ color: group.accent, fontSize: 10, fontWeight: 900, letterSpacing: '.16em', textTransform: 'uppercase', marginBottom: 8 }}>{group.eyebrow || 'SignalBoost'}</div>
      ...
      <nav ref={navRef} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '11px 22px', background: 'linear-gradient(135deg, rgba(4,8,18,.92), rgba(15,23,42,.76))', borderBottom: '1px solid rgba(26,240,255,.18)', boxShadow: '0 18px 60px rgba(0,0,0,.30), inset 0 1px 0 rgba(255,255,255,.08)', position: 'sticky', top: 0, zIndex: 100, backdropFilter: 'blur(14px)' }}>
        <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none', flexShrink: 0, marginRight: 8 }}>
          ...
          <span style={{ display: 'grid', lineHeight: 1.05 }}>
            ...
            <span style={{ color: 'rgba(148,163,184,.72)', fontSize: 10, letterSpacing: '.12em', textTransform: 'uppercase' }}>Client suite</span>
          </span>
        </Link>
