...
test('mass distillation requires confidence, identity and explicit rights', () => {
  assert.equal(retainedIdentityEligibleForMassDistillation({
    contentHash: h(1), subject: 'Machine Learning', sourceKind: 'scientific_journal', license: 'public domain', confidence: 0.91,
  }), true)
  assert.equal(retainedIdentityEligibleForMassDistillation({
    contentHash: h(2), subject: 'Machine Learning', sourceKind: 'scientific_journal', license: 'public domain', confidence: 0.79,
  }), false)
  assert.equal(retainedIdentityEligibleForMassDistillation({
    contentHash: h(3), subject: 'Machine Learning', sourceKind: 'video_transcript', license: 'YouTube caption track fetched in full for COS learning; source URL retained', confidence: 0.99,
  }), false)
})
...
test('mass distillation batches are bounded, deterministic and do not reuse assigned hashes', () => {
  const rows = Array.from({ length: MASS_DISTILLATION_MAX_BATCH + 25 }, (_, index) => ({
    ...
    subject: 'Reasoning & Decision Science',
    sourceKind: 'scientific_journal',
    license: index % 2 ? 'Public Domain' : 'OpenAlex CC0 abstract read for grounded learning',
    confidence: 0.9,
  }))
