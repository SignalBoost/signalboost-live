...
const decision=(overrides:Partial<EnterpriseDecisionSnapshot>={}):EnterpriseDecisionSnapshot=>({schemaVersion:EAE_DECISION_SCHEMA_VERSION,decisionId:'decision-1',tenant,selectedCandidateId:'candidate-1',rankedCandidateIds:['candidate-1'],disposition:'proceed',reasons:['highest_ranked_eligible_candidate'],evidenceRefs:['decision-evidence'],truncated:false,...overrides});
const step=(overrides:Partial<ExecutionStepCandidate>={}):ExecutionStepCandidate=>({stepId:'step-1',tenant,candidateId:'candidate-1',capabilityId:'content.publish',action:'prepare',dependsOn:[],riskLevel:'medium',requiresHumanApproval:false,rollbackAction:'undo-prepare',contingencyAction:'manual-prepare',evidenceRefs:['step-evidence'],...overrides});

const request=(steps:readonly ExecutionStepCandidate[],d=decision())=>({tenant,decision:d,steps,maxSteps:16,maxDepth:8});

test('builds deterministic immutable non-executable plans',()=>{const a=buildEnterpriseExecutionPlan(request([step()]));const b=buildEnterpriseExecutionPlan(request([step()]));assert.equal(a.planId,b.planId);assert.equal(a.executable,false);assert.ok(Object.isFrozen(a));assert.ok(Object.isFrozen(a.steps[0]));});
