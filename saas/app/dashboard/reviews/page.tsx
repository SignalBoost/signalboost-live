...
const COPY: Record<string, Record<Lang, string>> = {
  ...
  subtitle:         { en: 'Collect multilingual reviews, approve what publishes, monitor sentiment, and launch testimonial campaigns from one neon cockpit.', es: 'Recopila reseñas multilingües, aprueba lo que se publica, monitorea el sentimiento y lanza campañas de testimonios.', pt: 'Colete avaliações multilíngues, aprove o que publica, monitore sentimentos e lance campanhas de depoimentos.', pl: 'Zbieraj wielojęzyczne opinie, zatwierdzaj publikacje, monitoruj nastroje i uruchamiaj kampanie testimonialowe.', ru: 'Собирайте многоязычные отзывы, одобряйте публикации, отслеживайте настроения и запускайте кампании.' },
  ...
  adminCta:         { en: 'View admin telemetry', es: 'Ver telemetría de administrador', pt: 'Ver telemetria do administrador', pl: 'Zobacz telemetrię administratora', ru: 'Просмотр телеметрии' },
  ...
  sendLinkDesc:     { en: 'They click it, leave a review in their own language, attach images, and it appears as Pending until approval.', es: 'Hacen clic, dejan una reseña en su idioma, adjuntan imágenes y aparece como Pendiente hasta la aprobación.', pt: 'Eles clicam, deixam uma avaliação no seu idioma, anexam imagens e aparece como Pendente até aprovação.', pl: 'Klikają, zostawiają opinię w swoim języku, dołączają zdjęcia i pojawia się jako Oczekująca do zatwierdzenia.', ru: 'Они нажимают, оставляют отзыв на своём языке, прикладывают фото — он появляется как Ожидающий.' },
  loading:          { en: 'Loading…', es: 'Cargando…', pt: 'Carregando…', pl: 'Ładowanie…', ru: 'Загрузка…' },
  pickHandleDesc:   { en: 'Pick a handle for your review link. It can be your name, your business, anything — 3 to 30 lowercase letters, digits, and hyphens.', es: 'Elige un identificador para tu enlace. Puede ser tu nombre, tu negocio, cualquier cosa — 3 a 30 letras minúsculas, dígitos y guiones.', pt: 'Escolha um identificador para seu link. Pode ser seu nome, seu negócio, qualquer coisa — 3 a 30 letras minúsculas, dígitos e hífens.', pl: 'Wybierz identyfikator dla swojego linku. Może to być twoje imię, firma, cokolwiek — 3 do 30 małych liter, cyfr i myślników.', ru: 'Выберите идентификатор для своей ссылки. Это может быть ваше имя, бизнес, что угодно — 3–30 строчных букв, цифр и дефисов.' },
  ...
  previewText:      { en: 'Customer text keeps its original language. Dates and currency render per locale:', es: 'El texto del cliente mantiene su idioma original. Las fechas y monedas se muestran por región:', pt: 'O texto do cliente mantém seu idioma original. Datas e moedas são exibidas por localidade:', pl: 'Tekst klienta zachowuje oryginalny język. Daty i waluty wyświetlane są według regionu:', ru: 'Текст клиента сохраняет оригинальный язык. Даты и валюты отображаются по локали:' },
  ...
  conciergeBody:    { en: 'Concierge answers review questions in the selected language, suggests moderation actions, and proposes campaigns when sentiment is positive.', es: 'El Concierge responde preguntas de reseñas en el idioma seleccionado, sugiere acciones de moderación y propone campañas cuando el sentimiento es positivo.', pt: 'O Concierge responde perguntas de avaliações no idioma selecionado, sugere ações de moderação e propõe campanhas quando o sentimento é positivo.', pl: 'Concierge odpowiada na pytania dotyczące opinii w wybranym języku, sugeruje działania moderacyjne i proponuje kampanie gdy nastroje są pozytywne.', ru: 'Консьерж отвечает на вопросы об отзывах на выбранном языке, предлагает действия модерации и кампании при позитивных настроениях.' },
  noCampaigns:      { en: 'Positive reviews will trigger testimonial campaign ideas here.', es: 'Las reseñas positivas activarán ideas de campaña de testimonios aquí.', pt: 'Avaliações positivas ativarão ideias de campanha de depoimentos aqui.', pl: 'Pozytywne opinie wyzwolą tutaj pomysły na kampanie testimonialowe.', ru: 'Положительные отзывы вызовут здесь идеи для кампаний.' },
  ...
  sortDate:         { en: 'Sort: newest', es: 'Ordenar: más reciente', pt: 'Ordenar: mais recente', pl: 'Sortuj: najnowsze', ru: 'Сортировка: новейшие' },
  ...
  telemetryTitle:   { en: 'Reviews telemetry, sentiment, and moderation', es: 'Telemetría de reseñas, sentimiento y moderación', pt: 'Telemetria de avaliações, sentimento e moderação', pl: 'Telemetria opinii, nastroje i moderacja', ru: 'Телеметрия отзывов, настроения и модерация' },
  logsEnabled:      { en: 'Submissions · AI sentiment · moderation logged', es: 'Envíos · Sentimiento IA · moderación registrada', pt: 'Envios · Sentimento IA · moderação registrada', pl: 'Zgłoszenia · Nastroje AI · moderacja zarejestrowana', ru: 'Отправки · Настроения ИИ · модерация записана' },
  localeVolume:     { en: 'Review volume per locale', es: 'Volumen de reseñas por región', pt: 'Volume de avaliações por região', pl: 'Wolumen opinii według regionu', ru: 'Объём отзывов по регионам' },
  ...
  crmRegression:    { en: 'Regression guard: approved positive reviews can attach to Leads → Opportunities → Conversions.', es: 'Guardia de regresión: las reseñas positivas aprobadas pueden adjuntarse a Leads → Oportunidades → Conversiones.', pt: 'Guarda de regressão: avaliações positivas aprovadas podem ser anexadas a Leads → Oportunidades → Conversões.', pl: 'Ochrona regresji: zatwierdzone pozytywne opinie mogą być dołączone do Leadów → Szans → Konwersji.', ru: 'Защита регрессии: одобренные положительные отзывы могут прикрепляться к Лидам → Возможностям → Конверсиям.' },
  errSignIn:        { en: 'Please sign in to see your reviews.', es: 'Por favor inicia sesión para ver tus reseñas.', pt: 'Por favor faça login para ver suas avaliações.', pl: 'Zaloguj się, aby zobaczyć swoje opinie.', ru: 'Пожалуйста, войдите, чтобы увидеть ваши отзывы.' },
  ...
  errPickHandle:    { en: 'Pick a handle to continue.', es: 'Elige un identificador para continuar.', pt: 'Escolha um identificador para continuar.', pl: 'Wybierz identyfikator, aby kontynuować.', ru: 'Выберите идентификатор для продолжения.' },
...
export default function ReviewsPage() {
  ...
  const loadReviews = useCallback(async () => {
    ...
    try {
      ...
      if (res.status === 401) {
        setReviewsError(c('errSignIn', l))
        setReviews([])
        return
      }
      const j = await res.json()
      if (!res.ok) {
        setReviewsError(j?.error || c('errLoad', l))
        setReviews([])
        return
      }
      ...
      <section className="sb-reviews-hero" aria-label="Reviews module">
        <div>
          ...
          <div className="sb-review-hero-actions">
            ...
            <a href="#admin-console" className="sb-button-secondary">{c('adminCta', l)}</a>
          </div>
        </div>
