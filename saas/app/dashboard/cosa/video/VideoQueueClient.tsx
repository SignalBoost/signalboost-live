  ...
  return (
    <main style={{ maxWidth: 1240, margin: '0 auto', display: 'grid', gap: 18 }}>
      <section style={heroCard}>
        ...
        <p style={{ color: 'rgba(255,255,255,.68)', lineHeight: 1.7, maxWidth: 920 }}>{copy.intro}</p>
      ...
      <section style={gridSection}>
        ...
        <ConceptSummary item={selected} copy={copy} onMove={move} />
      </section>
    </main>
  )
}

function Workflow({ copy }: { copy: ReturnType<typeof getVideoQueueCopy> }) {
  return (
    <section style={workflowCard}>
      <div style={stepCard}><b>1</b><span>{copy.metricNeeds}</span></div>
      <div style={stepCard}><b>2</b><span>{copy.approveRender}</span></div>
      <div style={stepCard}><b>3</b><span>{copy.bucketProgress}</span></div>
      <div style={stepCard}><b>4</b><span>{copy.bucketPublished}</span></div>
    </section>
  )
}
  ...
  return (
    <section style={panel}>
      ...
      <div style={{ display: 'grid', gap: 10, marginTop: 12 }}>
        {items.length === 0 && <p style={{ color: 'rgba(255,255,255,.5)' }}>{empty}</p>}
        {items.map(item => <Card key={item.id} item={item} active={item.id === selectedId} copy={copy} onPreview={onPreview} onMove={onMove} />)}
      </div>
    </section>
  )
}
...
function Card({ item, active, copy, onPreview, onMove }: { item: Item; active: boolean; copy: ReturnType<typeof getVideoQueueCopy>; onPreview: (id: string) => void; onMove: (id: string, status: Status) => void }) {
  return (
    <article style={{ ...card, border: active ? '1px solid rgba(255,195,0,.55)' : card.border }}>
      <button onClick={() => onPreview(item.id)} style={openButton}>
        <p style={{ color: 'rgba(255,255,255,.52)', fontSize: 12, margin: 0 }}>{item.aspect} · {item.duration}</p>
        <h3 style={{ color: '#fff', margin: '6px 0 0', fontSize: 18 }}>{item.title}</h3>
        <p style={{ color: GOLD, margin: '8px 0 0', fontWeight: 850 }}>{copy.hookLabel} — “{item.hook}”</p>
        <p style={{ color: 'rgba(255,255,255,.56)', margin: '6px 0 0' }}>{copy.nicheLabel}: {item.niche} · {copy.formatLabel}: {item.format} · {copy.heroLabel}: {item.hero} · {copy.qualityLabel} {item.quality}</p>
        <p style={{ color: 'rgba(255,255,255,.44)', margin: '8px 0 0', fontSize: 12 }}>{item.progress || statusText(item.status, copy)}</p>
      </button>
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
        <button onClick={() => onPreview(item.id)} style={secondary}>{copy.preview}</button>
        {item.status === 'needs_approval' && <button onClick={() => onMove(item.id, 'rejected')} style={secondary}>{copy.reject}</button>}
        {item.status === 'needs_approval' && <button onClick={() => onMove(item.id, 'in_progress')} style={primary}>{copy.approve}</button>}
      </div>
    </article>
  )
}

function ConceptSummary({ item, copy, onMove }: { item: Item; copy: ReturnType<typeof getVideoQueueCopy>; onMove: (id: string, status: Status) => void }) {
  return (
    <aside style={summaryCard}>
      <p className="sb-eyebrow" style={{ margin: 0 }}>{copy.playablePreview}</p>
      <h2 style={{ color: '#fff', margin: '8px 0 0', fontSize: 24 }}>{item.title}</h2>
      <div style={summaryBox}>
        <p style={{ color: GOLD, fontWeight: 950, margin: 0 }}>{copy.hookLabel}</p>
        <p style={{ color: '#fff', fontSize: 20, lineHeight: 1.3, margin: '8px 0 0' }}>“{item.hook}”</p>
        <p style={{ color: 'rgba(255,255,255,.62)', margin: '12px 0 0' }}>{copy.nicheLabel}: {item.niche} · {copy.formatLabel}: {item.format} · {copy.heroLabel}: {item.hero}</p>
      </div>
      <div style={{ display: 'grid', gap: 8, marginTop: 14 }}>
        {item.scenes.map((scene, index) => <div key={scene} style={sceneRow}>{index + 1}. {scene}</div>)}
      </div>
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 16, flexWrap: 'wrap' }}>
        {item.status === 'needs_approval' && <button onClick={() => onMove(item.id, 'rejected')} style={secondary}>{copy.reject}</button>}
        {item.status === 'needs_approval' && <button onClick={() => onMove(item.id, 'in_progress')} style={primary}>{copy.approveRender}</button>}
      </div>
    </aside>
  )
}
...
const workflowCard: React.CSSProperties = { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 10 }
...
const gridSection: React.CSSProperties = { display: 'grid', gridTemplateColumns: 'minmax(320px,1fr) minmax(340px,.9fr)', gap: 18, alignItems: 'start' }
...
const summaryCard: React.CSSProperties = { position: 'sticky', top: 96, border: '1px solid rgba(255,195,0,.25)', borderRadius: 22, padding: 18, background: 'linear-gradient(145deg, rgba(255,195,0,.08), rgba(15,23,42,.86))', boxShadow: '0 22px 70px rgba(0,0,0,.34)' }
const summaryBox: React.CSSProperties = { marginTop: 14, border: '1px solid rgba(255,255,255,.09)', borderRadius: 16, padding: 14, background: 'rgba(0,0,0,.18)' }
