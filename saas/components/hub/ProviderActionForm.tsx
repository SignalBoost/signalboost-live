...
export default function ProviderActionForm({ templateId, lang, onSuccess, onError, onClose }: ProviderActionFormProps) {
  ...
  return (
    <div style={{ ...cardStyle, maxHeight: '85vh', height: '100%' }}>
      {/* Header Band */}
      ...
      <div style={{ ...bodyStyle, gap: 16, flex: 1, minHeight: 0, overflow: 'auto' }}>
        {state === 'idle' && (
          <>
...
function ResultView({ data }: { data: any }) {
  ...
  if (arrayKey) {
    ...
    const first = rows[0]
    // Array of objects -> structured table
    if (first && typeof first === 'object' && !Array.isArray(first)) {
      ...
      return (
        <div style={{ flex: 1, overflow: 'auto', minHeight: 0, border: '1px solid rgba(255,255,255,.1)', borderRadius: 10 }}>
          <table style={{ width: '100%', tableLayout: 'fixed', borderCollapse: 'collapse', fontSize: 11 }}>
            <thead>
              <tr>
                {cols.map(col => (
                  <th key={col} style={{ textAlign: 'left', padding: '8px 10px', position: 'sticky', top: 0, background: 'rgba(8,11,20,.98)', color: '#1af0ff', fontWeight: 700, fontSize: 10, letterSpacing: '.03em', textTransform: 'uppercase', borderBottom: '1px solid rgba(255,255,255,.12)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                    {col}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {rows.slice(0, 100).map((row: any, i: number) => (
                <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,.05)', background: i % 2 ? 'rgba(255,255,255,.02)' : 'transparent' }}>
                  {cols.map(col => (
                    <td key={col} title={formatCell(row?.[col])} style={{ padding: '7px 10px', color: col === 'name' || col === 'label' ? '#fff' : 'rgba(26,240,255,.82)', fontWeight: col === 'name' || col === 'label' ? 700 : 400, fontFamily: col === 'name' || col === 'label' ? 'inherit' : 'ui-monospace, SFMono-Regular, Menlo, monospace', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                      {formatCell(row?.[col])}
                    </td>
                  ))}
                </tr>
