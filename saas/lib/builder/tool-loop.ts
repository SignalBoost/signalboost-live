...
function parse(value: string | null): Action | null {
  try {
    ...
    if (parsed?.type === 'tool' && tools.includes(parsed.toolId) && parsed.input && typeof parsed.input === 'object' && !Array.isArray(parsed.input)) {
      return { type: 'tool', toolId: parsed.toolId, input: parsed.input }
    }
  } catch {}
  return null
}

function summarize(file: { path: string; content: string; updatedAt: number }) {
  return { path: file.path, bytes: new TextEncoder().encode(file.content).byteLength, updatedAt: file.updatedAt }
}
function summarizeRun(result: BuilderRunResult) {
  return { exitCode: result.exitCode, stdout: result.stdout.slice(0, 16_000), stderr: result.stderr.slice(0, 16_000), timedOut: result.timedOut }
}
  ...
  async run(input: { objective: string; workspaceId: string; maxRounds?: number }): Promise<BuilderLoopResult> {
    ...
    for (let round = 1; round <= maxRounds; round += 1) {
      const response = await this.ai.generate({
        systemPrompt: `You are COS Builder. Work only inside the supplied user workspace. Use tools to inspect, edit and run code. Prefer run after one successful read. You have at most ${MAX_WRITES_PER_TURN} file writes/edits and ${MAX_RUNS_PER_TURN} command runs. Never claim a file was changed or code ran unless the tool result in this turn proves it. Never access host files, secrets, repositories, networks, deployments or credentials. Return exactly one JSON control object.`,
        prompt: [
          `OBJECTIVE:\n${input.objective}`,
          `TOOLS: ${safeJson(tools)}`,
          trace.length ? `RESULTS:\n${safeJson(trace)}` : '',
          'Use: {"type":"tool","toolId":"run","input":{"command":"python3 main.py"}}',
      ...
      if ((action.toolId === 'write_file' || action.toolId === 'edit_file') && writeCount >= MAX_WRITES_PER_TURN) {
        return { ok: false, error: 'builder_write_budget_exhausted', trace }
      }
      if (action.toolId === 'run' && runCount >= MAX_RUNS_PER_TURN) {
        return { ok: false, error: 'builder_run_budget_exhausted', trace }
      }
      const fingerprint = `${action.toolId}:${safeJson(action.input)}`
      if (seen.has(fingerprint) && action.toolId !== 'list_files' && action.toolId !== 'read_file') {
        return { ok: false, error: `builder_repeated_tool_call:${action.toolId}`, trace }
      }
      ...
      try {
        ...
        if (action.toolId === 'run') {
          ...
          output = summarizeRun(await this.runner.run({
            workspaceId: input.workspaceId,
            command: text(action.input.command),
            files: files.filter((file): file is BuilderFile => file !== null),
          }))
        }
        ...
        trace.push({
          ...
          input: action.input,
          ok: false,
          error: error instanceof Error ? error.message : 'builder_tool_failed',
        })
      }
    }
