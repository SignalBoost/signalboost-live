...
export type CosCurriculumTrack = { id:'cyber_defense'|'software_engineering'|'agent_systems'|'ml_data_engineering'|'ai_systems_safety'|'cognitive_science'|'robotics_edge_ai'; title:string; topics:string[]; evaluation:string[]; safetyBoundary:string }
...
export function coreCurriculumTrackForSubject(subject:string):CosCurriculumTrack|null{
 ...
 if(/robot|drone|edge ai|lidar|thermal|multispectral|sensor fusion|bvlos|aerial/.test(value))return COS_CORE_CURRICULUM_TRACKS[4]
 if(/cognitive|psycholog|behavioral economics|decision bias|attention|memory|human.agent|accessibility|cognitive load/.test(value))return COS_CORE_CURRICULUM_TRACKS[5]
