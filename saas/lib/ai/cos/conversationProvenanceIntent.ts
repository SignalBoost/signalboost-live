...
const EN_ORIGIN =
  /(?<![\p{L}\p{N}_])(?:where\s+(?:did|do)\s+(?:you|u)\s+(?:get|find|take|pull|source)|where\s+(?:did|does)\s+(?:this|that|it|the\s+(?:idea|concept|scenario|answer|script|text|story|dialogue|joke|response|reply|draft))\s+(?:come|came)\s+from|where\s+(?:did|does)\s+the\s+(?:idea|concept|scenario|script|text|story|dialogue|draft)\s+(?:come|came)\s+from|where(?:'s|\s+is)\s+(?:this|that|it)\s+from|what\s+(?:is|are|was|were)\s+(?:this|that|it|the\s+(?:idea|concept|scenario|answer|script|text|story|dialogue))\s+based\s+on|how\s+did\s+you\s+come\s+up\s+with|what\s+inspired\s+(?:this|that|the\s+(?:idea|concept|scenario|script|story|dialogue))|did\s+you\s+(?:make|made)\s+(?:this|that|it)\s+up|did\s+you\s+(?:invent|imagine)\s+(?:this|that|it)|what\s+(?:is|was)\s+your\s+source|what\s+sources?\s+did\s+you\s+use|who\s+told\s+you)(?![\p{L}\p{N}_])/iu

const ES_ORIGIN =
  /(?<![\p{L}\p{N}_])(?:de\s+d[oó]nde\s+(?:lo\s+|la\s+|)?(?:sacaste|saca[sr]?|sac[oó]|obtuviste|sali[oó]|viene|vino|sale)|de\s+d[oó]nde\s+(?:vino|sali[oó])\s+(?:esta|esa|la)\s+(?:idea|respuesta|historia|escena|guion)|en\s+qu[eé]\s+(?:te\s+basaste|se\s+bas[oó]|se\s+basa)|c[oó]mo\s+se\s+te\s+ocurri[oó]|qu[eé]\s+inspir[oó]\s+(?:esta|esa|la)\s+idea|cu[aá]l\s+(?:es|fue)\s+tu\s+fuente|te\s+lo\s+inventaste|lo\s+inventaste)(?![\p{L}\p{N}_])/iu

const PT_ORIGIN =
  /(?<![\p{L}\p{N}_])(?:de\s+onde\s+(?:voc[eê]\s+)?(?:tirou|tiraste|tirei|veio|saiu|surgiu)|de\s+onde\s+(?:veio|surgiu)\s+(?:esta|essa|a)\s+(?:ideia|resposta|hist[oó]ria|cena|roteiro)|em\s+que\s+(?:voc[eê]\s+)?se\s+base(?:ou|ia)|no\s+que\s+(?:voc[eê]\s+)?se\s+base(?:ou|ia)|como\s+(?:voc[eê]\s+)?chegou\s+a\s+essa\s+ideia|o\s+que\s+inspirou\s+(?:essa|esta|a)\s+ideia|qual\s+(?:[ée]|foi)\s+a\s+sua\s+fonte|voc[eê]\s+inventou)(?![\p{L}\p{N}_])/iu

const PL_ORIGIN =
  /(?<![\p{L}\p{N}_])(?:sk[ąa]d\s+(?:to\s+|masz\s+)?(?:masz|wzi[ąa][łl]e[śs]|wzi[ęe][łl]a[śs]|pochodzi|wzi[ęe][łl]o|si[ęe])|sk[ąa]d\s+(?:wzi[ęe][łl]a|pochodzi)\s+(?:si[ęe]\s+)?(?:ta|taka)\s+(?:idea|odpowied[zź]|historia|scena)|na\s+czym\s+(?:si[ęe]\s+opiera(?:sz|)|to\s+bazuje)|jak\s+wpad[łl]e[śs]\s+na\s+(?:ten|taki)\s+pomys[łl]|co\s+zainspirowa[łl]o\s+(?:ten|taki)\s+pomys[łl]|jakie\s+(?:jest|by[łl]o)\s+[źz]r[óo]d[łl]o|wymy[śs]li[łl]e[śs]\s+to)(?![\p{L}\p{N}_])/iu
