...
test('evaluation phases are atomically one-shot before any paid readiness or scoring call', () => {
  ...
  assert.match(evaluator, /claimMassDistilledEvaluationPhase\(\{ candidateId, artifactHash, phase: 'initial' \}\)[\s\S]*?if \(!reservation\.claimed\)[\s\S]*?waitForMassDistilledReady\(canary\.endpointId\)[\s\S]*?runSuite\(\{ suiteName: 'holdout'/)
  assert.match(evaluator, /claimMassDistilledEvaluationPhase\(\{ candidateId, artifactHash, phase: 'retention' \}\)[\s\S]*?if \(!reservation\.claimed\)[\s\S]*?waitForMassDistilledReady\(canary\.endpointId\)[\s\S]*?runSuite\(\{ suiteName: 'retention'/)
