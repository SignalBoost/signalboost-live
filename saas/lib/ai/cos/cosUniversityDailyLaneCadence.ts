// saas/lib/ai/cos/cosUniversityDailyLaneCadence.ts
...
export async function readCosUniversityDailyLaneCadence(path: LearningPathId, now = new Date()): Promise<CosUniversityDailyLaneCadence> {
  const db = cosServiceDb()
  // Without the ledger the runner itself will surface service_database_unavailable; never skip silently.
  if (!db) return decideCosUniversityDailyLaneCadence({ path, now, rows: [] })
  ...
  return decideCosUniversityDailyLaneCadence({ path, now, rows: (result.data || []) as CosUniversityDailyLaneReceiptRow[] })
}
