...
test('maps football prompts to grassroots field images when no URLs are provided', () => {
  ...
  assert.match(String(site.sections?.[0]?.image_url), /^\/media\/generated\/football\/football-[a-z0-9-]+\.webp$/)
...
test('still sources imagery when the prompt contains a non-image business URL', () => {
  ...
  assert.match(String(site.sections?.[0]?.image_url), /^\/media\/generated\/food\/food-[a-z0-9-]+\.webp$/)
})
...
test('football prompts inject hero, gallery, and sponsor logo imagery without explicit photo words', () => {
  ...
  assert.match(String(hero?.image_url), /^\/media\/generated\/football\/football-[a-z0-9-]+\.webp$/)
  ...
  assert.ok((logos?.items || []).every(item => String(item.logo_url).startsWith('/media/generated/football/football-')))
})


test('replaces generic generated image paths with generated media asset paths', () => {
  const site = enrichSiteMedia({
  ...
  assert.match(String(site.sections?.[0]?.image_url), /^\/media\/generated\/bakery\/bakery-luxury-bakery-[a-z0-9-]+\.webp$/)
  assert.match(String(site.sections?.[1]?.items?.[0]?.image_url), /^\/media\/generated\/bakery\/bakery-luxury-bakery-[a-z0-9-]+\.webp$/)
  assert.match(String(site.logo_url), /^\/media\/generated\/bakery\/bakery-luxury-bakery-[a-z0-9-]+\.webp$/)
})
