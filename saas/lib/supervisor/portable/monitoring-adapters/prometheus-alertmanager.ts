// saas/lib/supervisor/portable/monitoring-adapters/prometheus-alertmanager.ts
//
// Alertmanager BATCHES. A single webhook routinely carries many alerts in one
...
export function createAlertmanagerDefinition(sourceId: string): IncidentSourceDefinition {
  return {
    sourceId,
    ...
    map(body, delivery) {
      ...
      if (alerts.length === 0) {
        ...
        return mapping({
          provider: String(first(labels.provider, labels.cloud, labels.job, 'prometheus')),
          delivery,
          message: first(annotations.description, annotations.summary, root.message, labels.alertname),
          ...
          affectedResource: first(labels.instance, labels.pod, labels.service, labels.job),
          dedupeKey: first(root.fingerprint, labels.alertname),
          metadata: { adapterId: 'prometheus-alertmanager', adapterMaturity: 'staged' },
        })
      }
