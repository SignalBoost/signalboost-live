...
if (!/^\d+\.\d+\.\d+(-[0-9A-Za-z.-]+)?$/.test(spec.version)) fail(`version "${spec.version}" is not semantic versioning`);
...
while (queue.length) {
  ...
  for (const s of specifiersIn(source)) {
    if (isBuiltin(s)) { builtins.add(s); continue; }
    if (!isRelative(s)) {
      ...
      if (s.startsWith('@/') || s.startsWith('~/')) { unresolved.push({ from: rel, spec: s }); continue; }
      if (!externals.has(s)) externals.set(s, new Set());
      externals.get(s).add(rel);
      continue;
    }
    const target = resolveRelative(abs, s);
    if (!target) { unresolved.push({ from: rel, spec: s }); continue; }
    queue.push(target);
  }
}
...
if (unresolved.length) {
  ...
  fail(`the buyer import graph reaches ${unresolved.length} specifier(s) that cannot be\n  packaged and were not declared in "hostFallbacks". A buyer extracting this\n  archive would get an unresolvable import:\n\n${lines}\n\n  Either make the module host-agnostic, or declare the specifier in the spec's\n  hostFallbacks with an explanation of what the buyer must supply.`);
}
if (spec.minModules && visited.size < spec.minModules) fail(`graph walk reached only ${visited.size} modules but the spec expects at least ${spec.minModules}. A short walk usually means an entry point moved or an import style changed — it must not silently ship a partial payload.`);
...
const payloadRoots = (Array.isArray(spec.payloadRoot) ? spec.payloadRoot : [spec.payloadRoot])
  .map((r) => String(r).replace(/^\.?\/*/, '').replace(/\/+$/, ''))
  .filter((r) => r.length > 0)
  .map((r) => r + '/');
if (payloadRoots.length === 0) fail('payloadRoot must name at least one directory');
const stripRoot = payloadRoots.length === 1;
const files = [];
for (const { rel, source } of [...visited.values()].sort((a, b) => a.rel.localeCompare(b.rel))) {
  const root = payloadRoots.find((r) => rel.startsWith(r));
  if (!root) fail(`graph reached "${rel}", which is outside every declared payloadRoot (${payloadRoots.join(', ')}).\n  Widen payloadRoot only if that module genuinely belongs to this portable.`);
  const buyerRel = stripRoot ? rel.slice(root.length) : rel;
  const dest = path.join(payloadDir, buyerRel);
  fs.mkdirSync(path.dirname(dest), { recursive: true });
  fs.writeFileSync(dest, source);
  files.push({ path: `payload/${buyerRel}`, bytes: Buffer.byteLength(source), sha256: sha256(source) });
}
...
const entryMap = {};
for (const [name, target] of Object.entries(spec.buyerEntryPoints || {})) entryMap[name] = target;
const manifest = {
  schema: 'signalboost.portable-release/1', id: spec.id, name: spec.name || spec.id, version: spec.version,
  description: spec.description || '', license: spec.license || 'SEE LICENSE IN LICENSE', builtAt: new Date().toISOString(),
  sourceCommit: gitCommit(), entryPoints: entryMap, moduleCount: visited.size, files,
  runtime: { nodeBuiltins: [...builtins].sort(), externalDependencies: [...externals.keys()].sort().map((s) => ({ specifier: s, hostFallback: hostFallbacks.has(s), importedBy: [...externals.get(s)].sort() })) },
  buyerMustSupply: spec.buyerMustSupply || [], notIncluded: spec.notIncluded || [], supportedPlatforms: spec.supportedPlatforms || [],
  upgradeCompatibility: spec.upgradeCompatibility || {}, knownLimitations: spec.knownLimitations || [], acceptance: spec.acceptance || {},
  namingExceptions: namingHits.sort().map((p) => ({ path: p, reason: namingExceptionReasons.get(p) || '' })),
};
fs.writeFileSync(path.join(outDir, 'manifest.json'), JSON.stringify(manifest, null, 2) + '\n');
fs.writeFileSync(path.join(outDir, 'SHA256SUMS'), files.map((f) => `${f.sha256}  ${f.path}`).join('\n') + '\n');
const sbom = {
  bomFormat: 'CycloneDX', specVersion: '1.5', version: 1,
  metadata: { timestamp: manifest.builtAt, component: { type: 'library', 'bom-ref': `${spec.id}@${spec.version}`, name: spec.id, version: spec.version, description: manifest.description } },
  components: [...externals.keys()].sort().map((s) => ({ type: 'library', 'bom-ref': s, name: s, scope: hostFallbacks.has(s) ? 'optional' : 'required', description: hostFallbacks.has(s) ? 'Host fallback — not shipped; buyer supplies an implementation or an alternative.' : 'Runtime dependency of the payload.' })),
};
fs.writeFileSync(path.join(outDir, 'sbom.json'), JSON.stringify(sbom, null, 2) + '\n');
fs.writeFileSync(path.join(outDir, 'package.json'), JSON.stringify({ name: spec.packageName || `@portable/${spec.id}`, version: spec.version, description: manifest.description, license: manifest.license, type: 'module', files: ['payload', 'manifest.json', 'SHA256SUMS', 'sbom.json', 'RELEASE-NOTES.md', 'docs'], engines: spec.engines || { node: '>=20' }, peerDependencies: Object.fromEntries([...externals.keys()].filter((s) => !hostFallbacks.has(s)).map((s) => [s, '*'])) }, null, 2) + '\n');
...
if (!NO_ARCHIVE) {
  ...
  const staged = path.join(outDir, '..', tgz);
  fs.rmSync(staged, { force: true });
  execFileSync('tar', ['--sort=name', '--mtime=@0', '--owner=0', '--group=0', '--numeric-owner', '-czf', staged, '-C', outDir, '.'], { cwd: ROOT });
  fs.renameSync(staged, path.join(outDir, tgz));
