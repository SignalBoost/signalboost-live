      ...
      <div className="content">
        <header className="hero">
          ...
          <div className="hero-right">
            ...
            <div className="stats" aria-label="Live platform activity">
              <Stat value={live?.activePortables ?? 0} label="active portables" />
              <Stat value={live?.totalRows ?? 0} label="verified activity rows" />
              <div className="stat"><strong className={`system-${live?.status ?? 'unreachable'}`}>{live ? live.status : 'connecting'}</strong><span>runtime status</span></div>
            </div>
          </div>
        </header>
        ...
        <section className="zone">
          ...
          <div className="grid grid-portables">
            {listPublicPortableProducts().map((p) => {
              ...
              const inner = <><div className="qcard-top"><span className="qcard-icon">{p.glyph}</span><span className={`runtime-tag runtime-${status}`}><i />{runtimeLabel(status)}</span>{maturity ? <span className="maturity-tag" title="Commercial maturity — separate from runtime activity">{copy(`portableMaturity.${p.manifest.status}`, maturity)}</span> : null}</div><h3>{name}</h3><p>{desc}</p><div className="runtime-metrics"><strong>{runtime ? runtime.totalRows.toLocaleString() : '—'}</strong><span>verified rows</span><small>{runtime ? relativeTime(runtime.lastActivityAt) : 'Connecting to live data'}</small></div></>
