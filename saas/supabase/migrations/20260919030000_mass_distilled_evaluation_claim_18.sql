-- saas/supabase/migrations/20260919030000_mass_distilled_evaluation_claim_18.sql
-- Forward repair: align the atomic mass-evaluation claim contract with the 18-call ceiling.
--
-- Why the ceiling moved 14 -> 18: Production 2026-09-19 showed all twelve fixed cases sharing ONE request
-- per model under a 1024-token output cap, which decayed scores by position in the batch - safety (first
-- four) 1.000, transfer 0.625, retention (last four) 0.000, identically on every run. The captured judge
-- response was well-formed and scored the tail zero honestly, so the tail answers were degenerate rather
-- than the judge broken. Each fixed suite now gets its own request per model: 6 calls instead of 2. Without
-- the raise a 13-case holdout would cut the slower candidate from 9 groups to 5, reintroducing the gateway
-- timeouts the previous raise existed to prevent.
--
-- This is a CALL ceiling only. maxJudgeCalls (4), maxRuntimeWakeAttempts (1), the $0.20 wake cost ceiling,
-- the 12-hour retention delay, exact-artifact binding and the Production-traffic prohibition are unchanged.
--
-- The TypeScript constant MASS_EVALUATION_ENDPOINT_CALLS and this assertion must move together: a TS-only
-- change makes every valid approval unclaimable and every tick report no_atomically_claimable.
-- This migration recreates the function rather than mutating historical migration files.
...
begin
  ...
  loop
    ...
    if v_max_endpoint<>18 or v_max_judge<>4 or v_max_wake<>1 or v_max_cost<=0 or v_max_cost>0.200000 then continue; end if;
