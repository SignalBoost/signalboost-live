...
export default async function PortableProductsPage({ searchParams }: { searchParams?: Promise<SearchParams> }) {
  ...
  if (!access.isAdmin) return <main style={page}><h1><LocalizedText fallback={"Portable Product Catalog"} /></h1><p style={muted}>Admin access required.</p></main>
  let filters
  try { filters = parsePortableProductCatalogFilters(toSearchParams(await searchParams ?? {})) } catch { return <main style={page}><h1><LocalizedText fallback={"Portable Product Catalog"} /></h1><p style={muted}>Invalid catalog filters. Use only the listed read-only filters.</p></main> }
  ...
  return <main style={page}>
    ...
    {items.map(item => <article key={item.productId} style={panel}><h2>{item.displayName}</h2><p style={muted}><code>{item.productId}</code> · {item.status} · {item.maturity} · {item.category} · public: {String(item.publicVisible)} · licensing: {String(item.licensingAvailable)}</p><p>{item.shortDescription}</p><dl style={{ display: 'grid', gap: 10 }}><div><dt><LocalizedText fallback={"Supported languages"} /></dt><dd>{values(item.supportedLanguages)}</dd></div><div><dt>Architecture references</dt><dd>{values(item.architectureReferences)}</dd></div><div><dt>Documentation references</dt><dd>{values(item.documentationReferences)}</dd></div><div><dt>Dependencies</dt><dd>{values(item.dependencies)}</dd></div><div><dt>Exclusions</dt><dd>{values(item.exclusions)}</dd></div></dl></article>)}
