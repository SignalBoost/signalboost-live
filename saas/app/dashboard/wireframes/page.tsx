  ...
  return (
    <main className="sb-hmi-shell" role="main" aria-labelledby="wireframe-title">
      <section
        ...
        aria-label={t(dict, 'wireframes.unifiedAria')}
      >
        <aside className="sb-hmi-sidebar" aria-label={t(dict, 'wireframes.sidebarAria')}>
          <Link href="/" className="sb-hmi-brand">{uiText('generatedUi.u_85647deec9865df5')}</Link>
          ...
          <div className="sb-hmi-status" role="status" aria-live="polite">
            <span /> {t(dict, 'wireframes.i18nStatus')}
          </div>
        </aside>

        <section className="sb-hmi-main">
          <header className="sb-hmi-hero">
            <p className="sb-hmi-kicker">{t(dict, 'wireframes.kicker')}</p>

            <h1 id="wireframe-title">
              {t(dict, 'wireframes.title')}
            </h1>
            ...
            <div className="sb-hmi-cta-row">
              <Link href="/pricing" className="sb-button-primary">
                {t(dict, 'wireframes.viewPricing')}
              </Link>

              <a href="#pricing-wireframe" className="sb-button-secondary">
                {t(dict, 'wireframes.jumpPricing')}
              </a>
            </div>
          </header>
          ...
          <section
            ...
            aria-label={t(dict, 'wireframes.telemetryAria')}
          >
            {modules.map((module) => (
              <Link
                key={module.key}
          ...
          <section
            ...
            aria-label={t(dict, 'wireframes.modulesAria')}
          >
            {modules.map((module) => (
              <ModulePreview
                key={module.key}
                module={module}
                openLabel={t(dict, 'wireframes.openDashboard')}
              />
            ))}
          </section>
          ...
          <section className="sb-hmi-preview-gallery" aria-labelledby="preview-gallery-title">
            <div>
              <p className="sb-hmi-kicker">{t(dict, 'wireframes.previewKicker')}</p>
              <h2 id="preview-gallery-title">{t(dict, 'wireframes.previewTitle')}</h2>
            </div>
              ...
              {[
                ...
                <a key={image} href={`/wireframes/${image}.svg`} className="sb-hmi-preview-card">
                  <img
                    ...
                    alt={t(dict, 'wireframes.previewAlt')}
                  />
          ...
          <section id="pricing-wireframe" className="sb-hmi-pricing-preview" aria-labelledby="pricing-wireframe-title">
            <div>
              <p className="sb-hmi-kicker">{t(dict, 'wireframes.pricing.kicker')}</p>

              <h2 id="pricing-wireframe-title">
                {t(dict, 'wireframes.pricing.title')}
              </h2>
            ...
            <div className="sb-hmi-price-grid">
              {pricingPlans.map((plan) => (
                <Link key={plan.key} href={plan.href} className="sb-hmi-price-card">
                  ...
                  <small>{t(dict, 'wireframes.viewPricing')}</small>
                </Link>
              ))}
            </div>
          </section>
