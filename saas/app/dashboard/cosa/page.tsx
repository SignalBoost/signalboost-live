...
function CampaignCard({ campaign, busy, onPatch, onGenerateDraft, onGenerateAll, onPublish, onRenderVideo, onCheckStatus }: { campaign: CampaignRow; busy: boolean; onPatch: (id: string, status: 'approved' | 'rejected' | 'queued') => void; onGenerateDraft: (id: string) => void; onGenerateAll: (id: string) => void; onPublish: (id: string, language?: string) => void; onRenderVideo: (id: string) => void; onCheckStatus: (id: string) => void }) {
  ...
  return <section style={{ ...panel, position: 'relative', overflow: 'hidden' }}>
    ...
    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 14, paddingTop: 12, borderTop: '1px solid rgba(255,255,255,.06)', flexWrap: 'wrap' }}>{waiting && <button disabled={busy} onClick={() => onPatch(campaign.id, 'rejected')} style={secondary}>Reject</button>}{waiting && (!['youtube', 'short_video'].includes(campaign.channel || '') || ((campaign.metadata as any)?.video?.branded === true && (campaign.metadata as any)?.video?.voicedUrl)) && <button disabled={busy} onClick={() => onPatch(campaign.id, 'approved')} style={primary}>Approve campaign</button>}{waiting && ['youtube', 'short_video'].includes(campaign.channel || '') && !((campaign.metadata as any)?.video?.branded === true && (campaign.metadata as any)?.video?.voicedUrl) && <span style={{ color: 'rgba(255,255,255,.55)', fontSize: 12, alignSelf: 'center' }}>Approval locked — preview the final branded video above first (unlocks when the SignalBoostAi banner is burned in).</span>}{!hasDraft(campaign) && <button disabled={busy} onClick={() => onGenerateDraft(campaign.id)} style={primary}>Generate review draft</button>}<button disabled={busy} onClick={() => onGenerateAll(campaign.id)} style={secondary}>Generate all languages</button></div>
  </section>
}
