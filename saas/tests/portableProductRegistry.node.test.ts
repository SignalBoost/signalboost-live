...
test('registry is frozen and preserves stable customer-facing manifest IDs', () => {
  ...
  assert.ok(ids.includes('dynamic-pipeline-router')); assert.ok(ids.includes('agent-operations-platform')); assert.ok(ids.includes('browser-agent-ecosystem')); assert.ok(ids.includes('ai-university')); assert.ok(ids.includes('ai-distillation-engine')); assert.ok(!ids.includes('agentRuntime'))
