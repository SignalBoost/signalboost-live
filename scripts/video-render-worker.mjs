...
// Bare POSIX path with filtergraph-special chars escaped. NEVER pass a
// file:// URL here: the ass filter splits on the colon and tries to parse
// '///path' as original_size (image size), killing the render.
function ffmpegFile(path) { return String(path || '').replace(/\\/g, '/').replace(/([:,'\[\]])/g, '\\$1') }
...
async function processSupabaseJob() { if (!supabase) return false; const { data } = await supabase.from('video_jobs').select('*').eq('status', 'queued').in('job_type', ['export']).order('created_at', { ascending: true }).limit(1).maybeSingle(); if (!data) return false; await supabase.from('video_jobs').update({ status: 'processing' }).eq('id', data.id); try { const result = await renderCaptionBurnJob(data.id, data.queue_payload); await supabase.from('video_jobs').update({ status: 'completed', result_url: result.resultUrl, result_path: result.outputPath, completed_at: new Date().toISOString() }).eq('id', data.id) } catch (error) { await supabase.from('video_jobs').update({ status: 'failed', error: error instanceof Error ? error.message : String(error) }).eq('id', data.id) } return true }
