...
import { NATIVE_LANGUAGE_ANSWER_POLICY } from './conciergeLanguageQuality.ts'

/**
 * Rules governing evidence discipline, native-language output quality, quantitative work,
 * constraint compliance, and what may be asserted. Included verbatim in the owner reasoner
 * prompt and the public stateless prompt before generation.
 */
export const QUANTITATIVE_ANSWER_POLICY: readonly string[] = [
  ...
  '- Open with yes or no only when the question names one operationally unambiguous factual proposition and the evidence supports that exact proposition without implying a stronger claim. If the wording reasonably spans a descriptive difference and a cause, intent, unequal treatment, or legal claim, or if the evidence measures more than one construct, population, comparison basis, or control structure, do not open with yes or no. Map what each measure establishes, name factors that remain in any residual, and state what is not established. A group label in the question may be one candidate factor; it is not the proven cause unless the evidence isolates it. If the requested proposition is not established, identify the missing comparison or evidence instead of substituting a nearby measure.',
  '- For diagnosis and troubleshooting, keep observations, candidate causes, and mitigations distinct. A mitigation that changes an outcome is evidence about the system response, not by itself proof of the original cause.',
  '- Do not use evidence about an adjacent mechanism as proof of the mechanism actually in question. When the available observations do not discriminate among plausible causes, say that the cause remains unresolved rather than choosing one by template.',
  '',
