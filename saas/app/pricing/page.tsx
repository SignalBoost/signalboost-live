...
const INDIVIDUAL_PLANS = [
  {
    ...
    cta: 'Start free', highlight: false, partner: false, seats: '1 user',
  },
  {
    ...
    cta: 'Start free trial', highlight: false, partner: true, seats: '1 user',
  },
  {
    ...
    cta: 'Start free trial', highlight: true, partner: true, seats: '1 user',
  },
  {
    ...
    cta: 'Contact us', highlight: false, partner: true, seats: '1 user',
  },
]
...
const TEAM_PLANS = [
  {
    ...
    cta: 'Start free trial', highlight: false, partner: true, seats: '3 users',
  },
  {
    ...
    cta: 'Start free trial', highlight: true, partner: true, seats: '10 users',
  },
  {
    ...
    cta: 'Contact sales', highlight: false, partner: true, seats: 'Unlimited users',
  },
]
    ...
    <main style={{ minHeight: '100vh', background: '#0a0a0f', color: '#fff', fontFamily: 'system-ui' }}>
      <section style={{ maxWidth: 1100, margin: '0 auto', padding: '28px 24px 80px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          ...
          <p style={{ color: 'rgba(255,255,255,0.45)', fontSize: 15, maxWidth: 460, margin: '0 auto 20px' }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {Object.keys(CURRENCIES).map(c => (
                <button key={c} onClick={() => setCurrency(c)}
                  style={{ padding: '4px 12px', borderRadius: 999, fontSize: 11, fontWeight: 700, cursor: 'pointer', border: `1px solid ${currency === c ? '#ffc300' : 'rgba(255,255,255,0.1)'}`, background: currency === c ? 'rgba(255,195,0,0.12)' : 'transparent', color: currency === c ? '#ffc300' : 'rgba(255,255,255,0.4)' }}>
                  {c}
                </button>
              ))}
            </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: `repeat(${plans.length}, 1fr)`, gap: 16 }}>
          {plans.map(plan => (
            <div key={plan.name} style={{ background: plan.highlight ? 'rgba(255,195,0,0.06)' : 'rgba(255,255,255,0.02)', border: `1px solid ${plan.highlight ? 'rgba(255,195,0,0.4)' : 'rgba(255,255,255,0.07)'}`, borderRadius: 20, padding: '28px 22px', display: 'flex', flexDirection: 'column', gap: 18, position: 'relative' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
                  {plan.index === 0 && plan.plan === 'free' ? (
                    ...
                    <>
                      ...
                      <span style={{ fontSize: 40, fontWeight: 900, letterSpacing: '-0.03em' }}>{tab === 'individual' ? cur.individual[plan.index] : cur.team[plan.index]}</span>
              ...
              {plan.partner && (
                <div style={{ fontSize: 11, color: 'rgba(255,195,0,0.6)', textAlign: 'center', marginTop: -8 }}>🤝 Partners: 30 days free</div>
              )}
