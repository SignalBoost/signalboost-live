...
function safeSubject(campaign) {
  const title = cleanText(campaign?.title, 120)
  return title && !isInstruction(title) ? title.replace(/[.:;]+$/g, '') : ''
}

function safeAudience(campaign, lang) {
  const audience = cleanText(campaign?.audience, 110)
...
function storedScript(campaign) {
  const value = cleanText(campaign?.metadata?.campaign_script || campaign?.metadata?.campaignScript, 420)
...
async function processCampaign(campaign) {
  ...
  const dir = await mkdtemp(join(tmpdir(), 'signalboost-fast-final-v2-'))
  ...
  try {
    ...
    const fadeOutStart = Math.max(0, duration - 0.45)
    const filter = [
      'format=yuv420p',
      'noise=alls=2:allf=t+u',
      'vignette=PI/6',
      'eq=contrast=1.03:brightness=-0.01:saturation=0.82',
    ...
    await run('ffmpeg', [
      '-y', '-f', 'lavfi', '-i', `color=c=0x07111f:s=${size}:r=30:d=${duration}`,
      '-i', voice.path,
      '-vf', filter,
    ...
    const patch = {
      ...
      captionsBurned: true,
      audioTrack: true,
      baseVisualSchemaVersion: 'signalboost-base-v4-clean-full-screen',
      captionSchemaVersion: 'signalboost-captions-v4-solid-panel',
      copySchemaVersion: 'signalboost-copy-v4-customer-only',
      finalSchemaVersion: FINAL_SCHEMA,
      voiceScriptSource: 'customer-facing-template-or-sanitized-campaign-script',
      voiceError: null,
      renderError: null,
      brandDebug: { mode: 'single-pass-clean-background', objectPath, lang, size, duration, noCenterStrips: true },
    }

    const update = await sb.from('cos_campaign_queue').update({ metadata: { ...(fresh.metadata || {}), video: patch } }).eq('id', campaign.id)
    if (update.error) throw update.error

    console.log(`COSA campaign ${campaign.id}: clean full-screen final complete (${lang}, ${voice.engine}, ${duration}s, ${size}).`)
    return { ok: true, id: campaign.id, lang, engine: voice.engine, duration, size }
  } catch (error) {
    const failure = errorText(error)
    console.error(`COSA campaign ${campaign.id}: clean final failed: ${failure}`)
    const fresh = (await sb.from('cos_campaign_queue').select('*').eq('id', campaign.id).single()).data || campaign
    const video = fresh?.metadata?.video || {}
    await sb.from('cos_campaign_queue').update({
      metadata: {
        ...
        video: {
          ...
          audioTrack: false,
          voiceError: `clean final worker error: ${failure.slice(0, 500)}`,
          brandingLock: null,
        },
      },
    }).eq('id', campaign.id)
...
console.log(`COSA clean final worker scanned=${campaigns?.length || 0} candidates=${candidates.length}`)
const results = []
