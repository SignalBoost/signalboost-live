...
// evidence, not the public web. This prevents the new general external-fact default from breaking
...
export function requiresFreshExternalEvidence(input: string): boolean {
  ...
  // This is the general stale-world protection. Any remaining direct external factual lookup is
  // verified live by default. It is intentionally last so internal, historical, conceptual, and
  // deterministic requests keep their correct specialized routes.
  if (LOOKUP_INTENT.test(text)) return true
