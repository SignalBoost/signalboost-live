...
const STRINGS: Record<Lang, Record<string, string>> = {
  ...
  pt: {
    ...
    [BUTTON_FINDING_DETAIL]: 'Um elemento de botão não tem onClick e também não é do tipo submit — provavelmente é um clique sem ação. Verifique se não é um falso positivo.',
    [BUTTON_FIX_TEXT]: 'Conecte um onClick à ação, transforme em um link com href real, use type="submit" dentro de um formulário ou remova o controle.',
  ...
  es: {
    ...
    [BUTTON_FINDING_DETAIL]: 'Un elemento de botón no tiene onClick y tampoco es de tipo submit — probablemente es un clic sin acción. Verifica que no sea un falso positivo.',
    [BUTTON_FIX_TEXT]: 'Conecta un onClick a la acción, conviértelo en un enlace con href real, usa type="submit" dentro de un formulario o elimina el control.',
  ...
  pl: {
    ...
    [BUTTON_FINDING_DETAIL]: 'Element przycisku nie ma onClick i nie jest przyciskiem submit — prawdopodobnie jest to kliknięcie bez akcji. Sprawdź, czy to nie fałszywy alarm.',
    [BUTTON_FIX_TEXT]: 'Podłącz onClick do właściwej akcji, zamień element na link z prawdziwym href, ustaw type="submit" w formularzu albo usuń kontrolkę.',
  ...
  ru: {
    ...
    [BUTTON_FINDING_DETAIL]: 'У элемента кнопки нет onClick, и он не является submit-кнопкой — вероятно, это клик без действия. Проверьте, не ложное ли это срабатывание.',
    [BUTTON_FIX_TEXT]: 'Подключите onClick к нужному действию, замените на ссылку с реальным href, используйте type="submit" внутри формы или удалите элемент.',
