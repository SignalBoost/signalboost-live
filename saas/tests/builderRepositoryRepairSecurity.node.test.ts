...
test('owner repository repair is routed before public-delivery isolation and Builder rechecks owner authority', () => {
  ...
  const auditIdentity = browser.indexOf('const auditUserId = (await getAccess()')
  const ownerLookup = browser.indexOf('const ownerRepairAccess = isPastedOperationalLog(prompt)', auditIdentity)
  const ownerGuard = browser.indexOf('if (ownerRepairAccess?.isOwner)', ownerLookup)
  const publicExecution = browser.indexOf('const response = await withPublicAuditIdentity', ownerGuard)
  assert.ok(auditIdentity >= 0)
  assert.ok(ownerLookup > auditIdentity)
  assert.ok(ownerGuard > ownerLookup)
  assert.ok(publicExecution > ownerGuard)
  assert.match(browser.slice(ownerGuard, publicExecution), /builderPost\(builderRequest\)/)
