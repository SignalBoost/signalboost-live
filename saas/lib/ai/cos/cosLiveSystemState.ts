// saas/lib/ai/cos/cosLiveSystemState.ts
...
export type CosLiveSystemState = {
  ...
  autonomousLearning:{enabled:boolean;ready:boolean;warnings:string[]}
...
export async function buildCosLiveSystemState(args:{userId?:string|null;privileged:boolean}):Promise<CosLiveSystemState>{
  ...
  const learningReadiness=autonomousLearningReadiness()
  return{generatedAt,deployment:{commitSha:s(process.env.VERCEL_GIT_COMMIT_SHA),environment:s(process.env.VERCEL_ENV)},localReasoner:await reasonerPromise,externalFallbackEnabled:externalFallbackEnabled(),enterpriseMemory,knowledgeGraph,learnedCorpus,autonomousLearning:{enabled:learningReadiness.autonomousEnabled,ready:learningReadiness.ready,warnings:learningReadiness.warnings},cognitiveSkills,cache,userMemory:{available:Boolean(args.userId),records:args.userId?memories.length:null},lastTurnRecord}
}
