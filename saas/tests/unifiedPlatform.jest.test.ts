...
describe('Unified NASA-style SignalBoost platform', () => {
  ...
  test('Admin Console exposes requested panels and CRM/forecasting primitives', () => {
    expect(ADMIN_SIDEBAR.map(item => item.label)).toEqual(['Overview', 'Logs', 'Outreach', 'Marketing + Sales', 'Insights', 'Role Management', 'Marketplace Monitor', 'SaaS Monitor', 'Concierge Monitor', 'Governance', 'Timeline'])
