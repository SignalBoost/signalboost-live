-- Atomically choose and reserve the next owner-authorized mass-distilled RunPod canary.
-- Newest approval-or-suspension control is authoritative; one short global lease prevents overlap.
...
declare
  ...
  v_reservation jsonb;
begin
  -- Serialize all claimers before checking the global in-flight lease. This prevents two callers
  -- from simultaneously reserving different artifacts when both started before either committed.
  perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended('mass-distilled-runtime-canary-global', 0));

  -- Only one mass canary may be in flight at a time. A stale reservation self-expires after 8 min.
  if exists (
    select 1
    ...
    where s.event_type='fine_tune'
      ...
      and not exists (
        ...
        where t.event_type='fine_tune'
          ...
          and t.evidence->>'claim' in ('local_distilled_runtime_canary_passed','local_distilled_runtime_canary_failed')
  ...
  -- Scan the whole eligible corpus oldest-first. No fixed row window can starve later approvals.
  for v_artifact in
    select a.*
  ...
  loop
    ...
    if v_started >= v_max_invocations then continue; end if;
    ...
    v_event_key := encode(extensions.digest(convert_to(
      'mass-distilled-canary-reservation:'||v_artifact.candidate_id||':'||v_artifact.trained_artifact_hash||':'||v_control.observed_at::text||':'||(v_started+1)::text,
      'UTF8'),'sha256'),'hex');
