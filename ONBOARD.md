...
**Version:** 1.35
**Builder chunked file assembly — 2026-09-05:** #1861 is deployed as `db4fe3e75dcd5ef50d112857f3fcd7d61c69ed2f`, Production `dpl_4TfvWiXoQYAwo4768NP8jhkQ653a` READY; protected continuation returned 200 at 06:27:50 UTC and a request without its secret returned 401. No live model-driven checkpoint acceptance was observed. The shared Builder tool loop now accepts sequential `write_file` chunks (`chunkIndex`, `final`), stages one file privately, and publishes the full assembled source only on finalization. Staged fragments have separate `stage_file` evidence, do not consume successful file-write counts, cannot satisfy mutation proof and are not workspace downloads. Pending content survives the existing private checkpoint. Commands and completion are blocked until finalization; duplicate/skipped indices, mismatched paths, changed targets, oversized files and more than 16 chunks fail explicitly. Total work limits and the 512 KiB file limit remain unchanged. A 600-line source was assembled across serialization/resume and executed with real Node; chunked existing-file fail/change/pass and storage-retry behavior are regression-covered. Model choices are scripted, not live acceptance. No database schema change is required. Pending exact CI/Preview, merge, Production READY and live Concierge large-file acceptance.
