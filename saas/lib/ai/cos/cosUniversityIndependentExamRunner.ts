...
import { cosUniversityIndependentExamRunKey } from './cosUniversityExamRunKey.ts'
import {
...
async function loadReadyStudyPlans(agentId: string, limit: number): Promise<ReadyStudyPlan[]> {
  ...
  const result = await db.from('cos_university_study_plans')
    .select('id,subject_id,language_code,language_dimension,attempt_count')
    .eq('agent_id', agentId)
  ...
  for (const row of result.data || []) {
    if (row.language_code && row.language_dimension) {
      plans.push({ id: row.id, attemptCount: Math.max(1, Math.floor(Number(row.attempt_count || 1))), target: { kind: 'language', language: row.language_code as CosPlatformLanguage, dimension: row.language_dimension as CosPlatformLanguageDimension } })
    } else if (row.subject_id) {
      plans.push({ id: row.id, attemptCount: Math.max(1, Math.floor(Number(row.attempt_count || 1))), target: { kind: 'subject', subjectId: row.subject_id as CosUniversitySubjectId } })
    }
  }
  return plans
}
...
async function runTarget(agentId: string, target: CosUniversityExamTarget, now: Date, readyStudyPlan?: ReadyStudyPlan | null): Promise<CosUniversityExamRunSummary> {
  const row = await createOrFindRun(agentId, target, now, readyStudyPlan)
