...
export interface ClusterRuntimeHealthSnapshot {
  ...
  summary: Readonly<{ totalInstructions: number; pendingAcknowledgments: number; verifiedOrCompleted: number; retryRecommendations: number; terminalOutcomes: number; staleTerms: number; warningAlerts: number; criticalAlerts: number; humanReviewRequired: number }>
  reasons: readonly string[]
  safety: Readonly<{ readOnly: true; automaticRetryEnabled: false; automaticRepairEnabled: false; infrastructureMutationEnabled: false; runtimeExecutionControlsExposed: false }>
  executable: false
}
function iso(value: string): string { const parsed = Date.parse(value); if (!Number.isFinite(parsed)) throw new Error('invalid cluster runtime health timestamp'); return new Date(parsed).toISOString() }
export function createClusterRuntimeHealth(input: { diagnostics: ClusterDeliveryDiagnosticsSnapshot; alerts: ClusterDeliveryAlertEvaluation }): ClusterRuntimeHealthSnapshot {
  const diagnostics = input.diagnostics; const alerts = input.alerts
  ...
  const status: ClusterRuntimeHealthStatus = staleTerms > 0 || alerts.counts.critical > 0 ? 'critical' : terminalOutcomes > 0 || retryRecommendations > 0 || alerts.counts.warning > 0 ? 'warning' : pendingAcknowledgments > 0 ? 'unknown' : 'healthy'
  return Object.freeze({ schemaVersion: 'agent-gateway-cluster-runtime-health-v1', generatedAt: iso(diagnostics.generatedAt), clusterId: diagnostics.clusterId, currentTerm: diagnostics.currentTerm, status, summary: Object.freeze({ totalInstructions: diagnostics.totalInstructions, pendingAcknowledgments, verifiedOrCompleted, retryRecommendations, terminalOutcomes, staleTerms, warningAlerts: alerts.counts.warning, criticalAlerts: alerts.counts.critical, humanReviewRequired }), reasons: Object.freeze([...reasons].sort()), safety: Object.freeze({ readOnly: true, automaticRetryEnabled: false, automaticRepairEnabled: false, infrastructureMutationEnabled: false, runtimeExecutionControlsExposed: false }), executable: false })
}
