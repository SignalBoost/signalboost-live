...
create table if not exists public.cos_local_distillation_artifacts (
  ...
  runtime_preference text not null default 'runpod_primary_deepinfra_fallback',
...
comment on column public.cos_local_distillation_artifacts.runtime_preference is
  'Intended serving topology: RunPod/iTMounts compute first, DeepInfra only as fallback/escalation.';

-- Backfill already-trained distilled artifacts so prior training is not stranded on Hugging Face.
with trained as (
...
select
  ...
  w.student_model_id,
  w.teacher_model_id,
  w.trained_artifact_id,
  w.trained_artifact_hash,
  w.evidence_ref,
  w.revision_key,
  ...
  'itmounts_local',
  'runpod_primary_deepinfra_fallback',
  'lora_adapter',
  jsonb_build_object(
    'owner','itmounts',
    'canonicalBaseModel',w.student_model_id,
    'adapterModel',w.trained_artifact_id,
