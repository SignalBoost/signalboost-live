...
test('unhealthy composite dependency is never selectable despite high similarity', () => {
  const result = assessSkillSelection({
    status: 'mastered',
    similarity: 0.99,
    productionAttempts: 20,
    productionSuccesses: 20,
    retentionAttempts: 5,
    retentionSuccesses: 5,
    failureCount: 0,
...
test('verified outcome history can break close semantic ties', () => {
  const reliable = assessSkillSelection({
    status: 'learned',
    similarity: 0.76,
  ...
  const fragile = assessSkillSelection({
    status: 'learned',
    similarity: 0.78,
    productionAttempts: 10,
    productionSuccesses: 6,
    retentionAttempts: 5,
    retentionSuccesses: 3,
    failureCount: 8,
    dependencyHealthy: true,
  })
...
test('metacognition marks unresolved capability without strong skills as weak', () => {
  const result = assessCapabilityRegion({
    strongSkills: 0,
    weakenedSkills: 0,
    quarantinedSkills: 0,
    unresolvedGaps: 3,
