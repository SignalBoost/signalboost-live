...
test('a passing gate without recorded evidence is rejected', () => {
  assert.throws(() => evaluateReleaseCandidateReadiness({ tenant: { tenantId: 'tenant-a', environmentId: 'prod' }, generatedAt: '2026-08-10T03:11:00.000Z', checks: [{ checkId: 'deployment', category: 'deployment', status: 'pass', required: true, summary: 'claimed green', evidence: [] }] }), /rc_pass_requires_evidence/)
})

test('evidence observed after the readiness snapshot is rejected', () => {
  assert.throws(() => evaluateReleaseCandidateReadiness({ tenant: { tenantId: 'tenant-a', environmentId: 'prod' }, generatedAt: '2026-08-10T03:11:00.000Z', checks: [{ checkId: 'deployment', category: 'deployment', status: 'pass', required: true, summary: 'production deployment green', evidence: [{ ...evidence[0], observedAt: '2026-08-10T03:12:00.000Z' }] }] }), /rc_evidence_timestamp_after_snapshot/)
})

test('not-run required evidence blocks RC status', () => {
  const snapshot = evaluateReleaseCandidateReadiness({ tenant: { tenantId: 'tenant-a', environmentId: 'prod' }, generatedAt: '2026-08-10T03:11:00.000Z', checks: [{ checkId: 'pentest', category: 'security', status: 'not_run', required: true, summary: 'external penetration test', evidence: [] }] })
...
test('multi-tenant isolation rejects any cross-tenant exposure', () => {
  assert.equal(evaluateTenantIsolation([{ probeId: 'p1', sourceTenant: 'tenant-a', targetTenant: 'tenant-b', blocked: true, leakedFields: [] }, { probeId: 'p2', sourceTenant: 'tenant-b', targetTenant: 'tenant-a', blocked: true, leakedFields: [] }]).pass, true)
})
...
test('security readiness remains blocked until penetration testing passes', () => {
  ...
  assert.equal(evaluateSecurityEvidence({ dependencyAuditPass: true, secretScanPass: true, authzRegressionPass: true, tenantIsolationPass: true, penetrationTestStatus: 'pass', criticalFindings: 0, highFindings: 0 }).pass, true)
})
