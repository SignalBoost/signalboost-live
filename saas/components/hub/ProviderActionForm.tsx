...
export default function ProviderActionForm({ templateId, lang, onSuccess, onError, onClose }: ProviderActionFormProps) {
  ...
  const handleSubmit = async () => {
    if (!validate()) return setState('idle')
    if (template.previewBeforeSubmit) return setState('preview')
    if (template.requiresConfirm) return setState('confirm')
    await executeAction()
  }
  ...
  const executeAction = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setState('success')
        if (template.api.method !== 'GET') window.setTimeout(() => onSuccess?.(), 1200)
      } else {
  ...
  return (
    <div style={{ ...cardStyle, width: '100%', height: '100%', maxHeight: '100%', minHeight: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ ...bodyStyle, gap: 16, flex: '1 1 auto', minHeight: 0, overflow: 'hidden' }}>
        ...
        {state === 'confirm' && <CenteredState icon="⚠️" title="Confirm action">You are about to execute <strong>{template.label}</strong> on {template.api.service}.</CenteredState>}
        ...
        {state === 'error' && result?.error && <CenteredState icon="❌" title="Error" titleColor="#ef4444"><span style={{ fontFamily: monoStyle.fontFamily }}>{result.error}</span></CenteredState>}
      </div>

      <div style={footerStyle}>
        {state === 'idle' && <>{onClose && <button onClick={onClose} className="hub-chip" style={secondaryButtonStyle}>Cancel</button>}<button onClick={handleSubmit} className="hub-btn" style={primaryButtonStyle}>{template.previewBeforeSubmit ? 'Preview' : template.requiresConfirm ? 'Confirm' : 'Execute'}</button></>}
        {state === 'preview' && <><button onClick={() => setState('idle')} className="hub-chip" style={secondaryButtonStyle}>Back</button><button onClick={() => (template.requiresConfirm ? setState('confirm') : executeAction())} className="hub-btn" style={warningButtonStyle}>{template.requiresConfirm ? 'Confirm' : 'Execute'}</button></>}
        {state === 'confirm' && <><button onClick={() => setState(template.previewBeforeSubmit ? 'preview' : 'idle')} className="hub-chip" style={secondaryButtonStyle}>Cancel</button><button onClick={executeAction} className="hub-btn" style={dangerButtonStyle}>Execute Now</button></>}
...
function FormField({ field, value, error, onChange }: FormFieldProps) {
  ...
  const renderInput = () => {
    switch (field.type) {
      ...
      case 'select':
        return <select value={(value as string) || ''} onChange={e => onChange(e.target.value)} style={{ ...baseStyle, cursor: 'pointer' }}><option value="" disabled style={{ color: '#111', background: '#fff' }}>{field.placeholder || 'Select an option'}</option>{field.options?.map(opt => <option key={opt.value} value={opt.value} style={{ color: '#111', background: '#fff' }}>{opt.label}</option>)}</select>
      case 'toggle':
  ...
  return <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 14 }}>{field.type !== 'toggle' && <label style={{ ...labelStyle, display: 'flex', alignItems: 'center', gap: 6 }}>{field.label}{field.required && <span style={{ color: '#ef4444' }}>*</span>}</label>}{renderInput()}{field.help && <div style={{ fontSize: 11, color: 'rgba(255,255,255,.45)', marginTop: -2 }}>{field.help}</div>}{error && <div style={{ fontSize: 11, color: '#ef4444', marginTop: -2 }}>⚠️ {error}</div>}</div>
}
...
function ResultView({ data }: { data: any }) {
  ...
  if (data && typeof data === 'object') {
    ...
    if (entries.length > 0) return <div style={scrollBoxStyle}>{entries.map(([k, v]) => <div key={k} style={keyValueRowStyle}><span style={{ color: 'rgba(255,255,255,.5)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{k}</span><span style={valueTextStyle} title={formatCell(v)}>{formatCell(v)}</span></div>)}</div>
  }
...
function ObjectArrayTable({ rows }: { rows: any[] }) {
  ...
  return <div style={{ flex: 1, minHeight: 0, overflow: 'hidden', border: '1px solid rgba(255,255,255,.08)', borderRadius: 10 }}><div style={{ height: '100%', overflowY: 'auto', overflowX: 'hidden' }}><div style={{ display: 'grid', gridTemplateColumns: gridColumns, gap: 10, padding: '8px 10px', position: 'sticky', top: 0, background: 'rgba(8,11,20,.98)', borderBottom: '1px solid rgba(255,255,255,.08)', zIndex: 1 }}>{columns.map(col => <div key={col} style={tableHeadStyle}>{niceLabel(col)}</div>)}</div>{rows.slice(0, 100).map((row, i) => <div key={i} style={{ display: 'grid', gridTemplateColumns: gridColumns, gap: 10, padding: '8px 10px', borderBottom: '1px solid rgba(255,255,255,.06)', alignItems: 'center' }}>{columns.map(col => <div key={col} title={formatCell(row[col])} style={col === 'name' || col === 'label' ? tableMainCellStyle : tableCellStyle}>{formatCell(row[col])}</div>)}</div>)}</div></div>
}
...
function makeColumns(columns: string[]) { if (columns.length <= 1) return 'minmax(0, 1fr)'; return columns.map((column, index) => index === 0 ? 'minmax(160px, 1.7fr)' : column === 'active' ? '70px' : column.toLowerCase().includes('created') ? '105px' : 'minmax(90px, 1fr)').join(' ') }
