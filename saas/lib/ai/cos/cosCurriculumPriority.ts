...
export type CosCurriculumTrack = { id:'cyber_defense'|'software_engineering'|'agent_systems'|'ml_data_engineering'|'ai_systems_safety'|'cognitive_science'; title:string; topics:string[]; evaluation:string[]; safetyBoundary:string }
...
export function coreCurriculumTrackForSubject(subject:string):CosCurriculumTrack|null{
 ...
 if(/cognitive|psycholog|behavioral economics|decision bias|attention|memory|human.agent|accessibility|cognitive load/.test(value))return COS_CORE_CURRICULUM_TRACKS[4]
 if(/agent|rag|retrieval|prompt|symbolic|multi-agent|planning/.test(value))return COS_CORE_CURRICULUM_TRACKS[5]
 return null
}
