...
function uniqueStrings(value: unknown, field: string): readonly string[] {
  ...
  if (!Array.isArray(value) || value.length > 50 || value.some(item => typeof item !== 'string' || item.length === 0 || item.length > 256)) throw new Error(`invalid cluster runtime health governance evidence index query ${field}`)
