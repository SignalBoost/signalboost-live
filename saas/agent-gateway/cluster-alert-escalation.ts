...
export interface ClusterAlertEscalationPolicy { operatorAfterMs: number; incidentAfterMs: number; executiveAfterMs: number; incidentRecurrenceThreshold: number; executiveRecurrenceThreshold: number }
export interface ClusterAlertEscalationRecommendation { schemaVersion: 'agent-gateway-cluster-alert-escalation-recommendation-v1'; recommendationId: string; alertId: string; clusterId: string; level: ClusterAlertEscalationLevel; reason: string; generatedAt: string; requiresHumanAction: boolean; notificationSent: false; automaticRemediationEnabled: false; readOnly: true; executable: false }
export interface ClusterAlertEscalationPlan { schemaVersion: 'agent-gateway-cluster-alert-escalation-plan-v1'; clusterId: string; generatedAt: string; recommendations: readonly ClusterAlertEscalationRecommendation[]; counts: Readonly<Record<ClusterAlertEscalationLevel, number>>; highestLevel: ClusterAlertEscalationLevel | null; notificationsEnabled: false; automaticRemediationEnabled: false; readOnly: true; executable: false }
...
function time(value: string): number { const parsed = Date.parse(value); if (!Number.isFinite(parsed)) throw new Error('invalid cluster alert escalation timestamp'); return parsed }
...
export function planClusterAlertEscalations(input: { diagnostics: ClusterAlertDiagnosticsSnapshot; policy?: Partial<ClusterAlertEscalationPolicy> }): ClusterAlertEscalationPlan {
  ...
  const recommendations = input.diagnostics.items.flatMap(item => {
    const level = levelFor(item, Math.max(0, generatedAtMs - time(item.lastDetectedAt)), policy)
    if (!level) return []
