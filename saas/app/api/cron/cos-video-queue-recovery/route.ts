...
async function recordEvent(sb: any, job: any, eventType: string, severity: string, payload: Record<string, unknown>) {
  await sb.from('cos_video_lifecycle_events').insert({
    ...
    actor_type: 'queue_recovery',
    actor_id: 'cos-video-queue-recovery',
  ...
  for (const job of jobs || []) {
    ...
    if (attempts + 1 < maxAttempts) {
      await sb.from('cos_video_production_jobs').update({
        ...
        watchdog_signal: { ...signal, next_attempt: attempts + 1 },
        updated_at: now,
      }).eq('id', job.id)
      await recordEvent(sb, job, 'yellow_warning_generated', 'warn', { ...signal, next_attempt: attempts + 1 })
      results.push({ job: job.id, action: 'retry_queued', pool, attempts: attempts + 1, maxAttempts })
      continue
    }
