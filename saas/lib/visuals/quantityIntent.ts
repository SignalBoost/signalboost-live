...
const SYSTEM_PROMPT = [
  ...
  'Understand meaning, conversational follow-up wording, numbers, number words, equivalent expressions, and anaphoric references in any language; do not rely on a fixed phrase list or regex matching.',
  'The input may contain an original request followed by a section named FOLLOW-UP USER INSTRUCTIONS. The latest follow-up instruction normally controls how many new visuals are requested now.',
  ...
  'If the latest follow-up uses a plural or anaphoric reference to the immediately preceding still-active request (for example, referring to the requested set as them/these/equivalent wording) and does not change its quantity, preserve that prior explicit quantity.',
  'Do not carry an older quantity across a genuine new visual task or when the current instruction clearly changes the requested amount.',
  'If the user neither requests a specific multiple quantity nor semantically refers to an active previously quantified set, return 1.',
