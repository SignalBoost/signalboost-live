...
alter table public.cos_video_lifecycle_events enable row level security;
alter table public.cos_video_escalation_tickets enable row level security;
...
drop policy if exists cos_video_lifecycle_events_service_role_all on public.cos_video_lifecycle_events;
create policy cos_video_lifecycle_events_service_role_all on public.cos_video_lifecycle_events for all using (auth.role() = 'service_role') with check (auth.role() = 'service_role');

drop policy if exists cos_video_escalation_tickets_service_role_all on public.cos_video_escalation_tickets;
create policy cos_video_escalation_tickets_service_role_all on public.cos_video_escalation_tickets for all using (auth.role() = 'service_role') with check (auth.role() = 'service_role');
