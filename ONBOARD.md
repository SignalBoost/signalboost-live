...
**Concierge visual designer:** implemented and deployed to Production on 2026-08-30; the DeepInfra request compatibility repair is `2fa3d82cca9fd3dbd0eb1d988bec342a69ceb7bb`, deployment `dpl_3J9z4h6MCXiCPEQmJTaZyDEqbyPX` — READY. Explicit sketch, drawing, illustration, diagram, scene, and image-creation requests run through an authenticated backend visual tool, reuse the approved DeepInfra image API when a dedicated OpenAI image account is absent, store the result only in the requesting user's private workspace, render it inline in the homepage Concierge, and provide a download. DeepInfra rejects the optional `response_format` field, so it is now sent only to OpenAI. A fresh signed-in Production visual-generation observation remains required before runtime acceptance.
