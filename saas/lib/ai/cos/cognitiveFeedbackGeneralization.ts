...
function reasoningTriggerKinds(input: FeedbackGeneralizationInput): string[] {
  return detectCognitiveReasoningTriggers(input.prompt).map(trigger => trigger.kind)
}
...
async function persistFeedbackDraft(input: FeedbackGeneralizationInput, draft: CognitiveSkillDraft, reasonerLabel: string) {
  ...
  const triggers = reasoningTriggerKinds(input)
  ...
  const inserted = await db.from('cos_cognitive_skills').insert({
    ...
    procedure: procedureWithTriggers(draft, triggers),
    status: 'encountered',
  ...
  return { row: inserted.data, triggers }
}
...
export async function generalizeFeedbackIntoCognitiveSkill(input: FeedbackGeneralizationInput): Promise<FeedbackGeneralizationResult> {
  ...
  if (!reflected?.text) return { attempted: true, candidateCreated: false, skillKey: null, localPracticeQueued: 0, reasoningTriggers: [], reason: 'local_reflection_unavailable' }
  if (noReusableSkill(reflected.text)) return { attempted: true, candidateCreated: false, skillKey: null, localPracticeQueued: 0, reasoningTriggers: [], reason: 'no_reusable_skill' }

  const draft = parseCognitiveSkillDraft(reflected.text)
  if (!draft) return { attempted: true, candidateCreated: false, skillKey: null, localPracticeQueued: 0, reasoningTriggers: [], reason: 'candidate_parse_failed' }
  const validation = validateCognitiveSkillDraft(draft)
  if (!validation.ok) return { attempted: true, candidateCreated: false, skillKey: null, localPracticeQueued: 0, reasoningTriggers: [], reason: `candidate_rejected:${validation.reasons.join(',')}` }

  const persisted = await persistFeedbackDraft(input, draft, reflected.reasoner.label)
  if (!persisted?.row?.skill_key) return { attempted: true, candidateCreated: false, skillKey: null, localPracticeQueued: 0, reasoningTriggers: [], reason: 'skill_store_unavailable' }
  const localPracticeQueued = await queueLocalPractice(input, draft, String(persisted.row.skill_key))
  return {
    attempted: true,
    candidateCreated: true,
    skillKey: String(persisted.row.skill_key),
    localPracticeQueued,
    reasoningTriggers: persisted.triggers,
    reason: 'candidate_encountered_requires_independent_validation',
  }
}
