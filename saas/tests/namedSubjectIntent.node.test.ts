...
test('the visuals route filters candidates before intent detection', async () => {
  ...
  const route = await fs.readFile('app/api/visuals/route.ts', 'utf8')
  assert.match(route, /const realPeople = await filterRealPeople\(objective, extractNamedPeople\(objective\)\)/)
  assert.match(route, /detectConciergeVisualIntent\(objective, \{ semanticVisual, realPeople \}\)/)
})
