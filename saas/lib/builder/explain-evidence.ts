import { builderRunSourceEvidence, builderSourceComparison, builderSourceDigest } from './source-evidence.ts'
...
export async function explainBuilderEvidence(input: {
  ...
  for (const path of paths) {
    ...
    if (file) files.push({ path, sha256: builderSourceDigest(file.content), content: file.content.slice(0, 8000), truncated: file.content.length > 8000 })
  }
  const evidence = formatBuilderExecutionEvidence(trace)
  const sourceComparison = builderSourceComparison(trace, files)
  const sourceHistory = trace.flatMap((item, index) => {
    if (item?.toolId !== 'run') return []
    const snapshot = builderRunSourceEvidence(item).sourceSnapshot
    return snapshot ? [{ eventId: index + 1, scope: snapshot.scope, omittedFiles: snapshot.omittedFiles,
      files: snapshot.files.filter(file => paths.includes(file.path)) }] : []
  }).slice(-3)
  const sourceRules = ' sourceHistory contains bounded snapshots of inputs supplied to recorded command runners, not final-job or post-command snapshots. Use sourceComparison as the authority for current-versus-historical identity: same only establishes matching full content for that named file at that runner input, different establishes different content, unavailable establishes no comparison. Never claim that current files match an old job or its final state without recorded identity evidence. Missing snapshots cannot be reconstructed from current files or successful checks. Read exact source expressions carefully; do not move slice boundaries or change conditions when paraphrasing. If no snapshot exists, explain recorded edits and outcomes but explicitly state that exact historical file identity is unavailable.'
  const context = { sourceComparison, sourceHistory, question: input.prompt, originalRequirement: typeof job.objective === 'string'
    ...
    recordedTrace: trace.slice(-20).map(item => item?.toolId === 'edit_file'
      ? { ...item, change: undefined, editEvidence: builderEditEvidence(item.change) } : { ...item, sourceSnapshot: undefined }), events: builderEvidenceEvents(trace).slice(-20), omittedEvents: Math.max(0, trace.length - 20) }
