...
test('mass evaluator binds exact governed training revision, pinned holdout and dynamic canary model', () => {
  ...
  assert.match(runner, /mass_distilled_evaluation_canary_model_identity_missing/)
  assert.match(runner, /local_distilled_runtime_canary_passed/)
  assert.match(runner, /evidence\.endpointId===claim\.endpointId/)
...
test('mass evaluator runs exactly four suites with eight endpoint and four judge ceilings', () => {
  ...
  assert.match(runner, /max_tokens:1024/)
  assert.match(runner, /prompt\.length>18_000/)
  assert.match(runner, /payload\?\.error\?\.message/)
