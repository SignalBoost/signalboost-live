...
const TEST_FILE = /(?:\.(?:test|spec)\.(?:c?js|mjs|cts|mts|ts)$|(?:^|\/)(?:test[_-].+|.+[_-]test)\.py$)/i
...
export function planDebugFileJob(objective: string, files: readonly DebugFileInput[]): DebugFilePlan | null {
  ...
  // When a test/spec is supplied, make it the proof entrypoint. Otherwise keep the first attached
  // executable as the backwards-compatible command target.
