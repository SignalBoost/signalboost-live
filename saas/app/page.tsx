...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // The thread is no longer an inner scroll box, so the PAGE is what scrolls.
    const thread = threadRef.current
    if (thread) window.scrollTo({ top: document.documentElement.scrollHeight, behavior: 'smooth' })
    ...
    >
                  ...
                  >
                    ...
                    {turn.visualPreviewUrl ? (
                      ...
                      >
                        <img
                          ...
                          width={512}
                          style={{ display: 'block', width: '100%', height: 'auto', maxHeight: '78vh', objectFit: 'contain', borderRadius: 16 }}
                        />
                    ...
                    {turn.builderWorkspaceId && turn.builderFiles?.some((path) => /\.html?$/i.test(path)) ? (
                      <iframe
                        title={turn.response}
                        sandbox=""
                        className="mt-3 h-[78vh] min-h-[520px] w-full rounded-xl border border-white/15 bg-white"
      ...
      <div className={'composer-area' + (showWelcome ? ' welcome-composer' : '')}>
        ...
        <form className="composer" onSubmit={ask}>
          ...
          >📎</button>
          <VoiceInputButton
            lang={(['en', 'es', 'pt', 'pl', 'ru'].includes(lang) ? lang : 'en') as 'en' | 'es' | 'pt' | 'pl' | 'ru'}
            value={question}
            onChange={setQuestion}
            disabled={loading}
            className="attach-button voice-button"
          />
          <textarea
            value={question}
      ...
      <style jsx>{`
        .concierge-shell{position:relative;isolation:isolate;display:flex;flex-direction:column;min-height:calc(100svh - 165px);max-width:1600px;margin:0 auto;padding:24px 28px 140px;box-sizing:border-box;color:#fff;font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;overflow:visible}
