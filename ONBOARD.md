...
**COS communication-intelligence follow-up:** PR #1844 merged as `9eb7202d0fec9f3c818ba6398d5b9fda19259504`; exact Production deployment was READY, but two real public Concierge observations **failed runtime acceptance**. The first invented a Subject line, recipient/signature placeholders and generic HR framing; the owner's exact Foreign Service draft then became more accusatory (`claim`), directive and compressed. Root cause: although the direct editor assembled the delicate register after its generic executive block, the strategic neural writer, quality board and last-repair functions each appended another generic executive block **after** the task-specific profile. That actual final prompt order contradicted the intended precedence and the prior static test inspected the wrong file. The active corrective branch `fix/cos-invented-correspondence-frame-20260905` now places the integrated objective/audience/voice/emotion/seniority/rhetoric/risk profile last in all three neural writing passes and tests those actual arrays. It also adds a narrow deterministic release guard removing generated Subject/opening/placeholder-signature framing only when absent from the source. Source-owned framing remains untouched. Exact Preview/merge/Production gates and another real COS output observation remain required before acceptance.
