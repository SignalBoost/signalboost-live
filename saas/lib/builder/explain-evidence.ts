...
export async function explainBuilderEvidence(input: {
  ...
  const context = { question: input.prompt, status: job.status, recordedError: job.result?.error, currentFiles: files,
    recordedTrace: trace.slice(-20), events: builderEvidenceEvents(trace).slice(-20), omittedEvents: Math.max(0, trace.length - 20) }
