...
import { msT, LANGS, type Lang } from '@/marketing-sales-core'
...
export default function ReviewClient() {
  ...
  async function act(campaignId: string, decision: 'approve' | 'edits' | 'reject') {
    setBusy(campaignId)
    try {
            ...
            return (
              <article key={campaign.id} style={{ border: '1px solid rgba(255,255,255,.10)', borderRadius: 14, padding: 16, background: 'linear-gradient(160deg, rgba(15,23,42,.55), rgba(3,7,18,.65))' }}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  <button disabled={busy === campaign.id} onClick={() => act(campaign.id, 'approve')}
                    style={{ background: '#ffc300', color: '#000', border: 'none', borderRadius: 999, padding: '9px 18px', fontWeight: 900, cursor: 'pointer' }}>{msT(L, 'approve')}</button>
                  <button disabled={busy === campaign.id} onClick={() => act(campaign.id, 'edits')}
                  ...
                  <button disabled={busy === campaign.id} onClick={() => act(campaign.id, 'reject')}
