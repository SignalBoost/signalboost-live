...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!VIDEO_JOB_ID_PATTERN.test(id)) {
    const body: JsonSafeVideoResponse<null> = { ok: false, data: null, error: 'Invalid video job id', meta: responseMeta() }
    ...
    if (existsSync(resultPath)) {
      try {
        ...
        if (!stats.isFile() || stats.size > MAX_RESULT_FILE_BYTES) {
          const body: JsonSafeVideoResponse<null> = { ok: false, data: null, error: 'Invalid video job result', meta: responseMeta() }
          return NextResponse.json(body, { status: 500 })
        }

        const parsed = JSON.parse(readFileSync(resultPath, 'utf8'))
        if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed) || ('id' in parsed && parsed.id !== id)) {
          const body: JsonSafeVideoResponse<null> = { ok: false, data: null, error: 'Invalid video job result', meta: responseMeta() }
          return NextResponse.json(body, { status: 500 })
        }
        data = parsed
