...
export const DATA_CENTER_OPERATIONS_COPY = {
  ...
  benchmarkBody: { en: 'Runs the 14 hidden data-center cases as bounded one-case requests. Holdout prompts stay server-side and facility control remains disabled.', es: 'Ejecuta los 14 casos ocultos de centros de datos como solicitudes limitadas de un solo caso. Los prompts de evaluación permanecen en el servidor y el control de instalaciones sigue deshabilitado.', pt: 'Executa os 14 casos ocultos de data center como solicitações limitadas de um caso. Os prompts de holdout permanecem no servidor e o controle da instalação continua desativado.', pl: 'Uruchamia 14 ukrytych przypadków centrum danych jako ograniczone żądania po jednym przypadku. Prompty holdout pozostają po stronie serwera, a sterowanie obiektem pozostaje wyłączone.', ru: 'Запускает 14 скрытых тестовых случаев дата-центра отдельными ограниченными запросами по одному случаю. Holdout-промпты остаются на сервере, управление объектом отключено.' },
  ...
  benchmarkKeepOpen: { en: 'Keep this page open while the bounded one-case requests run.', es: 'Mantén esta página abierta mientras se ejecutan las solicitudes limitadas de un caso.', pt: 'Mantenha esta página aberta enquanto as solicitações limitadas de um caso são executadas.', pl: 'Pozostaw tę stronę otwartą podczas wykonywania ograniczonych żądań po jednym przypadku.', ru: 'Оставьте эту страницу открытой, пока выполняются отдельные ограниченные запросы.' },
