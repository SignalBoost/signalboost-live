...
This is regression evidence, not a substitute for controlled live production outcome verification. Exact Vercel repair verification is handled separately by the objective-outcome path, which binds terminal provider state to the exact deployment ID created by the governed repair.
