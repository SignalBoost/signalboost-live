...
import { asksForTechnicalPriorAnswerProvenance } from '@/lib/ai/cos/technicalProvenanceIntent'
...
function noPriorReply(language:string):string{return language==='es'?'No tengo un registro de procedencia real para la respuesta inmediatamente anterior. No voy a inventarlo.':language==='pt'?'Não tenho um registro de procedência real para a resposta imediatamente anterior. Não vou inventá-lo.':"I don't have a real provenance record for the immediately preceding answer. I won't fabricate one."}
function briefAnswerOrigin(provenance:any):string{
  const local=provenance?.local_reasoning??{}, external=provenance?.external_ai??{}, origin=provenance?.answer_origin??{}
  if(origin.from_cache)return `That answer was reused from COS's semantic cache${origin.model?`; its original reasoning model was ${origin.model}`:''}. No new web search ran for this reply.`
  const contributors=[['Enterprise Memory',provenance?.enterprise_memory],['Knowledge Graph',provenance?.knowledge_graph],['Learned Corpus',provenance?.learned_corpus],['User Memory',provenance?.user_memory],['Cognitive Skills',provenance?.cognitive_skills]].filter(([,value]:any[])=>Boolean(value?.used)).map(([name])=>name)
  const model=local.model?String(local.model):null
  const first=local.invoked?`That answer was generated by COS's reasoning model${model?` (${model})`:''}, using its pretrained knowledge and reasoning applied to your message.`:'That answer was produced by a COS server-side utility rather than a model reasoning pass.'
  const evidence=contributors.length?` Material internal context: ${contributors.join(', ')}.`:' No Enterprise Memory, Knowledge Graph, Learned Corpus, User Memory, or Cognitive Skills were material to that answer.'
  const externalLine=external.invoked?` An external AI provider also contributed${external.provider?` (${external.provider})`:''}.`:' No web search or external AI provider contributed.'
  return `${first}${evidence}${externalLine}`
}
function requestsCompleteProvenance(input:string):boolean{return asksForTechnicalPriorAnswerProvenance(input)||/\b(?:complete|full|detailed)\s+(?:provenance|telemetry|audit)\b|\b(?:list|identify)\b[^.?!]{0,120}\b(?:systems?|components?|layers?|engines?)\b/i.test(input)}
...
export async function POST(req:NextRequest){
  ...
  if(isProvenanceIntrospection(input) || asksWhereTheAnswerCameFrom(input)){
    ...
    const reply=prior?(requestsCompleteProvenance(input)?formatAuthoritativeProvenance(prior as any,language):briefAnswerOrigin(prior)):noPriorReply(language)
    const liveTelemetry=emitRequestTelemetry({startedAt,input,reply,source:'deterministic',confidence:1,externalAiInvoked:false})
  ...
  try{
    ...
    const reply=continuityFailed?`COS independent reasoning could not complete this request: ${reason.detail} External fallback was also unavailable. No answer or action was released.`:rawReply
