...
//
// Social Outreach Connector release builder and boundary guard.
//
...
const ENTRY = 'lib/outreach/social-portable.ts'
...
const PACKAGE_NAME = '@signalboost/social-outreach-connector'
const PACKAGE_VERSION = '1.0.0-rc.1'
const BUILD_ROOT = path.join(APP_ROOT, 'dist', 'social-outreach-build')
const PACKAGE_ROOT = path.join(BUILD_ROOT, 'package')
const ARTIFACT_ROOT = path.join(APP_ROOT, 'dist', 'portable')
const DOCS = [
  ['../docs/portables/social-outreach-integration-guide.md', 'integration-guide.md'],
]
...
const manifest = {
  ...
  type: 'module',
  description: 'Publish approved content to seven social platforms using your own developer applications and credentials.',
  main: 'src/social-portable.ts',
  types: 'src/social-portable.ts',
  files: ['src', '*.md'],
...
const proving = fs.mkdtempSync(path.join(os.tmpdir(), 'social-portable-'))
...
const installedEntry = path.join(proving, 'node_modules', ...PACKAGE_NAME.split('/'), 'src', 'social-portable.ts')
