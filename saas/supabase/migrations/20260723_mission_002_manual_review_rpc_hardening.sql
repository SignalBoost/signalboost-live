-- Mission 002 phase 5 hardening: direct service-role RPC callers must retain
-- the same non-mutating manual-route shape enforced by the TypeScript store.
...
begin
 ...
 if p_decision->>'actionType' <> 'route_to_manual_review' or p_decision->>'externalSideEffect' <> 'false' then raise exception 'manual_review_not_non_mutating'; end if;
 ...
 if coalesce(jsonb_typeof(p_binding->'approvedStepIds'), '') <> 'array' or coalesce(jsonb_array_length(p_binding->'approvedStepIds'), 0) = 0 then raise exception 'manual_review_scope_missing'; end if;
 if exists (select 1 from jsonb_array_elements_text(p_binding->'approvedStepIds') as approved(step_id) group by approved.step_id having count(*) > 1) then raise exception 'manual_review_scope_duplicate'; end if;
 if exists (select 1 from jsonb_array_elements_text(p_binding->'approvedStepIds') as approved(step_id) where not exists (select 1 from jsonb_array_elements(p_decision->'repairPlan'->'steps') as plan_step where plan_step->>'stepId'=approved.step_id)) then raise exception 'manual_review_scope_mismatch'; end if;
