...
export default function PublicSaaSConsole() {
  ...
  return (
    <section className="relative overflow-hidden rounded-[28px] border border-white/10 bg-slate-950/80 p-5 shadow-2xl shadow-cyan-950/30 md:p-7" aria-label={t(dict, 'studio.catalog.aria')}>
      ...
      <div className="relative">
        <p className="text-xs font-black uppercase tracking-[0.24em] text-cyan-200">{t(dict, 'studio.catalog.eyebrow')}</p>
        <div className="mt-3 flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
          <div>
            <h2 className="text-3xl font-black tracking-tight text-white md:text-5xl">{t(dict, 'studio.catalog.title')}</h2>
            <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-300 md:text-base">{t(dict, 'studio.catalog.subtitle')}</p>
          </div>
          {CREDITS_ACTIVATION && (
            <div className="rounded-2xl border border-amber-200/25 bg-amber-300/10 px-4 py-3 text-sm font-bold text-amber-100">
              {t(dict, 'credits.packs')}
            </div>
          )}
        </div>
              ...
              <section key={group.key} className="rounded-3xl border border-white/10 bg-white/[.045] p-4 backdrop-blur-xl">
                <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-400">{t(dict, `studio.group.${group.key}.desc`, uiText('generatedUi.u_4b58c8b8e3f5233b'))}</p>
                  </div>
                  <span className="rounded-full border border-cyan-300/20 bg-cyan-300/10 px-3 py-1 text-xs font-black uppercase tracking-[.16em] text-cyan-100">{uiText('generatedUi.u_501f2a5f38579faa')}</span>
                </div>
