...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // COS-FIRST — the fix for the Aug 12 root finding: this route previously went
    // straight to Anthropic with none of the COS-first architecture ever engaged.
    // Mirrors /api/cos-primary/route.ts's proven decision flow via the shared
    // lib/ai/cos/cosOrchestration.ts module — ONE implementation, not a second copy
    // that can drift. A provenance-introspection question ("show me the provenance
    // for your last answer") is answered from the REAL recorded provenance of the
    // immediately preceding turn when one exists — never regenerated by a model,
    // which is exactly the fabrication this whole fix exists to stop. Without a
    // prior turn on record, it says so plainly rather than inventing one.
    let cosFirst: Awaited<ReturnType<typeof tryCOSFirstAnswer>> | null = null
    let cosFirstError: string | null = null
    const requestedAction = requestsExternalAction(latestUserMessage)
    ...
    if (isProvenanceQuestion) {
      ...
      if (priorTurn && (priorTurn as any).provenance) {
        ...
        return NextResponse.json({
          reply: formatAuthoritativeProvenance(provenance, languageCode),
          source: 'cos-authoritative-provenance',
          execution_provenance: provenance,
          external_ai_invoked: false,
        })
      }
      // No provenance was recorded for the prior turn yet — most likely because this
      // is the FIRST message in the conversation, or the prior turn predates this fix
      // being deployed. Said plainly rather than fabricated: cross-turn provenance
      // persistence (storing the provenance object alongside each reply) is the
      // necessary next increment, tracked separately, not silently faked here.
      return NextResponse.json({
        reply: languageCode === 'es' ? 'No tengo un registro de procedencia real para un turno anterior en esta conversación — probablemente esta es la primera pregunta, o el turno anterior es de antes de que este seguimiento existiera. No voy a inventar una respuesta a esto.'
          : languageCode === 'pt' ? 'Não tenho um registro de proveniência real para um turno anterior nesta conversa — provavelmente esta é a primeira pergunta, ou o turno anterior é de antes deste rastreamento existir. Não vou inventar uma resposta para isso.'
          : 'I don\'t have a real provenance record for a prior turn in this conversation — most likely this is the first question, or the prior turn predates this tracking. I\'m not going to fabricate an answer to this.',
    ...
    if (cosFirst?.handled) {
      const provenance = authoritativeProvenance(cosFirst, { invoked: false })
      return NextResponse.json({
        reply: cosFirst.reply,
        source: cosFirst.provenance.responseSource === 'semantic_cache' || cosFirst.provenance.responseSource === 'semantic_similarity' ? 'cos-semantic-cache' : 'cos-local-primary',
        confidence_score: cosFirst.confidence,
        confidence_threshold: cosConfidenceThreshold(),
        execution_provenance: provenance,
        external_ai_invoked: false,
      })
    }

    // COS could not answer independently. Logged the same way cos-primary logs an
    // escalation, for the same reason: a decision to spend on external AI should
    // leave a trace explaining why, not just happen silently.
    const reason = escalationReason(cosFirst, cosFirstError, requestedAction)
    logEscalation({
      event: 'production_chat_escalation_decision',
      reason_code: reason.code,
      reason: reason.detail,
      confidence: cosFirst?.confidence ?? null,
      threshold: cosConfidenceThreshold(),
...
── SOURCE-OF-TRUTH NOTE ── Answering this accurately needs live data from ${ev.source}, which is not connected yet. Tell the owner you can't confirm the current figure rather than guessing from memory.`
    ...
    // Narrated-execution backstop: a turn that ran ZERO tools may not tell the owner
    // it is "executing immediately". Appends an honest correction; never rewrites.
    reply = guardNarratedExecution(reply, firedTools.size, languageCode)
