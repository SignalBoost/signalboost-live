  ...
  return (
    <main style={{ maxWidth: 1180, margin: '0 auto', display: 'grid', gap: 18 }}>
      <section style={heroCard}>
        ...
        <Link href="/dashboard/cosa" className="sb-button-secondary" style={{ textDecoration: 'none', display: 'inline-flex', marginTop: 12 }}>{copy.openCosa}</Link>
      </section>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(280px, 1.2fr) minmax(240px, .8fr)', gap: 16, marginTop: 16 }}>
              <article style={mockup}>
                ...
                <div style={{ marginTop: 12, background: '#f8fafc', color: '#111827', borderRadius: 12, padding: 18, minHeight: 280 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 96px', gap: 12, alignItems: 'end', marginTop: 18 }}>
                    <p style={{ fontSize: 12, margin: 0 }}>Visit saas.signalboostapp.com · Campaign ID {campaign.id.slice(0, 8)}</p>
                    <div style={{ border: '6px solid #111827', height: 86, display: 'grid', placeItems: 'center', fontSize: 10, textAlign: 'center' }}>{copy.qr}</div>
                  </div>
                </div>
              </article>
              <aside style={sidePanel}>
                <h3 className="sb-h3">{copy.tracking}</h3>
                <p style={{ ...body, wordBreak: 'break-all' }}>{url}</p>
                <h3 className="sb-h3">{copy.analytics}</h3>
                <Metric label={copy.clicks} value={Number(analytics.clicks || 0)} />
                <Metric label={copy.reach} value={Number(analytics.reach || 0)} />
                <Metric label={copy.conversions} value={Number(analytics.conversions || 0)} />
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 16 }}>
                  <button disabled={Boolean(busy)} onClick={() => patch(campaign, 'APPROVED')} style={primary}>{copy.approve}</button>
