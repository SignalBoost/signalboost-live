...
type Item = { icon: string; label: string; href: string; desc?: string; role?: 'customer-concierge' }
type Group = { id: string; label: string; eyebrow: string; accent: string; width?: number; align?: 'left' | 'right'; ownerOnly?: boolean; authOnly?: boolean; items: Item[] }
...
const CUSTOMER_GROUPS: Group[] = [
  {
    id: 'console', label: 'Console Hub', eyebrow: 'Customer workspace', accent: GOLD, width: 420, authOnly: true,
    items: [
      { icon: '🏠', label: 'Dashboard', href: '/dashboard', desc: 'Customer home base, recent work, credits, and next actions.' },
      { icon: '🤖', label: 'Concierge', href: '/dashboard/assistant', role: 'customer-concierge', desc: 'Normal customer concierge for product help, account questions, audit guidance, cybersecurity guidance, and report explanations.' },
      { icon: '📅', label: 'Calendar', href: '/dashboard/calendar', desc: 'Events, appointments, launch dates, and operating schedule.' },
      { icon: '📑', label: 'Spreadsheets', href: '/dashboard/spreadsheets', desc: 'Imported data and customer working tables.' },
      { icon: '💬', label: 'Feedback', href: '/dashboard/feedback', desc: 'Feedback, requests, issues, and customer comments.' },
      { icon: '📋', label: 'Infrastructure PRs', href: '/dashboard/infrastructure', desc: 'Approve AI-drafted infrastructure changes before anything is applied.' },
    ],
  },
  {
    id: 'audit', label: 'Audit Cockpit', eyebrow: 'Readiness reports', accent: '#a78bfa', width: 410, authOnly: true,
    items: [
      { icon: '📋', label: 'Audit Console', href: '/dashboard/audit', desc: 'Run readiness checks and open the audit workflow.' },
      { icon: '🎛️', label: 'Audit Cockpit', href: '/hub/audit', desc: 'Audit reports, evidence, provider inventory, and roadmap.' },
      { icon: '🔑', label: 'Identity & Secrets Review', href: '/hub/audit/identity', desc: 'Identity, access, secrets, and key exposure reports.' },
      { icon: '🧾', label: 'Printable Audit Reports', href: '/hub/audit', desc: 'Download PDF reports or print/save audit evidence.' },
    ],
  },
  {
    id: 'cyber', label: 'Cybersecurity', eyebrow: 'Monitoring & fixes', accent: CYAN, width: 410, authOnly: true,
    items: [
...
const PRODUCT_GROUPS: Group[] = [
  {
    id: 'website', label: 'Website', eyebrow: 'Build & improve', accent: '#38bdf8', width: 360,
    items: [
  ...
  {
    id: 'studio', label: 'Studio', eyebrow: 'Podcast & content', accent: '#f472b6', width: 400,
    items: [
  ...
  {
    id: 'launchpad', label: 'Launchpad', eyebrow: 'Guided starts', accent: '#fb923c', width: 380,
    items: [
  ...
  {
    id: 'grow', label: 'Grow', eyebrow: 'Outreach & sales', accent: GREEN, width: 410,
    items: [
...
const OWNER_GROUP: Group = {
  id: 'owner', label: 'Owner/Admin', eyebrow: 'Private COS & internal control', accent: RED, align: 'right', width: 440, ownerOnly: true,
  items: [
    { icon: '🤖', label: 'COS Assistant', href: '/dashboard/assistant', desc: 'Your private Chief of Staff assistant for strategy, architecture, code, and operations.' },
...
const HELP_GROUP: Group = {
  id: 'help', label: 'Help', eyebrow: 'Support', accent: '#94a3b8', align: 'right', width: 260,
  items: [
...
function customerGroupsFor(ownerAccess: boolean) {
  if (!ownerAccess) return CUSTOMER_GROUPS
  return CUSTOMER_GROUPS.map(group => group.id === 'console'
    ? { ...group, items: group.items.filter(item => item.role !== 'customer-concierge') }
    : group)
}

export default function PremiumCustomerNavbar() {
  ...
  const groups = [...(user ? customerGroupsFor(ownerAccess) : []), ...PRODUCT_GROUPS, ...(ownerAccess ? [OWNER_GROUP] : []), HELP_GROUP]
  ...
  const triggerStyle = (active: boolean, accent?: string): CSSProperties => ({
    ...
    borderRadius: 999, cursor: 'pointer', color: active ? '#fff' : 'rgba(226,232,240,.76)',
    fontWeight: active ? 800 : 650, fontSize: 13, fontFamily: 'inherit',
    padding: '8px 11px', whiteSpace: 'nowrap', textDecoration: 'none',
        ...
        <div style={{ position: 'absolute', top: '100%', ...dropdownPosition(group.align), paddingTop: 12, opacity: open ? 1 : 0, visibility: open ? 'visible' : 'hidden', transform: open ? 'translateY(0)' : 'translateY(8px)', pointerEvents: open ? 'auto' : 'none', transition: 'opacity .18s ease, transform .18s ease, visibility .18s', zIndex: 220 }} onMouseEnter={() => openNow(group.id)} onMouseLeave={closeSoon}>
          <div style={{ width: group.width || 360, maxWidth: '92vw', position: 'relative', overflow: 'hidden', borderRadius: 22, border: `1px solid ${group.accent}33`, background: 'linear-gradient(145deg, rgba(3,7,18,.98), rgba(15,23,42,.96))', boxShadow: `0 28px 90px rgba(0,0,0,.58), 0 0 45px ${group.accent}12`, backdropFilter: 'blur(18px)', WebkitBackdropFilter: 'blur(18px)' }}>
            ...
            <div style={{ padding: '15px 15px 8px' }}>
              ...
              <div style={{ display: 'grid', gap: 4 }}>{group.items.map((item) => (
                <Link key={`${group.id}:${item.href}:${item.label}`} href={item.href} onClick={() => setOpenMenu(null)} className="sbnav-row" style={{ display: 'flex', gap: 12, alignItems: 'center', padding: 12, textDecoration: 'none' }}>
                  ...
                  <span style={{ minWidth: 0 }}><span style={{ display: 'block', color: '#fff', fontWeight: 800, fontSize: 13 }}>{item.label}</span>{item.desc ? <span style={{ display: 'block', color: 'rgba(148,163,184,.88)', fontSize: 11, marginTop: 2, lineHeight: 1.35 }}>{item.desc}</span> : null}</span>
                </Link>
              ))}</div>
            </div>
          </div>
    ...
    <>
      ...
      <nav ref={navRef} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '11px 22px', background: 'linear-gradient(135deg, rgba(4,8,18,.92), rgba(15,23,42,.76))', borderBottom: '1px solid rgba(26,240,255,.18)', boxShadow: '0 18px 60px rgba(0,0,0,.30), inset 0 1px 0 rgba(255,255,255,.08)', position: 'sticky', top: 0, zIndex: 100, backdropFilter: 'blur(14px)', WebkitBackdropFilter: 'blur(14px)' }}>
        <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none', flexShrink: 0, marginRight: 8 }}>
          ...
          <span style={{ display: 'grid', lineHeight: 1.05 }}><span style={{ color: '#fff', fontWeight: 900, fontSize: 17 }}>signal<span style={{ color: GOLD }}>boost</span></span><span style={{ color: 'rgba(148,163,184,.72)', fontSize: 10, letterSpacing: '.12em', textTransform: 'uppercase' }}>Client suite</span></span>
        </Link>
      ...
      {mobileOpen ? <div style={{ position: 'sticky', top: 65, zIndex: 99, background: 'rgba(4,8,18,.98)', borderBottom: '1px solid rgba(255,255,255,.14)', padding: 16, maxHeight: '80vh', overflowY: 'auto', backdropFilter: 'blur(14px)', WebkitBackdropFilter: 'blur(14px)' }}>
        {user ? <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14, flexWrap: 'wrap' }}><span style={{ fontSize: 12, fontWeight: 800, color: 'rgba(255,195,0,.95)', fontFamily: 'monospace' }}>⚡ {credits}</span><span style={{ fontSize: 11, fontWeight: 900, letterSpacing: '.06em', padding: '3px 10px', borderRadius: 999, background: currentPlanStyle.bg, color: currentPlanStyle.color, fontFamily: 'monospace' }}>{currentPlanLabel}</span>{displayName ? <span style={{ fontSize: 12, fontWeight: 650, color: 'rgba(226,232,240,.72)' }}>{displayName}</span> : null}</div> : null}
        ...
        <div style={{ display: 'grid', gap: 4, marginBottom: 14 }}><Link href="/" style={{ padding: 10, textDecoration: 'none', color: '#fff', fontWeight: 800, fontSize: 14 }}>🏠 Home</Link><Link href="/pricing" style={{ padding: 10, textDecoration: 'none', color: '#fff', fontWeight: 800, fontSize: 14 }}>💳 Pricing</Link></div>
        {groups.map((group) => <div key={group.id} style={{ display: 'grid', gap: 4, marginBottom: 15 }}><span style={{ color: group.accent, fontSize: 11, fontWeight: 900, letterSpacing: '.10em', textTransform: 'uppercase' }}>{group.label}</span>{group.items.map((item) => <Link key={`${group.id}:${item.href}:${item.label}`} href={item.href} style={{ display: 'flex', gap: 10, alignItems: 'center', padding: 10, textDecoration: 'none', color: '#fff', fontWeight: 650, fontSize: 14, borderRadius: 12, background: 'rgba(255,255,255,.03)' }}><span>{item.icon}</span><span>{item.label}</span></Link>)}</div>)}
      </div> : null}
