...
test('public and owner direct edits delegate to canonical COS before browser semantic classifiers', () => {
  ...
  const branch = source.indexOf('if (directTextTransformation && !directTextHasAttachments)')
  const delegate = source.indexOf('cosPrimaryPost(req)', branch)
  const access = source.indexOf('const access = await getAccess()', branch)
  ...
  assert.ok(detect >= 0)
  assert.ok(branch > detect)
  assert.ok(delegate > branch)
  assert.ok(access > delegate)
  assert.ok(identity > delegate)
  assert.ok(visual > delegate)
  assert.doesNotMatch(source, /tryDirectTextTransformation/)
})
