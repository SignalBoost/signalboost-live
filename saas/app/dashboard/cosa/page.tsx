...
const copy = {
  title: uiText('generatedUi.u_9ad621dcc1ec669b'),
  intro: uiText('generatedUi.u_57346a0ad311a287'),
  refresh: "Refresh",
  loading: "Loading...",
...
function VideoBox({ campaign, busy, onRenderVideo, onCheckStatus }: { campaign: CampaignRow; busy: boolean; onRenderVideo: (id: string) => void; onCheckStatus: (id: string) => void }) {
  ...
  if (!video) return <button disabled={busy} onClick={() => onRenderVideo(campaign.id)} style={secondary}><LocalizedText fallback={uiText('generatedUi.u_30c929194235f50d')} /></button>
  ...
  return <div style={{ marginTop: 12, border: '1px solid rgba(26,240,255,.25)', borderRadius: 12, background: 'rgba(26,240,255,.06)', padding: 12 }}>
    {video.status === 'rendering' && <div><p style={{ color: CYAN, fontSize: 13, margin: 0 }}>{copy.rendering}</p><button disabled={busy} onClick={() => onCheckStatus(campaign.id)} style={{ ...secondary, marginTop: 8 }}><LocalizedText fallback={uiText('generatedUi.u_69cd3590300a68c6')} /></button></div>}
    {video.status === 'ready' && finalUrl && <div><p style={{ color: CYAN, fontSize: 12, margin: '0 0 8px' }}>{copy.brandedPreview}</p><p style={{ color: 'rgba(255,255,255,.55)', fontSize: 11, margin: '0 0 8px' }}>{uiText('generatedUi.u_930c60b3e9e8508e')}</p><video src={finalUrl} controls style={{ width: '100%', borderRadius: 10, maxHeight: 300, background: '#000' }} /></div>}
    {video.status === 'ready' && !finalUrl && <div><p style={{ color: GOLD, fontSize: 13, margin: 0, fontWeight: 900 }}><LocalizedText fallback={uiText('generatedUi.u_b9f42840e5dbdad7')} /></p><p style={{ color: 'rgba(255,255,255,.7)', fontSize: 12, margin: '8px 0 0' }}>{videoStatusText(video)}</p><p style={{ color: 'rgba(255,255,255,.5)', fontSize: 11, margin: '8px 0 0' }}>{uiText('generatedUi.u_bcd33856e0fd858b')}</p></div>}
    {video.status === 'failed' && <p style={{ color: '#fca5a5', fontSize: 13, margin: 0 }}>{uiText('generatedUi.u_e44d2be32f3e2047')}{String(video.error || "unknown error")}</p>}
...
function CampaignCard({ campaign, busy, onPatch, onGenerateDraft, onGenerateAll, onPublish, onRenderVideo, onCheckStatus }: { campaign: CampaignRow; busy: boolean; onPatch: (id: string, status: 'approved' | 'rejected' | 'queued') => void; onGenerateDraft: (id: string) => void; onGenerateAll: (id: string) => void; onPublish: (id: string, language?: string) => void; onRenderVideo: (id: string) => void; onCheckStatus: (id: string) => void }) {
  ...
  return <section style={{ ...panel, position: 'relative', overflow: 'hidden' }}>
    ...
    <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12 }}><div><strong style={{ color: '#fff' }}>{campaign.title || campaign.channel || uiText('generatedUi.u_268286d2ef7573e0')}</strong><p style={{ color: 'rgba(255,255,255,.48)', margin: '4px 0 0', fontSize: 12 }}>{campaign.channel || uiText('generatedUi.u_3dc260b2472062d9')} · {fmt(campaign.created_at)}</p></div><span style={{ color: GOLD, fontSize: 12, fontWeight: 900 }}>{campaign.status || uiText('generatedUi.u_62a2fed3d6e08c44')}</span></div>
    <p style={muted}>{short(campaign.objective, "No objective attached.")}</p>
    {drafts.map((item, idx) => <div key={item.id || idx} style={{ border: '1px solid rgba(255,195,0,.22)', borderRadius: 14, background: 'rgba(255,195,0,.06)', padding: 12, marginTop: 10 }}><p style={{ color: GOLD, fontSize: 11, fontWeight: 900, margin: 0 }}>{uiText('generatedUi.u_c6e5edcb50ebc66f')}{item.input?.language ? `· ${String(item.input.language).toUpperCase()}` : ''}</p><h3 style={{ color: '#fff', margin: '8px 0 0', fontSize: 15 }}>{item.output?.title || campaign.title}</h3>{item.output?.draft && <pre style={{ whiteSpace: 'pre-wrap', color: 'rgba(255,255,255,.78)', background: 'rgba(0,0,0,.25)', borderRadius: 12, padding: 10, fontSize: 12, lineHeight: 1.5, maxHeight: 220, overflow: 'auto' }}>{item.output.draft}</pre>}{item.output?.call_to_action && <p style={{ color: GOLD, fontWeight: 900 }}>{uiText('generatedUi.u_60735360aecaf19e')}{item.output.call_to_action}</p>}{canPublish && <button disabled={busy} onClick={() => onPublish(campaign.id, item.input?.language)} style={secondary}>{uiText('generatedUi.u_859390eb495b2ead')}</button>}</div>)}
    ...
    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 14, paddingTop: 12, borderTop: '1px solid rgba(255,255,255,.06)', flexWrap: 'wrap' }}>
      {waiting && <button disabled={busy} onClick={() => onPatch(campaign.id, 'rejected')} style={secondary}>{uiText('generatedUi.u_ab604a360777735f')}</button>}
      {waiting && (!['youtube', 'short_video'].includes(campaign.channel || '') || finalVideoReady) && <button disabled={busy} onClick={() => onPatch(campaign.id, 'approved')} style={primary}><LocalizedText fallback={uiText('generatedUi.u_891c935481e9bf91')} /></button>}
      {waiting && ['youtube', 'short_video'].includes(campaign.channel || '') && !finalVideoReady && <span style={{ color: 'rgba(255,255,255,.55)', fontSize: 12, alignSelf: 'center' }}>{uiText('generatedUi.u_45d56174578d34bf')}</span>}
      {!hasDraft(campaign) && <button disabled={busy} onClick={() => onGenerateDraft(campaign.id)} style={primary}><LocalizedText fallback={uiText('generatedUi.u_ca7191c8d91383b2')} /></button>}
      <button disabled={busy} onClick={() => onGenerateAll(campaign.id)} style={secondary}><LocalizedText fallback={uiText('generatedUi.u_6591048bccb7b4af')} /></button>
    </div>
  </section>
}
...
export default function MarketingSalesCosaPage() {
  ...
  return <main style={{ maxWidth: 1200, margin: '0 auto', display: 'grid', gap: 18 }}>
    <section style={{ ...panel, background: 'linear-gradient(145deg, rgba(15,23,42,.95), rgba(2,6,23,.97))' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}><div><p style={{ margin: 0, color: GOLD, fontSize: 12, fontWeight: 950, letterSpacing: '.12em', textTransform: 'uppercase' }}><LocalizedText fallback={uiText('generatedUi.u_c13043b6dbf74dcf')} /></p><h1 style={{ color: '#fff', margin: '10px 0 0', fontSize: 34 }}>{copy.title}</h1><p style={{ ...muted, maxWidth: 760 }}>{copy.intro}</p></div><button disabled={busy || loading} onClick={() => load()} style={secondary}>{loading ? copy.loading : copy.refresh}</button></div>
      <CosaCampaignConfigurator busy={busy} onSubmit={createStructuredCampaign} />
      <div style={{ color: 'rgba(255,255,255,.56)', fontSize: 12, marginTop: 12 }}>{uiText('generatedUi.u_6e293a8c009e0011')}{stats.waiting}{uiText('generatedUi.u_795b018e57721286')}{stats.approved}{uiText('generatedUi.u_e6ee8fc642e5d0ca')}{stats.running}{uiText('generatedUi.u_605a1c91aaa72225')}{stats.drafted}{uiText('generatedUi.u_0bb74b7591393b8d')}{outreach.length}</div>
