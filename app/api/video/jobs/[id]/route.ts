...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!JOB_ID_PATTERN.test(id)) {
    return NextResponse.json({ ok: false, data: null, error: 'Invalid video job id', meta: { locale: 'en', generatedAt: new Date().toISOString() } }, { status: 400 })
  }
  ...
  if (!data) {
    ...
    const relativePath = relative(queueDir, resultPath)
    ...
    if (existsSync(resultPath)) {
      try {
        ...
        if (!stats.isFile() || stats.size > MAX_RESULT_FILE_BYTES) {
          return NextResponse.json({ ok: false, data: null, error: 'Invalid video job result', meta: { locale: 'en', generatedAt: new Date().toISOString() } }, { status: 500 })
        }
