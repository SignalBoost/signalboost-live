...
test('the exact production incident: an expected lane whose flag is off is a fault, not an absence', () => {
  ...
  const status = classifyCosUniversityLane(expectation, {
    path: 'independent_exams', featureEnabled: false, verified: false,
...
test('graduation off during residence is correct and raises nothing', () => {
  ...
  const status = classifyCosUniversityLane(expectation, {
    path: 'graduation', featureEnabled: false, verified: false,
    executionBlocker: 'execution_evidence_missing',
  })
...
test('an enabled lane that produced no receipt is dark, separately from being disabled', () => {
  const status = classifyCosUniversityLane('expected_running', {
    path: 'continuous_learning', featureEnabled: true, verified: false,
    executionBlocker: 'execution_evidence_missing',
  })
...
test('runner_not_invoked is patience, not a fault — the A-range lanes clear themselves', () => {
  for (const path of ['subject_a_range_evidence', 'language_a_range_evidence', 'delayed_retention'] as const) {
    const status = classifyCosUniversityLane(
      ...
      { path, featureEnabled: true, verified: false, executionBlocker: 'runner_not_invoked' },
    )
...
test('the verified lanes report as running', () => {
  for (const path of ['registered_agent_cycle', 'continuous_learning', 'deliberate_practice'] as const) {
    const status = classifyCosUniversityLane(
      cosUniversityLaneExpectation({ path, timingByLevel: RESIDENCE }),
      { path, featureEnabled: true, verified: true, executionBlocker: null },
    )
...
test('paths for a program nobody is enrolled in owe no receipt', () => {
  for (const path of ['masters_learning', 'masters_exams', 'phd_runtime', 'phd_progress'] as const) {
    ...
    assert.equal(classifyCosUniversityLane(expectation, {
      path, featureEnabled: false, verified: false,
    }), 'absent_as_expected')
  }
})
...
test('PhD research running while nobody is enrolled in a PhD is staging drift, and is surfaced', () => {
  ...
  const status = classifyCosUniversityLane(gated, {
    path: 'phd_admission', featureEnabled: true, verified: true,
  })
...
test('fine tuning belongs to no calendar and is never reported dark', () => {
  ...
  assert.equal(expectation, 'expected_gated')
  assert.equal(classifyCosUniversityLane(expectation, {
    path: 'controlled_fine_tuning', featureEnabled: true, verified: false,
...
test('an undeclared path is never assumed healthy', () => {
  ...
  const status = classifyCosUniversityLane(expectation, {
    path: 'some_future_lane' as never, featureEnabled: true, verified: true,
  })
...
test('the whole Sep 13 board yields exactly one fault', () => {
  const board = [
    { path: 'registered_agent_cycle', featureEnabled: true, verified: true, executionBlocker: null },
    { path: 'continuous_learning', featureEnabled: true, verified: true, executionBlocker: null },
    { path: 'deliberate_practice', featureEnabled: true, verified: true, executionBlocker: null },
    { path: 'independent_exams', featureEnabled: false, verified: false, executionBlocker: 'execution_evidence_missing' },
    { path: 'subject_a_range_evidence', featureEnabled: true, verified: false, executionBlocker: 'runner_not_invoked' },
    { path: 'language_a_range_evidence', featureEnabled: true, verified: false, executionBlocker: 'runner_not_invoked' },
    { path: 'delayed_retention', featureEnabled: true, verified: false, executionBlocker: 'runner_not_invoked' },
    { path: 'graduation', featureEnabled: false, verified: false, executionBlocker: 'execution_evidence_missing' },
    { path: 'masters_learning', featureEnabled: false, verified: false, executionBlocker: 'execution_evidence_missing' },
    { path: 'masters_admission', featureEnabled: false, verified: false, executionBlocker: 'execution_evidence_missing' },
    { path: 'masters_exams', featureEnabled: false, verified: false, executionBlocker: 'execution_evidence_missing' },
    { path: 'masters_progress', featureEnabled: false, verified: false, executionBlocker: 'execution_evidence_missing' },
    { path: 'phd_runtime', featureEnabled: false, verified: false, executionBlocker: 'execution_evidence_missing' },
    { path: 'phd_admission', featureEnabled: false, verified: false, executionBlocker: 'execution_evidence_missing' },
    { path: 'phd_progress', featureEnabled: false, verified: false, executionBlocker: 'execution_evidence_missing' },
    { path: 'phd_research', featureEnabled: true, verified: true, executionBlocker: null },
    { path: 'phd_methodology_exams', featureEnabled: true, verified: true, executionBlocker: null },
    { path: 'controlled_fine_tuning', featureEnabled: true, verified: false, executionBlocker: 'runner_not_invoked' },
  ] as const
