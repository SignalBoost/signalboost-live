...
export function mapSupervisorStepsToBrowserRuntimeTask(input: { incident: SupervisorIncident; plan: RepairPlan; approvedSteps: RepairStep[]; targetOrigin: string; dispatchId: string; createdAt: string }): { browserTask: BrowserTask; stepMapping: BrowserRuntimeStepMapping[]; approvalRequirements: BrowserRuntimeApprovalRequirements; verificationRequirements: BrowserRuntimeVerificationRequirements } {
  ...
  const browserTask: BrowserTask = { taskId: `${input.dispatchId}:${input.plan.planId}:dry-run`, incidentId: input.incident.incidentId, provider: input.plan.targetProvider, adapterId: 'signalboost.browser-runtime.dry-run.v1', mode: 'prepare_change', issuedAt: input.createdAt, expiresAt: new Date(Date.parse(input.createdAt) + 15 * 60_000).toISOString(), allowedOrigins: [input.targetOrigin], steps, approvalToken: 'DRY_RUN_NO_RUNTIME_APPROVAL_TOKEN_CREATED', metadata: { dryRunOnly: true, supervisorPolicyApprovalDoesNotGrantRuntimeApproval: true } }
