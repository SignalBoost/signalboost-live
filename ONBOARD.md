...
**Concierge public prompt-exfiltration boundary:** corrective release in progress. The first deployment (`ea4ee219…`) correctly guarded `/api/concierge`, but the retest used the shared `/api/support` ingress and exposed that endpoint bypass. The same deterministic input and output boundary is now being applied to Support, with a regression that proves it executes before legacy model dispatch. Do not call the guard Production-accepted until this corrected deployment is READY and original attacks return the opaque refusal through the real Concierge UI.    
