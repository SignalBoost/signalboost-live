...
test('owner topology never substitutes model or provider names when runtime facts are absent', () => {
  ...
  try {
    ...
    for (const key of keys) assert.match(context, new RegExp(`${key} is not configured in this runtime`))
    assert.equal(/Qwen\//.test(context), false)
    assert.equal(/DeepSeek/.test(context), false)
    assert.equal(/BAAI\//.test(context), false)
    assert.equal(/Managed inference provider: deepinfra/i.test(context), false)
  } finally {
