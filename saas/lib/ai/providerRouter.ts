...
async function callClaude(args: ModelCallArgs): Promise<string | null> {
  try {
    ...
    if (!apiKey) { console.error('providerRouter: ANTHROPIC_API_KEY missing'); return null }
...
async function callOpenAI(args: ModelCallArgs): Promise<string | null> {
  try {
    ...
    if (!apiKey) {
      console.warn('providerRouter: OPENAI_API_KEY missing — falling back to Claude')
    ...
    if (!response.ok) {
      ...
      console.warn('providerRouter: OpenAI failed — falling back to Claude')
      return callClaude(args)
    }
    ...
    console.error('providerRouter: OpenAI exception — falling back to Claude', err)
    return callClaude(args)
  }
}
