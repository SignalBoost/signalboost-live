...
async function recoverCompletedTurn(conversationId: string, userContent: string, sentAtMs: number): Promise<string | null> {
  ...
  for (const delayMs of RECOVERY_POLL_DELAYS_MS) {
    ...
    try {
      ...
      const recovered = findRecoveredAssistantReply(Array.isArray(data?.messages) ? data.messages : [], userContent, sentAtMs)
...
export default function AssistantPage() {
  ...
  function removeHandoffQuery() {
    const url = new URL(window.location.href)
    let changed = false
    for (const key of ['prompt', 'conversation', 'draft']) {
  ...
  function clearComposer(removeQuery = false) {
    setInput('')
    collapseComposer()
    if (removeQuery) removeHandoffQuery()
  }
  ...
  useEffect(() => {
    ...
    const explicitDraft = params.get('draft') === '1'
    if (prompt && explicitDraft) {
      setInput(prompt.slice(0, 8000))
      return
    }

    // Legacy Concierge links used ?prompt=<entire old request>, which caused the old
    // request to reappear in the composer. Plain prompt handoffs are no longer accepted;
    // callers that intentionally want a draft must opt in with draft=1.
    if (prompt) removeHandoffQuery()
  }, [])
  ...
  const suggestions = [c(COPY.suggestions.s1, l), c(COPY.suggestions.s2, l), c(COPY.suggestions.s3, l), c(COPY.suggestions.s4, l)]
  ...
  async function processFiles(fileList: FileList | File[]) {
    ...
    for (const file of files) {
      if (!ALLOWED_MIME.includes(file.type)) { setFileError(c(COPY.fileTypeErr, l)); continue }
      if (file.size > MAX_FILE_BYTES) { setFileError(c(COPY.fileTooLarge, l)); continue }
      try {
        const dataUrl = await readFileAsDataUrl(file)
        results.push({ id: crypto.randomUUID(), name: file.name, size: file.size, mimeType: file.type, dataUrl, isImage: file.type.startsWith('image/') })
      } catch {}
    }
    if (results.length > 0) setStagedFiles(prev => [...prev, ...results])
  }

  function onDragOver(e: DragEvent<HTMLDivElement>) { e.preventDefault(); e.stopPropagation(); setDragOver(true) }
  function onDragLeave(e: DragEvent<HTMLDivElement>) { e.preventDefault(); e.stopPropagation(); setDragOver(false) }
  function onDrop(e: DragEvent<HTMLDivElement>) { e.preventDefault(); e.stopPropagation(); setDragOver(false); if (e.dataTransfer?.files?.length) processFiles(e.dataTransfer.files) }
  function onFileInputChange(e: ChangeEvent<HTMLInputElement>) { if (e.target.files?.length) { processFiles(e.target.files); e.target.value = '' } }
  function removeFile(id: string) { setStagedFiles(prev => prev.filter(f => f.id !== id)) }
  ...
  async function send(text: string) {
    ...
    try {
      const apiMessages = next.map((m, i) => i === next.length - 1 && m.role === 'user' && fileLabel ? { role: m.role, content } : m)
      const attachments = filesToSend.map(f => ({ name: f.name, mimeType: f.mimeType, size: f.size, dataUrl: f.dataUrl }))
      ...
      try {
        const res = await fetch('/api/concierge', {
          ...
          body: JSON.stringify({ messages: apiMessages, attachments: attachments.length ? attachments : undefined, context: { language: lang, currentPage: '/dashboard/assistant', conversationId } }),
        })
        ...
        if (!directReply) {
          ...
          if (recovered) { setMessages([...next, { role: 'assistant', content: recovered }]); return }
        }
        ...
        setMessages([...next, { role: 'assistant', content: directReply || fallback }])
      } catch (err: any) {
  ...
  async function submitFeedback(messageIndex: number, assistantContent: string, feedbackType: FeedbackKind, correctionText?: string) {
    ...
    try {
      const response = await fetch('/api/assistant/feedback', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ conversationId, assistantContent, feedbackType, correctionText }) })
  ...
  function stopRequest() { abortRef.current?.abort() }

  async function openHistory() {
    setHistoryOpen(true); setHistoryLoading(true); setHistoryError(false)
    try {
      const res = await fetch('/api/assistant/chats')
      if (!res.ok) throw new Error('history load failed')
      const data = await res.json()
      setConversations(Array.isArray(data?.conversations) ? data.conversations : [])
    } catch { setHistoryError(true); setConversations([]) } finally { setHistoryLoading(false) }
  }

  async function loadConversation(id: string, fromHandoff = false) {
    setHistoryLoading(true)
    try {
      const res = await fetch(`/api/assistant/chats?id=${encodeURIComponent(id)}`, { cache: 'no-store' })
      ...
      clearComposer(fromHandoff)
      setHistoryOpen(false)
    } catch {
      setHistoryError(true)
      if (fromHandoff) { conversationIdRef.current = ''; setMessages([]); clearComposer(true) }
  ...
  function formatDate(iso: string): string {
    try { return new Date(iso).toLocaleDateString(DATE_LOCALES[l], { month: 'short', day: 'numeric' }) } catch { return '' }
  }

  const canSend = !loading && (input.trim().length > 0 || stagedFiles.length > 0)

  return (
    <div onDragOver={onDragOver} onDragLeave={onDragLeave} onDrop={onDrop} style={{ position: 'relative', display: 'flex', flexDirection: 'column', height: 'calc(100vh - 165px)', minHeight: 480, maxWidth: 1280, margin: '0 auto', padding: '24px 28px', width: '100%', boxSizing: 'border-box', color: 'var(--text-primary)' }}>
      {dragOver && <div style={{ position: 'absolute', inset: 0, zIndex: 20, borderRadius: 24, background: 'rgba(3,7,18,.82)', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)', border: '2px dashed rgba(26,240,255,.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' }}><div style={{ textAlign: 'center' }}><div style={{ fontSize: 40 }}>📎</div><p style={{ color: 'rgba(26,240,255,.9)', fontSize: 16, fontWeight: 700, marginTop: 10 }}>{c(COPY.dropHere, l)}</p></div></div>}
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
          <div style={{ minWidth: 0 }}>
            ...
            {cosStatus && (() => {
              ...
              return <div style={{ marginTop: 10 }}><span title={cosStatus.detail} style={{ display: 'inline-flex', alignItems: 'center', padding: '5px 12px', borderRadius: 999, background: bg, border: `1px solid ${accent}`, color: accent, fontSize: 12, fontWeight: 900, letterSpacing: '.02em' }}>{label}</span>{!ok && <p style={{ color: accent, fontSize: 11.5, margin: '6px 0 0', lineHeight: 1.5, maxWidth: 620 }}>{cosStatus.detail}</p>}</div>
            })()}
          </div>
      ...
      <div style={{ position: 'relative', flex: 1, minHeight: 0, display: 'flex' }}>
        <div ref={threadRef} style={{ flex: 1, overflowY: 'auto', minHeight: 0, background: 'linear-gradient(145deg, rgba(15,23,42,.78), rgba(3,7,18,.68))', border: '1px solid rgba(255,255,255,.1)', borderRadius: 22, padding: 18, display: 'flex', flexDirection: 'column', gap: 12 }}>
          {messages.length === 0 && !loading && <div style={{ margin: 'auto', textAlign: 'center', maxWidth: 520 }}><div style={{ fontSize: 40 }}>✨</div><p style={{ color: 'rgba(255,255,255,.6)', fontSize: 14, marginTop: 8, lineHeight: 1.6 }}>{c(COPY.empty, l)}</p><div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, justifyContent: 'center', marginTop: 14 }}>{suggestions.map(s => <button key={s} onClick={() => send(s)} className="sb-button-secondary" style={{ fontSize: 12, padding: '9px 14px' }}>{s}</button>)}</div></div>}
          ...
          {messages.map((msg, i) => (
            <div key={`${msg.role}-${i}`} style={{ display: 'flex', justifyContent: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
              <div style={{ maxWidth: msg.role === 'user' ? '80%' : '100%', width: msg.role === 'assistant' ? '100%' : 'auto', padding: '12px 16px', borderRadius: 16, borderTopRightRadius: msg.role === 'user' ? 4 : 16, borderTopLeftRadius: msg.role === 'user' ? 16 : 4, background: msg.role === 'user' ? 'rgba(255,195,0,.12)' : 'rgba(26,240,255,.07)', border: `1px solid ${msg.role === 'user' ? 'rgba(255,195,0,.28)' : 'rgba(26,240,255,.2)'}`, color: '#fff', fontSize: 14, lineHeight: 1.7, whiteSpace: msg.role === 'user' ? 'pre-wrap' : 'normal' }}>
                {msg.role === 'assistant' ? <><VideoJsonMessage content={msg.content} />{(() => {
                  ...
                  return <div style={{ marginTop: 10, paddingTop: 8, borderTop: '1px solid rgba(255,255,255,.08)' }}><div style={{ display: 'flex', gap: 7, flexWrap: 'wrap', alignItems: 'center' }}><button type="button" disabled={busy || feedback.status === 'saved'} onClick={() => submitFeedback(i, msg.content, 'positive')} style={{ padding: '5px 9px', borderRadius: 8, border: '1px solid rgba(34,197,94,.32)', background: 'rgba(34,197,94,.08)', color: '#86efac', cursor: busy ? 'wait' : 'pointer', fontSize: 11.5 }}>👍 {uiText('assistantFeedback.helpful')}</button><button type="button" disabled={busy || feedback.status === 'saved'} onClick={() => submitFeedback(i, msg.content, 'negative')} style={{ padding: '5px 9px', borderRadius: 8, border: '1px solid rgba(248,113,113,.32)', background: 'rgba(248,113,113,.08)', color: '#fca5a5', cursor: busy ? 'wait' : 'pointer', fontSize: 11.5 }}>👎 {uiText('assistantFeedback.notHelpful')}</button><button type="button" disabled={busy || feedback.status === 'saved'} onClick={() => setFeedbackByMessage(prev => ({ ...prev, [i]: { ...(prev[i] || { status: 'idle' }), correctionOpen: true, correction: prev[i]?.correction || '' } }))} style={{ padding: '5px 9px', borderRadius: 8, border: '1px solid rgba(26,240,255,.28)', background: 'rgba(26,240,255,.06)', color: '#67e8f9', cursor: busy ? 'wait' : 'pointer', fontSize: 11.5 }}>✎ {uiText('assistantFeedback.correctThis')}</button>{feedback.status === 'saved' && <span style={{ color: '#86efac', fontSize: 11.5 }}>{uiText('assistantFeedback.feedbackSaved')}</span>}{feedback.status === 'error' && <span style={{ color: '#fca5a5', fontSize: 11.5 }}>{uiText('assistantFeedback.feedbackError')}</span>}</div>{feedback.correctionOpen && feedback.status !== 'saved' && <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 7 }}><textarea value={correction} onChange={e => setFeedbackByMessage(prev => ({ ...prev, [i]: { ...(prev[i] || { status: 'idle' }), correctionOpen: true, correction: e.target.value } }))} placeholder={uiText('assistantFeedback.correctionPlaceholder')} rows={3} maxLength={4000} style={{ width: '100%', boxSizing: 'border-box', borderRadius: 10, border: '1px solid rgba(255,255,255,.16)', background: 'rgba(3,7,18,.55)', color: '#fff', padding: '9px 10px', fontSize: 12, resize: 'vertical' }} /><div style={{ display: 'flex', gap: 7 }}><button type="button" disabled={busy || !correction.trim()} onClick={() => submitFeedback(i, msg.content, 'correction', correction)} style={{ padding: '6px 10px', borderRadius: 8, border: '1px solid rgba(26,240,255,.4)', background: 'rgba(26,240,255,.12)', color: '#67e8f9', cursor: busy ? 'wait' : 'pointer', fontSize: 11.5, fontWeight: 700 }}>{uiText('assistantFeedback.submitCorrection')}</button><button type="button" disabled={busy} onClick={() => setFeedbackByMessage(prev => ({ ...prev, [i]: { ...(prev[i] || { status: 'idle' }), correctionOpen: false } }))} style={{ padding: '6px 10px', borderRadius: 8, border: '1px solid rgba(255,255,255,.14)', background: 'transparent', color: 'rgba(255,255,255,.65)', cursor: 'pointer', fontSize: 11.5 }}>{uiText('assistantFeedback.cancelCorrection')}</button></div></div>}</div>
                })()}</> : msg.content}
              </div>
            </div>
          ))}

          {loading && <div style={{ display: 'flex', justifyContent: 'flex-start' }}><div style={{ padding: '12px 16px', borderRadius: 16, borderTopLeftRadius: 4, background: 'rgba(26,240,255,.07)', border: '1px solid rgba(26,240,255,.2)', color: 'rgba(255,255,255,.5)', fontSize: 14 }}>{c(COPY.thinking, l)}</div><button type="button" onClick={stopRequest} style={{ marginLeft: 10, alignSelf: 'center', padding: '6px 12px', borderRadius: 10, border: '1px solid rgba(255,255,255,.2)', background: 'transparent', color: 'rgba(255,255,255,.7)', fontSize: 13, cursor: 'pointer' }}>{c(COPY.stop, l)}</button></div>}
        </div>

        {historyOpen && <div style={{ position: 'absolute', top: 0, left: 0, bottom: 0, width: 'min(320px, 88%)', zIndex: 5, background: 'linear-gradient(160deg, rgba(10,16,32,.97), rgba(3,7,18,.97))', border: '1px solid rgba(26,240,255,.25)', borderRadius: 22, display: 'flex', flexDirection: 'column', gap: 10, boxShadow: '0 18px 50px rgba(0,0,0,.55)', overflow: 'hidden' }}><div style={{ position: 'sticky', top: 0, zIndex: 3, background: 'linear-gradient(160deg, rgba(10,16,32,.99), rgba(3,7,18,.99))', padding: '14px 14px 10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0, borderBottom: '1px solid rgba(255,255,255,.07)' }}><span style={{ fontSize: 13, fontWeight: 800, letterSpacing: '.02em', color: 'rgba(26,240,255,.9)' }}>🕘 {c(COPY.history, l)}</span><button onClick={() => setHistoryOpen(false)} className="sb-button-secondary" style={{ fontSize: 11, padding: '6px 10px' }}>{c(COPY.close, l)}</button></div><div style={{ flex: 1, overflowY: 'auto', minHeight: 0, padding: '0 14px 14px', display: 'flex', flexDirection: 'column', gap: 8 }}>{historyLoading && <p style={{ color: 'rgba(255,255,255,.5)', fontSize: 13, textAlign: 'center', marginTop: 20 }}>{c(COPY.loadingHistory, l)}</p>}{!historyLoading && historyError && <p style={{ color: 'rgba(255,140,140,.8)', fontSize: 13, textAlign: 'center', marginTop: 20 }}>{c(COPY.historyError, l)}</p>}{!historyLoading && !historyError && conversations.length === 0 && <p style={{ color: 'rgba(255,255,255,.5)', fontSize: 13, textAlign: 'center', marginTop: 20 }}>{c(COPY.noHistory, l)}</p>}{!historyLoading && conversations.map(conv => <div key={conv.id} style={{ display: 'flex', alignItems: 'stretch', gap: 6, background: conversationIdRef.current === conv.id ? 'rgba(26,240,255,.1)' : 'rgba(255,255,255,.04)', border: `1px solid ${conversationIdRef.current === conv.id ? 'rgba(26,240,255,.35)' : 'rgba(255,255,255,.1)'}`, borderRadius: 14, padding: '10px 12px' }}><button onClick={() => loadConversation(conv.id)} style={{ flex: 1, minWidth: 0, background: 'none', border: 'none', padding: 0, cursor: 'pointer', textAlign: 'left', color: '#fff' }}><div style={{ fontSize: 13, fontWeight: 700, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{conv.title || c(COPY.untitled, l)}</div>{conv.summary ? <div style={{ fontSize: 11, color: 'rgba(255,255,255,.5)', marginTop: 3, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{conv.summary}</div> : null}<div style={{ fontSize: 10, color: 'rgba(26,240,255,.6)', marginTop: 4 }}>{formatDate(conv.updated_at)} · {conv.message_count}</div></button><button onClick={() => deleteConversation(conv.id)} title={c(COPY.deleteConfirm, l)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(255,140,140,.7)', fontSize: 14, padding: '0 2px', flexShrink: 0 }}>🗑</button></div>)}</div></div>}
      </div>

      <div style={{ marginTop: 12, flexShrink: 0 }}>
        {fileError && <div style={{ marginBottom: 8, padding: '8px 14px', borderRadius: 10, background: 'rgba(255,80,80,.12)', border: '1px solid rgba(255,80,80,.35)', color: 'rgba(255,180,180,.9)', fontSize: 12, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}><span>⚠️ {fileError}</span><button onClick={() => setFileError('')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(255,180,180,.7)', fontSize: 14, padding: 0, lineHeight: 1 }}>✕</button></div>}
        {stagedFiles.length > 0 && <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 8, maxHeight: 120, overflowY: 'auto', padding: '2px 0' }}>{stagedFiles.map(f => <div key={f.id} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(26,240,255,.08)', border: '1px solid rgba(26,240,255,.25)', borderRadius: 10, padding: '5px 10px 5px 6px', maxWidth: 200, minWidth: 0 }}>{f.isImage ? <img src={f.dataUrl} alt={f.name} style={{ width: 28, height: 28, objectFit: 'cover', borderRadius: 6, flexShrink: 0 }} /> : <span style={{ fontSize: 18, flexShrink: 0 }}>📄</span>}<div style={{ minWidth: 0, flex: 1 }}><div style={{ fontSize: 11, fontWeight: 700, color: '#fff', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{f.name}</div><div style={{ fontSize: 10, color: 'rgba(255,255,255,.45)' }}>{formatBytes(f.size)}</div></div><button onClick={() => removeFile(f.id)} title={uiText('generatedUi.u_c3812fc4acb861d5')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(255,255,255,.5)', fontSize: 13, padding: 0, lineHeight: 1, flexShrink: 0 }}>✕</button></div>)}</div>}

        <div style={{ display: 'flex', gap: 10, alignItems: 'flex-end' }}>
          <input ref={fileInputRef} type="file" multiple accept={ALLOWED_MIME.join(',')} onChange={onFileInputChange} style={{ display: 'none' }} aria-hidden="true" />
          <button onClick={() => fileInputRef.current?.click()} disabled={loading} title={uiText('generatedUi.u_e697cc1e45afa541')} aria-label={uiText('generatedUi.u_e697cc1e45afa541')} style={{ flexShrink: 0, width: 42, height: 42, borderRadius: 12, background: stagedFiles.length > 0 ? 'rgba(26,240,255,.15)' : 'rgba(255,255,255,.06)', border: `1px solid ${stagedFiles.length > 0 ? 'rgba(26,240,255,.45)' : 'rgba(255,255,255,.15)'}`, cursor: loading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, opacity: loading ? 0.5 : 1, transition: 'background .15s, border-color .15s', color: stagedFiles.length > 0 ? '#1af0ff' : 'rgba(255,255,255,.7)' }}>📎</button>
          <textarea ref={composerRef} className="sb-input" value={input} onChange={e => { setInput(e.target.value); e.target.style.height = 'auto'; e.target.style.height = Math.min(e.target.scrollHeight, 200) + 'px' }} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); send(input) } }} placeholder={c(COPY.placeholder, l)} rows={1} style={{ flex: 1, padding: '13px 16px', borderRadius: 14, fontSize: 14, resize: 'none', lineHeight: 1.4, maxHeight: 200, overflowY: 'auto', fontFamily: 'inherit' }} disabled={loading} />
          <button onClick={() => send(input)} disabled={!canSend} className="sb-button-primary" style={{ padding: '0 24px', height: 42, borderRadius: 14, opacity: canSend ? 1 : 0.6, cursor: loading ? 'wait' : 'pointer', flexShrink: 0 }}>{c(COPY.send, l)}</button>
        </div>
      </div>
    </div>
  )
