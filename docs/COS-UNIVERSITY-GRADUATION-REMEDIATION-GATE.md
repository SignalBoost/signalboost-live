...
The additive database migration was applied successfully as `20260911194828`; the repository migration filename matches that exact applied version. At `2026-09-11T19:49:27.462538Z`, both new triggers were enabled. The live host read counted 23 pending COS remediation plans and one Software Specialist remediation plan; the credential table still contained zero rows.

A transaction-scoped database probe exercised all three remediation sources and all three pending statuses. All nine credential attempts were denied by the expected check violation. Terminal plans, normal study rotation, another identity and a Master's plan were excluded correctly. A 26-plan probe returned a complete count of 26 and only 25 diagnostic IDs. Browser RPC execution was denied and service-role execution remained allowed. All probe plans were rolled back; a subsequent query confirmed zero remaining probe plans and zero probe credentials. These are database contract probes, not academic results or Production route receipts, and they do not constitute a concurrent stress test.

Full current-head CI and Preview must pass before merge. Confirm the exact merged commit in Production and collect a real route receipt for this new guard; the earlier #2125 receipt, unit tests, SQL probes and READY status do not establish execution of the new code. Never manufacture learner credits or assurance receipts.
