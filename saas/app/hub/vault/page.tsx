  ...
  return (
    ...
    >
      ...
      <header style={{ position: 'sticky', top: 0, zIndex: 25, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 14, flexWrap: 'wrap', marginBottom: 18, padding: '10px 0 14px', background: 'linear-gradient(180deg, rgba(11,18,32,.98), rgba(11,18,32,.72) 70%, transparent)', backdropFilter: 'blur(10px)' }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 'clamp(24px, 3vw, 34px)', fontWeight: 900, letterSpacing: '-.03em', background: 'linear-gradient(90deg, #fff 30%, #1af0ff 75%, #ffc300 100%)', WebkitBackgroundClip: 'text', backgroundClip: 'text', color: 'transparent' }}>{uiText('generatedUi.u_1411cfe893444c39')}</h1>
          <p style={{ margin: '4px 0 0', fontSize: 13.5, color: 'rgba(255,255,255,.58)' }}>{uiText('generatedUi.u_453176f25a244972')}</p>
        </div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          <a href="/hub" className="hub-chip" style={{ padding: '8px 13px', borderRadius: 10, border: '1px solid rgba(255,255,255,.12)', background: 'rgba(255,255,255,.04)', color: 'rgba(255,255,255,.72)', textDecoration: 'none', fontSize: 12.5, fontWeight: 800 }}>{uiText('generatedUi.u_1641d242a6af76cc')}</a>
          <a href="/hub/providers" className="hub-chip" style={{ padding: '8px 13px', borderRadius: 10, border: '1px solid rgba(255,255,255,.12)', background: 'rgba(255,255,255,.04)', color: 'rgba(255,255,255,.72)', textDecoration: 'none', fontSize: 12.5, fontWeight: 800 }}>{uiText('generatedUi.u_2cdda41fd070970a')}</a>
          {LANGS.map(l => (
