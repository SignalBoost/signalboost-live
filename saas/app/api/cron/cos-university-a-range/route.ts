...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    for (const agent of agents) {
      ...
      if (!programGate.allowed) {
        if (programGate.reason === 'service_database_unavailable') {
          await recordCosUniversityProductionPath({ path: 'subject_a_range_evidence', invocationSucceeded: false, evidence: { skipped: true, agentId: agent.agentId, programGate } })
      ...
      const cadence = await readCosUniversityDailyLaneCadence('subject_a_range_evidence', now, agent.agentId)
      if (!cadence.due) {
        notDue.push({ agentId: agent.agentId, cadence })
        continue
      }
      const result = await runCosUniversityARangeBatch({ now, agentId: agent.agentId })
      await recordCosUniversityProductionPath({ path: 'subject_a_range_evidence', invocationSucceeded: result.errors.length === 0, evidence: { ...result, agentId: agent.agentId, programGate } })
    ...
    if (!notDue.length) {
      ...
      await recordCosUniversityProductionPath({ path: 'subject_a_range_evidence', invocationSucceeded: true, evidence: { skipped: true, agents: gated } })
      return NextResponse.json({ ok: true, skipped: true, agents: gated })
    }
    const cadence = { agents: notDue, gated }
    await recordCosUniversityProductionPath({ path: 'subject_a_range_evidence', invocationSucceeded: true, evidence: { dailyCadence: 'not_due', runnerInvoked: false, cadence } })
