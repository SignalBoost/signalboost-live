...
**Concierge internal Builder:** implementation is active and not Production-accepted. Concierge is the only user-facing surface: explicit coding, repair, and design objectives invoke the authenticated Builder tool loop in the backend and return its answer, evidence, and file-download links in the same conversation. Queued Builder work is followed by job id through the authoritative read-only `/api/builder` status endpoint to terminal output instead of treating the initial running response as final. Text/code results render inline with a copy control as well as a download link. Passive logs retain no execution authority, but diagnostic questions containing logs continue through ordinary COS instead of ending at the obsolete canned “not editable source” reply. Ordinary questions remain on normal COS. The legacy redirect to `/dashboard/developer` is removed from the authenticated Assistant; supported text/code attachments are staged into the isolated network-denied workspace. While either public Concierge or owner COS is waiting, the UI renders accepted/running/completed request events and elapsed time. This is operational progress visibility, not disclosure of private chain-of-thought. The Developer workspace may remain an internal diagnostics surface, not a required customer journey. A fresh authenticated Concierge code-repair observation, including inline copy rendering, is required before runtime acceptance.
**Concierge artifacts:** implementation is active and not Production-accepted. Every authenticated public Concierge request that explicitly asks for a `.txt` or text-based `.pdf` is routed to a backend artifact tool; it persists the file in the private workspace and returns a same-chat download link. A format follow-up after Builder is bound to the actual user-scoped Builder file (or, when no file exists, the preceding assistant result) so “give me that as TXT/PDF” does not lose conversation context or regenerate a generic substitute. It never sends the document or treats a request for research as permission to invent factual content. A fresh authenticated PDF and TXT conversion/download observation is required before runtime acceptance.
