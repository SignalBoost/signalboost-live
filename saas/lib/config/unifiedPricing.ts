// PASTE THIS IDENTICAL CONTENT INTO BOTH FILES:
//   saas/lib/config/unifiedPricing.ts
//   saas/lib/config/unifiedPricingConfig.ts
// They are kept identical on purpose: the storefront imports the first name and
// the copy file imports the second, so having the same registry in both means no
// import can break regardless of which file is which. (Single source can be
// reunified later once the build is stable.)
//
