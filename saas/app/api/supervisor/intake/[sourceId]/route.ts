...
export async function POST(req: NextRequest, ctx: { params: Promise<{ sourceId: string }> }) {
  ...
  if (result.status === 'rejected') {
    return NextResponse.json({ ok: false, status: 'rejected', reason: result.reason }, { status: statusForRejection(result.reason) })
  }
  if (result.status === 'ignored') {
    return NextResponse.json({ ok: true, status: 'ignored', reason: result.reason }, { status: 200 })
  }
  if (result.status === 'duplicate') {
    return NextResponse.json({ ok: true, status: 'duplicate', incidentId: result.duplicateOf }, { status: 200 })
  }
  ...
  return NextResponse.json({
    ok: true,
    status: 'accepted',
    incidentId: result.record.incidentId,
...
export async function GET(_req: NextRequest, ctx: { params: Promise<{ sourceId: string }> }) {
  ...
  return NextResponse.json({
    ...
    sourceId,
    method: 'POST',
    note: 'Send incidents with POST. See the incident intake guide for the payload and signing scheme.',
  }, { status: 200 })
}
