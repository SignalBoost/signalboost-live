...
export async function executeConciergeLanguageAcceptanceCase(test: ConciergeLanguageAcceptanceCase): Promise<ConciergeLanguageAcceptanceExecution> {
  ...
  if (test.mode === 'deterministic_fallback') {
    ...
    const result = await withPublicDeliveryScope(() => tryCOSFirstAnswer({ prompt: test.prompt, language: test.language, privileged: false, disableCache: true, previousAssistant: null, userId: null }))
  ...
  const verdicts = evaluateLanguageAcceptanceText({ test, reply, handled, responseSource, localModelInvoked, externalAiInvoked, latencyMs })
  return { test, reply, responseSource, localModelInvoked, externalAiInvoked, latencyMs, nativeReviewConfidence, nativeReviewerUsed, verdicts, passed: Object.values(verdicts).every(Boolean) }
}
