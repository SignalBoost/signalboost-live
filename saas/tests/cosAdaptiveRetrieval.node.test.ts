...
test('adaptive retrieval does not propose a policy below the distinct-case sample gate', () => {
  const candidate = deriveAdaptiveRetrievalCandidate([row('a'), row('b'), row('c'), row('d'), row('e')], 0.45)
...
test('high context waste with preserved quality proposes only a 6 to 4 shadow cap', () => {
  const candidate = deriveAdaptiveRetrievalCandidate([row('a'), row('b'), row('c'), row('d'), row('e'), row('f', false)], 0.45)
...
test('repeated runs of one case count once rather than inflating the training breadth', () => {
  const duplicate = row('a'); duplicate.turnId = 'newest-a'
  const older = row('a'); older.turnId = 'older-a'
  const candidate = deriveAdaptiveRetrievalCandidate([duplicate, older, row('b'), row('c'), row('d'), row('e'), row('f')], 0.45)
  assert.equal(candidate.metrics.distinctCases, 6)
...
test('a weak verified success rate blocks an adaptive retrieval candidate', () => {
  const candidate = deriveAdaptiveRetrievalCandidate([row('a', false), row('b', false), row('c'), row('d'), row('e'), row('f')], 0.45)
  assert.equal(candidate.eligible, false)
...
test('validation excludes training cases and prefers a domain absent from training', () => {
  const cases = [
    { id: 'sre-a', domain: 'sre' }, { id: 'sre-b', domain: 'sre' },
    { id: 'postgres-a', domain: 'postgres' }, { id: 'cloud-a', domain: 'cloud' },
    { id: 'network-a', domain: 'networking' },
  ]
  const selected = selectAdaptiveRetrievalValidationCase({ cases, trainingCaseIds:['sre-a','postgres-a'], priorValidationCaseIds:[] })
  assert.equal(selected?.id, 'cloud-a')
})

test('second validation prefers another unused domain', () => {
  const cases = [
    { id:'sre-a', domain:'sre' }, { id:'postgres-a', domain:'postgres' },
    { id:'cloud-a', domain:'cloud' }, { id:'cloud-b', domain:'cloud' },
    { id:'network-a', domain:'networking' },
  ]
  const selected = selectAdaptiveRetrievalValidationCase({ cases, trainingCaseIds:['sre-a','postgres-a'], priorValidationCaseIds:['cloud-a'] })
...
test('null shadow cap means no override rather than zero injected context', async () => {
