...
test('extracts exact incident and execution correlations from governed JSON context', () => {
  const refs = extractCouncilCorrelationRefs(JSON.stringify({ incident_id: 'incident-prod-123', traceId: 'trace-456', execution_id: 'exec-789' }))
...
test('explicit deterministic read-back verification is success', () => {
  const result = classifyDeterministicToolOutcome({ ok: true, result: { verified: true, changed: true, policyInstanceId: 'vercel-observation-cron' } })
...
test('successful execution without verification remains observed rather than pretending recovery', () => {
  assert.equal(classifyDeterministicToolOutcome({ ok: true, result: { accepted: true } }).status, 'observed')
})

test('explicit deterministic verification failure is failure', () => {
  assert.equal(classifyDeterministicToolOutcome({ ok: true, result: { verified: false } }).status, 'failure')
})

test('model references cannot masquerade as objective outcome sources', () => {
  assert.throws(() => normalizeCouncilObjectiveOutcome({
    sourceClass: 'deterministic_tool', sourceRef: 'model:qwen-self-grade',
    correlation: { kind: 'incident_id', value: 'incident-prod-123' }, outcomeStatus: 'success', summary: 'self reported success',
