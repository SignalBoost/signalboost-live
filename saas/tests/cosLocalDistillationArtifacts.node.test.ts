...
test('local distillation strategy is RunPod-first and adapter-based', () => {
  ...
  assert.equal(LOCAL_DISTILLATION_STRATEGY.servingTopology, 'runpod_primary_deepinfra_fallback')
...
test('migration creates a service-only local artifact library and backfills prior distillation', () => {
  ...
  assert.match(migration, /runpod_primary_deepinfra_fallback/)
