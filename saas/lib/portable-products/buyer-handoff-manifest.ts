...
function hasNonEmptyEntry(values: readonly string[]): boolean { return values.some(value => value.trim().length > 0) }
...
export function createPortableBuyerHandoffManifest(input: PortableBuyerHandoffManifestInput): PortableBuyerHandoffManifest {
  ...
  return Object.freeze({ schemaVersion: portableBuyerHandoffManifestSchemaVersion, productId: input.productId, releaseVersion: input.releaseVersion, packageFormat: input.packageFormat, artifacts, buyerResponsibilities: Object.freeze([...input.buyerResponsibilities]), supplierResponsibilities: Object.freeze([...input.supplierResponsibilities]), exclusions: Object.freeze([...input.exclusions]), complete: blockers.length === 0, blockers: Object.freeze(blockers) })
}
