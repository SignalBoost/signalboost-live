...
function formatMaterialProvenance(provenance:any):string{
  ...
  if(provenance?.local_reasoning?.invoked)lines.push(`Local Reasoning Engine : ${provenance.local_reasoning.model||'local model'} — invoked.`)
  if(provenance?.external_ai?.invoked)lines.push(`External AI Provider   : ${provenance.external_ai.provider||'unknown'}${provenance.external_ai.model?` / ${provenance.external_ai.model}`:''} — invoked.`)
