...
export interface MonitoringAdapterContext { readonly sourceId:string }
...
export function createMonitoringIncidentSourceDefinition(adapterId:MonitoringAdapterId,context:MonitoringAdapterContext):IncidentSourceDefinition{
  ...
  switch(adapterId){case'datadog':return createDatadogDefinition(context.sourceId);case'pagerduty':return createPagerDutyDefinition(context.sourceId);case'aws-cloudwatch-eventbridge':return createAwsDefinition(context.sourceId);case'prometheus-alertmanager':return createAlertmanagerDefinition(context.sourceId);case'splunk':return createSplunkDefinition(context.sourceId);case'azure-monitor':return createAzureMonitorDefinition(context.sourceId);case'grafana-alerting':return createGrafanaDefinition(context.sourceId);case'google-cloud-operations':return createGoogleCloudDefinition(context.sourceId)}
}
