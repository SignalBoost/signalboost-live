...
import { absenceWindowSeconds, listObservationPolicies, observationDue } from '@/lib/supervisor/observation-policy'
...
export default async function SupervisorOperationsCenter({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const expiredLeasesWithWork = expiredLeases.filter(l => l.work_item_id || l.workItemId).length
  ...
  const missedWindows = enabledPolicies.filter(policy => {
    const parsed = Date.parse(lastObservationAt || '')
    if (!Number.isFinite(parsed)) return true
    return (Date.now() - parsed) / 1000 > absenceWindowSeconds(policy)
  }).length
  const leadPolicy = enabledPolicies[0] || null
  const due = leadPolicy ? observationDue(leadPolicy, lastObservationAt) : null
  ...
  const unverifiableRuntimes = activeInstances
    ...
    .map(i => String(i.instance_id || i.instanceId || 'unnamed runtime'))
  ...
  const execution = {
    model: t.executionModelValue || 'Scheduled observation, serverless. No persistent leader by design.',
    currentState: activeWork.length ? (t.runtimeObserving || 'Working') : (t.runtimeIdle || 'Idle — no work in progress'),
    nextObservation: due ? (due.due ? (t.observationDue || 'Due now') : due.reason) : (t.noPolicy || 'No observation policy is configured.'),
    lastCompleted: fmt(lastObservationAt),
    lastResult: latest ? `${latest.status} · ${latest.verification.status}` : (t.none || 'None'),
  }
  ...
  return <main style={page}>
    ...
    <OperationalAssessmentPanel assessment={assessment} forecast={forecast} execution={execution} t={t} />

    {/* Diagnostics collapse to one line. Eighteen green cards are not information. */}
    <Card title={t.systemDiagnostics || 'System diagnostics'}>
      <p style={strongText}>{diagnosticSummary.headline}</p>
      <p style={muted}>{diagnosticSummary.quiet ? (t.diagnosticsQuiet || 'Nothing here needs a person. These are internal measurements, not service states.') : (t.diagnosticsAttention || 'Some diagnostics need attention during working hours. None of them is an outage.')}</p>
      {diagnosticSummary.attention.length ? <details style={subcard}><summary>{`${t.diagnosticsNeedingAttention || 'Needing attention'} · ${diagnosticSummary.attention.length}`}</summary>{diagnosticSummary.attention.map(d => <article key={d.subsystemId} style={mini}><h3>{(t as any)[d.subsystemId] || d.subsystemId}</h3><dl style={fields}><Field k={t.status} v={d.label}/><Field k={t.explanation || 'What this means'} v={d.explanation}/><Field k={t.operationalImpactLabel || 'Operational impact'} v={d.impactStatement}/><Field k={t.recommendation || 'Recommendation'} v={d.recommendation || (t.noActionRequired || 'None.')}/></dl></article>)}</details> : null}
    </Card>

    {/* Incidents, split. Twelve verified failures are evidence the Supervisor worked. */}
    <div style={grid2}>
      <Card title={t.incidentQueue}>
        <details open style={subcard}><summary>{incidents.activeLabel}</summary>{incidents.active.length ? incidents.active.map(rec => { const r = incidentById.get(rec.runId); return <article key={rec.runId} style={mini}><h3>{`${r?.incident?.provider} · ${r?.incident?.incidentId}`}</h3><dl style={fields}><Field k={t.severity} v={rec.severity}/><Field k={t.verification} v={rec.status}/><Field k={t.evidence} v={r?.evidence.map(e=>e.summary).join(' | ')}/></dl></article> }) : <p style={muted}>{t.noOperationalIncidents || 'No incident is currently open.'}</p>}</details>
        <details style={subcard}><summary>{incidents.historicalLabel}</summary><p style={muted}>{t.recordedIncidentsMeaning || 'Observed, verified and recorded. These are evidence that the Supervisor worked, not live outages.'}</p>{incidents.historical.map(rec => { const r = incidentById.get(rec.runId); return <article key={rec.runId} style={mini}><h3>{`${r?.incident?.provider} · ${r?.incident?.incidentId}`}</h3><dl style={fields}><Field k={t.severity} v={rec.severity}/><Field k={t.verification} v={rec.status}/><Field k={t.auditTimeline} v={r?.auditEvents.map(e=>e.eventType).join(' → ')}/><Field k={t.metadata} v={JSON.stringify({ project:r?.projectId, environment:r?.environment, deployment:r?.governance?.deploymentId || r?.incident?.affectedResource || null })}/></dl></article> })}</details>
      </Card>
    ...
    <details style={panel}>
      <summary style={summaryText}>{t.engineeringView || 'Engineering detail'}</summary>
      <div style={grid2}>
        <Card title={t.subsystemHealth || 'Subsystem measurements'}><div style={cards}>{health.subsystems.map(s => { const d = diagnostics.find(x => x.subsystemId === s.id); return <article key={s.id} style={card}><h3>{(t as any)[s.id] || s.id}</h3><dl style={fields}><Field k={t.status} v={d ? d.label : s.status}/><Field k={t.metric || 'Metric'} v={s.metric ?? '—'}/><Field k={t.operationalImpactLabel || 'Operational impact'} v={d ? d.impactStatement : '—'}/><Field k={t.evidence} v={s.evidence.join(' · ') || s.summary}/></dl></article> })}</div></Card>
        <Card title={t.healthMetrics}><dl style={fields}><Field k={t.totalObservations} v={runs.length}/><Field k={t.successfulObservations} v={successful.length}/><Field k={t.verificationSuccess} v={verificationSuccess}/><Field k={t.avgObservationDuration} v={avg(durations)}/><Field k={t.queueDepth} v={activeWork.length}/><Field k={t.providerAvailability} v={providers.map(p => `${p.providerId}: ${p.health.state}`).join(' · ')}/><Field k={t.observationRate || 'Observation rate'} v={`${filteredRuns.length}/50`}/><Field k={t.incidentRate || 'Incident rate'} v={filteredRuns.length ? `${Math.round((incidentRuns.length/filteredRuns.length)*100)}%` : '—'}/></dl></Card>
      </div>
      ...
      <div style={grid2}>
        <Card title={t.recentAlerts || 'Raw rule firings'}>{health.alerts.length ? health.alerts.map(a => <article key={a.alertId} style={mini}><strong>{(t as any)[a.type] || a.type}</strong><p>{a.message}</p><Field k={t.evidence} v={a.evidence.join(' · ') || '—'}/></article>) : <p style={muted}>{t.noData}</p>}</Card>
        <Card title={t.trendGraphs || 'Trends'}><div style={cards}>{health.trends.map(tr => <article key={tr.bucket} style={mini}><h3>{(t as any)[tr.bucket] || tr.bucket}</h3><div style={{height:10,borderRadius:999,background:'rgba(255,255,255,.12)',overflow:'hidden'}}><span style={{display:'block',height:'100%',width:`${tr.score}%`,background:tr.score>89?'#38f2a4':tr.score>69?'#ffd166':'#ff5c7a'}} /></div></article>)}</div></Card>
      </div>
    ...
    <details style={panel}>
      <summary style={summaryText}>{t.auditView || 'Audit and evidence'}</summary>
      <Card title={t.healthAssessment || 'Health assessment'}>
        <dl style={fields}><Field k={t.healthScore || 'Health score'} v={ledger.score === null ? (t.notMeasured || 'Not measured') : `${ledger.score}%`}/><Field k={t.coverage || 'Coverage'} v={ledger.coverage}/><Field k={t.reconciles || 'Reconciles with the domain model'} v={ledger.reconciles ? (t.yes || 'Yes') : (t.no || 'No')}/></dl>
        <p style={muted}>{t.scoreMeaning || 'A consequence of the evidence above, not a substitute for it. Every point lost is itemised below.'}</p>
        {ledger.deductions.length ? <details style={subcard}><summary>{`${t.deductions || 'Deductions'} · ${ledger.deductions.length}`}</summary>{ledger.deductions.map(d => <article key={d.code} style={mini}><h3>{`${d.label} · −${d.points.toFixed(1)}`}</h3><dl style={fields}><Field k={t.why || 'Why'} v={d.why}/><Field k={t.evidence} v={d.evidence.join(' · ')}/><Field k={t.impact || 'Impact'} v={d.impact}/><Field k={t.confidence || 'Confidence'} v={d.confidence}/><Field k={t.recommendation || 'Recommendation'} v={d.recommendation}/></dl></article>)}</details> : <p style={strongText}>{t.noDeductions || 'No deductions. Every measured domain is at 100.'}</p>}
        {ledger.unmeasured.length ? <details style={subcard}><summary>{`${t.unmeasuredDomains || 'Unmeasured domains'} · ${ledger.unmeasured.length}`}</summary>{ledger.unmeasured.map(u => <p key={u.label} style={mini}>{`${u.label} — ${u.why}`}</p>)}</details> : null}
        {ledger.diagnostics.length ? <details style={subcard}><summary>{`${t.formulaDiagnostics || 'Measurement notes (affect nothing)'} · ${ledger.diagnostics.length}`}</summary>{ledger.diagnostics.map(d => <article key={d.code} style={mini}><p>{d.note}</p><Field k={t.remedy || 'Remedy'} v={d.remedy}/></article>)}</details> : null}
      </Card>
      <div style={grid2}>
        ...
        <Card title={t.observationPolicy || 'Observation policy'}><dl style={fields}>{enabledPolicies.map(p => <Field key={p.instanceId} k={p.instanceId} v={`${p.intervalSeconds}s · ${t.absenceWindow || 'absence window'} ${absenceWindowSeconds(p)}s · ${p.source}`}/>)}<Field k={t.policyRationale || 'Rationale'} v={leadPolicy?.rationale || t.none}/></dl></Card>
      </div>
