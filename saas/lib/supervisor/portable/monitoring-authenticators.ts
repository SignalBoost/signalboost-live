...
// timingSafeEqual throws when the two buffers differ in length, which would both
// crash intake and leak length through the error path. Hashing each side first gives
// two equal-length digests, so the comparison is uniform regardless of input.
...
function normalizeSecrets(secret: string | string[], label: string): string[] {
  ...
  for (const value of list) {
    // Checked when the deployment is wired, not when an alert arrives. Discovering a
    // weak secret during an incident is the worst possible moment.
...
// ── Shared secret ────────────────────────────────────────────────────────────
// Datadog, Splunk, Azure Monitor and Google Cloud all authenticate by echoing a value
// the operator configured. The vendor sends it in a header of their choosing.
export function createSharedSecretAuthenticator(options: { secret: string | string[]; headerName: string }): DeliveryAuthenticator {
  ...
  return (delivery) => {
    ...
    // whichever key in a rotation set was used.
    let matched = false
...
// ── HMAC signature ───────────────────────────────────────────────────────────
// PagerDuty signs the raw body; several others follow the same pattern with a
// different header, encoding or prefix. `signedPayload` covers the variants that
// include a timestamp in the signed material.
...
export function createHmacSignatureAuthenticator(options: HmacSignatureOptions): DeliveryAuthenticator {
  ...
  return (delivery) => {
    ...
    if (timestampHeader) {
      ...
      // The timestamp goes INSIDE the signed material, so a captured request cannot
      // be replayed with a fresh one — restamping invalidates the signature. Without
      // that, a replay window is decoration.
      signedPayload = `${raw}.${signedPayload}`
    }

    // A vendor may send several signatures in one header during key rotation.
...
// ── Explicit opt-out ─────────────────────────────────────────────────────────
// Some deployments genuinely terminate authentication upstream — an API gateway, a
// service mesh, mutual TLS. That is legitimate, but it must be a DECISION someone
// wrote down rather than the accidental result of leaving authentication unset, so it
// carries a required reason that surfaces wherever the source is reported.
