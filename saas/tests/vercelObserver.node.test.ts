...
test('latest failed deployment is selected even when a newer deployment is healthy', async () => { const out = await incidents([dep({ id: 'ready-newer', state: 'READY', createdAt: now.getTime() - 1_000 }), dep({ id: 'failed-older', state: 'ERROR', createdAt: now.getTime() - 2_000 })]); assert.equal(out[0].affectedResource, 'failed-older') })
