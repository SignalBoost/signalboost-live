      ...
      <section className="mx-auto max-w-6xl">
        ...
        <div className="mt-8 overflow-hidden rounded-[2rem] border border-cyan-300/20 bg-[radial-gradient(circle_at_top_right,rgba(34,211,238,.14),transparent_35%),linear-gradient(145deg,rgba(15,23,42,.98),rgba(2,6,23,.98))] p-7 shadow-2xl shadow-cyan-950/30 md:p-12">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className="rounded-full border border-emerald-300/30 bg-emerald-300/10 px-3 py-1 text-xs font-bold text-emerald-100">Real implementation</span>
          </div>

          <div className="mt-8 grid gap-8 lg:grid-cols-[1.4fr_.8fr] lg:items-start">
            <div>
              <p className="text-sm font-black uppercase tracking-[0.2em] text-cyan-200">Governed AI connectivity for enterprise</p>
              <h1 className="mt-4 text-5xl font-black tracking-tight md:text-7xl">Agent Gateway</h1>
              <p className="mt-6 max-w-3xl text-lg leading-8 text-slate-300 md:text-xl">
                Connect AI agents, enterprise applications, APIs, automation platforms, robots, and industrial systems through one governed gateway with consistent policy, approvals, runtime coordination, provider integration, diagnostics, and immutable evidence.
              </p>
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                <Link href="/dashboard" className="rounded-xl bg-cyan-300 px-5 py-3 text-center font-black text-slate-950 transition hover:bg-white">Open SignalBoost</Link>
                <Link href="/pricing" className="rounded-xl border border-white/15 px-5 py-3 text-center font-black transition hover:border-cyan-300/60 hover:text-cyan-100">View pricing</Link>
              </div>
            </div>
        ...
        <section className="mt-8 rounded-3xl border border-amber-300/20 bg-amber-300/[0.06] p-6 md:p-8">
          <p className="text-sm font-black uppercase tracking-[0.18em] text-amber-200">Governance boundary</p>
          <h2 className="mt-3 text-3xl font-black">Minimum Human Work. Maximum Human Control.</h2>
        ...
        <section className="mt-8 rounded-3xl border border-white/10 bg-white/[0.035] p-6 md:p-8">
          <p className="text-sm font-black uppercase tracking-[0.18em] text-violet-200">What is implemented</p>
