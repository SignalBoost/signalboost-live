...
**Builder verification order and generalization — 2026-09-06:** `fix/builder-verification-order` addresses the observed rounding job's sequencing failure. Explicit run-command-before-changing-named-file instructions now gate mutations in the existing tool loop; when new tests/assertions are requested, a baseline run before those test edits is insufficient. Real completed failures count as baseline evidence, timeouts do not. The gate derives from the original objective and recorded trace across checkpoints. Generalization guidance asks for boundary/counterexample checks and rejects input-specific patches or arbitrary tolerances as a substitute for the requested rule. An independent integer-decimal probe found 30 failures among 10,102 cases in the prior live rounding source despite its twelve passing tests; the observed source and probe are preserved under `docs/fixtures/builder-rounding`. This is a narrow deterministic ordering contract plus generalization guidance/evaluation, not a universal semantic-order parser or evidence of learned-inference transfer. Exact gates, deployment and fresh live repair/probe results remain to be recorded.

**COS Software Specialist inference application — 2026-09-06:** `feat/cos-specialist-inference-application-v2-20260906` extends the newly restored same-project repair signals with governed cross-task procedural inference. Ordinary Builder, bounded debug, and owner-authorized Platform Engineer work retrieve only relevant `validated`, `learned`, or `mastered` cognitive procedures scoped to the Software Specialist and inject them as advisory reasoning guidance; current workspace evidence remains authoritative and retrieval grants no authority. A retrieved procedure receives a production-success record only when Builder writes/edits a file and a host-run command exits zero. Failed, read-only, paused, or merely generated answers earn no application credit. Outcome recording uses the existing service-role-only cognitive production RPC; no schema or RLS change is introduced. Production currently has two owner-directed software lessons captured and zero software cognitive candidates, so candidate evaluation must occur before this path can retrieve those lessons. TypeScript and all 978 mandatory tests pass; merge, deployment, and live application evidence remain pending.
