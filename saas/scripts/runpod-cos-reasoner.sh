...
# SignalBoost COS — RunPod one-command independent reasoner bootstrap.
#
# Persistent state lives under /workspace so a RunPod container reset does not
# require downloading the model again. Ollama itself is reinstalled when the
# ephemeral container image is recreated.
#
# Public surface:
#   0.0.0.0:11434  -> authenticated gateway only
# Private surface:
#   127.0.0.1:11435 -> Ollama OpenAI-compatible API
#
...
# Remove only processes previously started by this script. A stale manually-started
# Ollama on our ports is reported clearly rather than silently killed.
stop_pid_file "$GATEWAY_PID_FILE"
stop_pid_file "$OLLAMA_PID_FILE"

if ss -ltn 2>/dev/null | grep -q ':11435 '; then
  log 'ERROR: port 11435 is already in use. Stop the old Ollama process and run this script again.'
  exit 1
fi
if ss -ltn 2>/dev/null | grep -q ':11434 '; then
  log 'ERROR: port 11434 is already in use. Stop the old public Ollama/Nginx process and run this script again.'
  exit 1
fi
...
class Handler(BaseHTTPRequestHandler):
    ...
    def log_message(self, fmt, *args):
        print('[cos-gateway] ' + (fmt % args), flush=True)
    ...
    def _proxy(self):
        ...
        try:
            ...
            payload = json.dumps({'error': 'upstream_unavailable', 'detail': str(exc)}).encode('utf-8')
...
log 'Public RunPod port: 11434 (must remain exposed as HTTP in Pod settings)'
log 'Vercel LOCAL_AI_MODEL should match the model above.'
log 'Vercel LOCAL_AI_BASE_URL should be your RunPod 11434 proxy URL plus /v1.'
log 'Vercel LOCAL_AI_ALLOWED_HOSTS should be the exact hostname from that proxy URL.'
log 'Vercel LOCAL_AI_API_KEY must equal the secret stored in the key file.'
log "To copy the key privately into Vercel, run locally in this terminal: cat $KEY_FILE"
