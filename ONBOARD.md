...
**Next Builder task:** correct task and conversation intent routing. The documentation-only request to extend README with guidance about an existing error was treated as a code repair, resulting in `builder_regression_evidence_required` despite the requested edit and fifteen passing tests. Its fallback also incorrectly said proof had not passed. A Does-question about an explicit Builder job's source identity entered generic answer-provenance handling; the equivalent Explain-question correctly used Builder evidence. Preserve genuine failure-first repair requirements while recognizing what action the user actually requested. Broad legacy repair narration still needs useful supported explanations. After these gaps, assess authorized validated-procedure use and inference transfer on unfamiliar tasks. Engineering owns implementation and verification; owner certification is not a prerequisite.
