...
test('terminal cost settlement rounds conservatively by minute and cannot exceed the stage reserve', () => {
  ...
  assert.equal(conservativeObservedHuggingFaceCostUsd({
    job: baseJob, hourlyCostUsd: 0.4, reservedCostUsd: 0.2,
  }), 0.013333)
  assert.equal(conservativeObservedHuggingFaceCostUsd({
    job: { ...baseJob, totalSeconds: 5000 }, hourlyCostUsd: 0.4, reservedCostUsd: 0.2,
  }), 0.2)
  assert.equal(conservativeObservedHuggingFaceCostUsd({
    job: { ...baseJob, totalSeconds: null, runningSeconds: null }, hourlyCostUsd: 0.4, reservedCostUsd: 0.2,
  }), 0.2)
})
