  ...
  try {
    ...
    if (body.claim === 'trained_artifact_registered' || body.claim === 'rollback_artifact_registered') {
      try {
        ...
        // The signed training evidence has already been admitted. Never make the external worker retry
        // a valid callback merely because local adoption reconciliation is temporarily unavailable;
        // the hourly controlled-fine-tuning reconciliation retries this non-spending step.
