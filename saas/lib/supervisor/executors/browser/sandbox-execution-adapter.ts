...
import { createBrowserExecutionId, type BrowserExecutionStore, type BrowserSessionRegistry } from '../../../browser-runtime/execution-state.ts'
...
import { liveSandboxExecutionStore, liveSandboxSessionRegistry } from './live-sandbox-runtime.ts'
...
import { approvalRequestSchemaVersion, type ApprovalQueueStore } from '../../approvals/index.ts'
...
export class SandboxExecutionAdapter {
  ...
  constructor(deps: SandboxExecutionAdapterDeps) {
    this.deps = deps
    this.executionStore = deps.executionStore ?? liveSandboxExecutionStore
    this.sessionRegistry = deps.sessionRegistry ?? liveSandboxSessionRegistry
  ...
  private async createApprovalRequest(executionId: string, request: SandboxBrowserExecutionRequest, promoted: PromotedSandboxPackage, runtimeResult: BrowserTaskResult): Promise<void> { if (!this.deps.approvalQueue) return; const nowIso = ts(this.deps.now?.()); const summaries = promoted.postCheckpointStepIds.map(stepId => ({ stepId, summary: `step ${stepId}`, expectedResult: 'sandbox verification', verificationRequirement: 'deterministic runtime verification', riskLevel: 'sandbox' })); await this.deps.approvalQueue.createRequest({ approvalRequestId: `${executionId}:approval`, executionId, dispatchId: request.dispatchId, incidentId: request.incidentId, planId: request.planId, packageId: promoted.metadata.packageId, packageFingerprint: request.packageFingerprint, provider: promoted.task.provider, environment: 'sandbox', targetOrigin: promoted.metadata.sandboxOrigin, checkpointId: promoted.checkpointStepId, remainingStepIds: promoted.postCheckpointStepIds, protectedStepSummaries: summaries, evidenceReferences: runtimeResult.evidence.map(e => ({ evidenceId: `${executionId}:${e.sequence}:${e.stepId}`, stepId: e.stepId, kind: e.kind, artifactReference: e.artifactRef, summary: e.summary, capturedAt: e.timestamp })), phaseOneApprovalDigest: digestBrowserApprovalToken(request.browserTaskApprovalToken), requestedAt: nowIso, expiresAt: promoted.task.expiresAt, status: 'pending', schemaVersion: approvalRequestSchemaVersion }); if (this.deps.persistentStore) await this.deps.persistentStore.appendAuditEvent({ eventId: eventId('approval_request_created', request.dispatchId, executionId), executionId, dispatchId: request.dispatchId, incidentId: request.incidentId, eventType: 'approval_request_created', occurredAt: nowIso, payload: { approvalRequestId: `${executionId}:approval`, remainingStepIds: promoted.postCheckpointStepIds, targetOrigin: promoted.metadata.sandboxOrigin }, schemaVersion: auditRecordSchemaVersion, createdAt: nowIso }) }
