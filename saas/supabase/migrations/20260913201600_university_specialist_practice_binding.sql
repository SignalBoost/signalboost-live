-- saas/supabase/migrations/20260913201600_university_specialist_practice_binding.sql
-- Supersede the software-only practice-result identity literal. The queue/plan/study-round fences,
-- service-role boundary and non-credit semantics remain unchanged; only the registered specialist
-- role/runtime mapping becomes role-generic and exact.
...
begin
  ...
  if q.metadata->>'origin' = 'cos_university_deliberate_practice' then
    ...
    if university_agent_id <> 'cos' then
      -- Keep the host identity stable until the transaction commits; unregistered/generalist roles fail closed.
      select role into registered_role from public.cos_university_agent_registry
      where agent_id = university_agent_id for share;
      registered_runtime := public.cos_university_specialist_runtime_for_role(registered_role);
      if registered_runtime is null
      ...
      if (jsonb_typeof(execution) = 'object'
        and execution->>'runtime' = registered_runtime
        ...
        and p_evidence->>'responseSource' = registered_runtime
...
comment on function public.cos_record_cognitive_practice_result(uuid, boolean, double precision, text, jsonb) is
  'Canonical service-only recorder: exact learner/accepted-study round locked atomically; non-COS execution bound to that registered specialist role/runtime; terminal failures reopen study; practice never awards academic credit.';
