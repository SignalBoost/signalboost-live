...
export function translateClusterTransition(commit: ClusterTransitionCommit): readonly ClusterRuntimeInstruction[] {
  ...
  if (commit.promotedReplicaId) {
    instructions.push(createInstruction({ clusterId: commit.clusterId, term: commit.term, replicaId: commit.promotedReplicaId, action: 'promote', reason: 'committed cluster leader promotion' }))
  }
  for (const replicaId of [...commit.demotedReplicaIds].sort()) {
    instructions.push(createInstruction({ clusterId: commit.clusterId, term: commit.term, replicaId, action: 'demote', reason: 'committed conflicting or previous leader demotion' }))
  }
  if (instructions.length === 0) {
    instructions.push(createInstruction({ clusterId: commit.clusterId, term: commit.term, action: 'noop', reason: 'cluster transition requires no runtime membership change' }))
  }
