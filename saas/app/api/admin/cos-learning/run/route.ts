...
async function runManualLearning() {
  ...
  try {
    ...
    return NextResponse.json({ ok: true, triggeredBy: access.email || access.userId, summary: mining.summary, learning })
  } catch (error) {
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : String(error), summary: mining.summary }, { status: 500 })
  }
}
