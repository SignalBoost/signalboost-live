...
**COS communication-intelligence follow-up:** implementation is active on `fix/cos-integrated-communication-profile-20260904` and is **not Production-accepted**. The existing semantic register pass now returns one integrated profile covering objective, audience relationship, authentic voice, emotional stakes, supported seniority/experience, rhetorical elements, required transformations and publication risks. Generation and final review must preserve supported emotional/experiential meaning without copying risky wording, transform rather than merely delete the point beneath disparagement, reject generic memo structure and invented signature placeholders, and reject unsupported categorical claims about fairness, efficiency, morale or institutional outcomes. This is an extension of the existing neural writer/quality-board path, not ten new rule engines. Focused deterministic regression is green; exact dependency-backed typecheck/build/deployment gates and a fresh real COS output observation remain required.
