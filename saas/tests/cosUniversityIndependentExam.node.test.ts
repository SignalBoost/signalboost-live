...
test('cron is secret-gated, scheduled after study, and never returns prompt, reply, or rubric', () => {
  ...
  assert.deepEqual(
    vercel.crons?.find(cron => cron.path === '/api/cron/cos-university-exam'),
...
test('ready exam queue recovers one no-valid-execution error and scans past blocked rows', () => {
  const runner = file('../lib/ai/cos/cosUniversityIndependentExamRunner.ts')
  const recoveryStart = runner.indexOf('function retryableInfrastructureError')
  const recoveryEnd = runner.indexOf('async function createOrFindRun', recoveryStart)
  assert.ok(recoveryStart >= 0 && recoveryEnd > recoveryStart)
  const recovery = runner.slice(recoveryStart, recoveryEnd)
  assert.match(recovery, /reason\.startsWith\('execution_error:'\)/)
  assert.match(recovery, /fresh_execution_required/)
  assert.match(recovery, /empty_reply/)
  assert.match(recovery, /not_handled/)
  assert.doesNotMatch(recovery, /exam_manifest_drift|agent_execution_identity_mismatch|execution_response_hash_missing|scored_reply_execution_mismatch/)
  assert.match(runner, /infrastructure-retry:\$\{existing\.id\}/)
  assert.match(runner, /if \(retryExisting\) return retryExisting/)
  assert.match(runner, /READY_PLAN_SCAN_MULTIPLIER = 12/)
  assert.match(runner, /MAX_READY_PLAN_SCAN = 24/)
  assert.match(runner, /if \(run\.status === 'not_claimed'\) continue/)
  assert.match(runner, /if \(executionSlots >= maxExams\) break/)
  assert.match(runner, /run\.status === 'error'\) executionSlots \+= 1/)
})
