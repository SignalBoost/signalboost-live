// saas/tests/renderCore.node.test.ts
//
// Proves the portable render engine in ISOLATION — no SignalBoost, no network,
// no real money. This is the "sellable module" proof: the collect-before-spend
// and refund-on-failure guarantees hold against fake adapters a buyer could swap
// in. Run: node --test tests/renderCore.node.test.ts
...
function fakeHost(balanceCents: number) {
  ...
  const host: RenderHost = {
    ...
    storage: {
      async persist() { state.persisted += 1; return { url: 'https://fake/out.mp3' } },
    },
    log: { log: () => {} },
