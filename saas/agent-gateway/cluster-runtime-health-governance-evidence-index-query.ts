...
export function queryClusterRuntimeHealthGovernanceEvidenceIndex(
  ...
  const artifactIds = uniqueStrings(rawQuery['artifactIds'], 'artifactIds')
  const kinds = uniqueStrings(rawQuery['kinds'], 'kinds')
  const schemas = uniqueStrings(rawQuery['schemas'], 'schemas')
  const provenance = uniqueStrings(rawQuery['provenance'], 'provenance')
  const limitValue = rawQuery['limit']
