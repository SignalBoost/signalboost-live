...
async function wakeMassDistilledRuntime(endpointId: string, deadlineMs: number) {
  ...
  return Object.freeze({
    ok: true as const,
    endpointId,
    // /ping reports whether the loaded model is ready; it returns no model list. Reading a model-list length here
    // was left over from the /v1/models wake and threw "Cannot read properties of undefined (reading 'length')"
    // on every evaluation after the wake was repointed (Production 2026-09-17 20:01 and 20:10 UTC).
