...
/**
 * The single safe readiness projection for SignalBoost social connectors.
 * It deliberately returns no OAuth token, refresh token, client id, or client secret.
 * Provider Hub and the existing social cockpit both consume this projection so the
 * platform list/readiness rules cannot drift into separate implementations.
 */
export async function loadSocialCapabilityInventory(
  ...
  // any connector lookup. Built-in discovery below intentionally follows SOCIAL_CONNECTORS,
  // the same canonical list the existing cockpit has always exposed.
