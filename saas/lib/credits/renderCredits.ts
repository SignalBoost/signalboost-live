...
// Server-side render-credit wallet. Customers prepay before paid-provider use;
// the verified platform owner has unlimited internal access while provider cost
// is still recorded for accounting and observability.
...
export async function hasUnlimitedRenderCredits(userId: string): Promise<boolean> {
  const entitlements = await getOwnerEntitlements(userId)
  return entitlements.unlimitedCredits
}
...
export type ChargeResult =
  | { ok: true; ledgerId: string; creditsCharged: number; newBalance: number; unlimited?: boolean }
  | { ok: false; code: 'insufficient_credits' | 'daily_cap' | 'error'; message: string; balance?: number; needed?: number }

async function recordOwnerUsage(args: {
  userId: string
  provider: string
...
export async function chargeForRender(args: {
  ...
  const entitlements = await getOwnerEntitlements(args.userId)
  if (entitlements.unlimitedCredits) return recordOwnerUsage(args)
...
export async function refundRender(userId: string, ledgerId: string): Promise<void> {
  ...
  if (credits > 0) {
    await admin.rpc('increment_render_credits', { target_user_id: userId, add_amount: credits })
  }
