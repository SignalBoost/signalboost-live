...
function confidenceFromOutreach(row: any) {
  ...
  // These rows already consumed discovery/analyzer work. Give them useful baseline
  // confidence without pretending every historical field was independently verified.
  let score = 0.58
...
export function outreachRowToCorpusRecord(row: any): BusinessIntelligenceRecord | null {
  ...
  return {
    canonicalDomain,
    companyName,
    aliases: arrayOfStrings(analyzer.aliases),
    ...
    technologies: [
      ...arrayOfStrings(analyzer.technologies),
      ...arrayOfStrings(website.technologies),
      ...arrayOfStrings(profile.technologies),
    ].filter((value, index, all) => all.indexOf(value) === index),
    contacts: contactEmail ? [{ email: contactEmail }] : [],
    attributes: {
      outreachQueueId: String(row.id || ''),
      businessId: row.business_id || null,
      sourcePlatform: row.source_platform || null,
      outreachStatus: row.status || 'pending',
      analyzerSummary: analyzer,
      businessModelProfile: profile,
      predictiveNeeds: needs,
      websiteAnalysis: website,
      reviewStrategy: row.review_strategy || {},
      socialPlan: row.social_plan || {},
      promoPlan: row.promo_plan || {},
      previouslyResearched: true,
      paidDiscoveryReuse: true,
    },
    confidence: confidenceFromOutreach(row),
    sourceType: 'learned',
...
function mergeRecords(current: BusinessIntelligenceRecord, incoming: BusinessIntelligenceRecord): BusinessIntelligenceRecord {
  const contacts = [...current.contacts || [], ...incoming.contacts || []]
  ...
  return {
    ...
    employeeCount: incoming.employeeCount ?? current.employeeCount,
    revenueUsd: incoming.revenueUsd ?? current.revenueUsd,
    website: incoming.website || current.website,
    description: incoming.description || current.description,
    technologies: [...new Set([...(current.technologies || []), ...(incoming.technologies || [])])],
    contacts,
    attributes: {
      ...current.attributes,
      ...incoming.attributes,
      outreachHistoryIds: [...new Set([
        ...((current.attributes.outreachHistoryIds as string[] | undefined) || []),
        ...current.sourceIds,
        ...incoming.sourceIds,
      ])],
    },
    ...
    verifiedAt: Date.parse(incoming.verifiedAt) > Date.parse(current.verifiedAt) ? incoming.verifiedAt : current.verifiedAt,
    refreshedAt: Date.parse(incoming.refreshedAt) > Date.parse(current.refreshedAt) ? incoming.refreshedAt : current.refreshedAt,
    expiresAt: Date.parse(incoming.expiresAt) > Date.parse(current.expiresAt) ? incoming.expiresAt : current.expiresAt,
  }
}
...
export async function loadExistingOutreachCorpusRecords() {
  ...
  for (let from = 0; ; from += PAGE_SIZE) {
    const { data, error } = await admin
      .from('outreach_queue')
      .select('id,business_id,source_platform,business_name,business_url,analyzer_summary,business_model_profile,predictive_needs,website_json,review_strategy,social_plan,promo_plan,status,contact_email,created_at,updated_at,approved_at,sent_at')
...
export async function seedCorpusFromExistingOutreachHistory() {
  ...
  return {
    ...
    rawOutreachRows: loaded.rawRows,
    uniqueCompanies: loaded.records.length,
    unusableRows: loaded.unusableRows,
    attempted: loaded.records.length,
    succeeded,
    ...
    providerCalls: 0,
    externalAiCalls: 0,
    strategy: 'reuse_existing_paid_discovery_first',
  }
}
