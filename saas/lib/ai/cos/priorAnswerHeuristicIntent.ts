...
const NON_ENGLISH_PATTERNS = [
  /(?=.*\b(?:heur[ií]sticas?|reglas?|instrucciones?|pol[ií]ticas?|criterios?)\b)(?=.*\b(?:tu|tus|su|respuesta|salida|resultado)\b)(?=.*\b(?:influy\w*|afect\w*|determin\w*|usaste|aplic\w*|por\s+qu[eé])\b)/i,
  /(?=.*\b(?:heur[ií]sticas?|regras?|instru[cç][oõ]es?|pol[ií]ticas?|crit[eé]rios?)\b)(?=.*\b(?:sua|seu|resposta|sa[ií]da|resultado)\b)(?=.*\b(?:influenc\w*|afet\w*|determin\w*|usou|aplic\w*|por\s+qu[eê])\b)/i,
  /(?=.*\b(?:heurystyk\w*|zasad\w*|instrukcj\w*|polityk\w*|kryteri\w*)\b)(?=.*\b(?:twoj\w*|odpowied[zź]\w*|wynik\w*)\b)(?=.*\b(?:wp[lł]yn\w*|kszta[lł]t\w*|zdecyd\w*|u[zż]y\w*|dlaczego)\b)/i,
  /(?=.*(?:эвристик\w*|правил\w*|инструкц\w*|политик\w*|критери\w*))(?=.*(?:ваш\w*|твой\w*|ответ\w*|результат\w*))(?=.*(?:влиял\w*|определ\w*|использ\w*|примен\w*|почему))/i,
]

export function asksWhichHeuristicsInfluencedPriorAnswer(input: string): boolean {
  const text = String(input || '').trim()
  if (!text || text.length > 400) return false
  if (NON_ENGLISH_PATTERNS.some(pattern => pattern.test(text))) return true
